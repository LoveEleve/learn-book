...
>
> Raft 今天是工业界主流共识算法——etcd / TiKV / CockroachDB / Nacos / SOFA-JRaft / Consul 全部基于它。**理解 Raft 是理解现代分布式系统的钥匙**。
>
...
**`candidate_log_is_up_to_date` 规则**（选举限制，第 5.2 节详解）：
...
**跨地域集群**：超时必须放大。北京-上海 RTT 30ms，超时至少 100ms；中美 RTT 200ms，超时至少 500ms。
...
**关键不变量**（第 5 节详解）：
- 如果两条日志条目在相同 index 且 term 相同，则命令相同
- 如果两条日志条目在相同 index 且 term 相同，则之前所有条目相同
...
**关键约束**（提交限制，第 5.3 节详解）：
- Leader 只能提交**当前 term** 的日志条目
- 不能提交旧 term 的日志条目（即使它们已被多数派复制）
...
| 提交规则 | 任意已复制条目可提交 | 只能提交当前 term 的条目 |
| 选举 | 未明确给出（工程自己实现） | 内置（RequestVote RPC + 随机超时） |
| 成员变更 | 联合共识（复杂） | 单节点变更（简化） |
...
**现代系统倾向 Raft**——etcd / TiKV / CockroachDB / Nacos / Consul / SOFA-JRaft 都基于它。
...
- 强一致读写（默认 Leader Read，可选 Lease Read）
...
**Nacos Raft 的特点**（基于 SOFA-JRaft 改造）：
- 持久实例存储在 Raft 日志
- 选举超时：`raft.electionTimeout=5000`（ms，默认）
- 心跳间隔：`raft.heartbeatPeriodMs=500`（ms，默认）

> **待 MCP 核实声明**：Nacos 2.x 引入 gRPC 长连接后，Distro/Raft 协议路由实现有调整，但持久/临时实例的协议边界保留。具体路由代码在 `naming/consistency/persistent` 与 `naming/consistency/ephemeral` 包。
...
**应用场景**：
- 蚂蚁内部多个系统（如分布式锁、配置中心）
- Nacos 持久实例的 Raft 实现（基于 SOFA-JRaft 改造）
...
**认知 3：Raft 是现代分布式系统的主流共识算法**
- etcd / TiKV / CockroachDB / Nacos / Consul / SOFA-JRaft 全部基于它
- 理解 Raft 是理解这些系统的前提
- Paxos 仍在历史系统使用，但新系统倾向 Raft
...
**本篇的核心论断**：Raft 不是新算法，而是 Paxos 的工程化重新设计——通过强 Leader + 三子问题分解 + 状态简化，让共识算法易理解、易实现、易维护。理解 Raft 是理解现代分布式系统（etcd / TiKV / CockroachDB / Nacos）的钥匙。
