...
1. **完整容错体系**：不只熔断/限流，而是**熔断/降级/限流/隔离/重试/超时/热点防护/系统自适应保护**的组合——Sentinel 的 slots 链（flow/degrade/authority/system/hot-param 等）。注意：Sentinel 的 system 保护是**应用内** QPS/负载自适应保护，与第 7 节 Tomcat 容器限流（线程数/连接数）是不同层
2. **雪崩防护**：容错的核心目标是**防止级联故障（雪崩）**——下游故障 → 熔断 → 快速失败 → 上游不被拖垮；这是容错的架构级意义
3. **熔断状态机细节**：CLOSED→OPEN→HALF_OPEN 转换已源码验证（见 KP-02）；此处补 KP-02 未展开的——熔断恢复策略（探测请求/冷却时间）、滑动窗口统计口径
