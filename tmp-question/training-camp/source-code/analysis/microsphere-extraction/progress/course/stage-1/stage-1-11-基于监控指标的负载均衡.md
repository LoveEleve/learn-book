...
### 完整认知：基于监控指标的负载均衡在真实架构中完整该讲什么

docs 覆盖了"监控指标 + JMX + Eureka"。作为架构师，这个主题完整还该包含：

1. **监控指标体系全貌**：不只 CPU/Load，而是**完整的服务健康指标集**——CPU/内存/线程/GC/RT/QPS/错误率/连接数（衔接第 13 节 Micrometer 指标模型），以及这些指标的业务含义
2. **基于指标的负载均衡算法**：不只"读 cpu-usage"，而是**完整算法设计**——如何选实例（最小负载/加权/一致性哈希/延迟感知）、指标来源（实例上报 vs 中心统计）、权重动态更新（衔接第 12 节动态权重）
3. **指标上报链路**：实例如何上报指标到注册中心 metadata——**上报时机/频率/一致性**（指标是快照，有过期性），负载均衡读时如何处理过期
4. **负载均衡在调用链的位置**：客户端负载均衡(第 4 节 RestTemplate/Feign 走 LoadBalancer) vs 网关负载均衡（网关本身是入口，可做集中分发，见第 19 节网关稳定性；网关负载均衡为架构师发散，非 stage-1 专门章节）——两种位置
5. **指标采集实现**：JMX(第 11 节 KP-02/03) vs Micrometer(第 13 节) vs 其他——指标采集的技术选型
6. **注册中心的角色**：注册中心不只存"地址"，还存"指标元信息"(metadata)——Nacos(第 11 节) 承载 cpu-usage 等（Eureka 过时）

### 关键决策与权衡

| 决策 | 权衡 |
|------|------|
| 实例上报 vs 中心采集指标 | 实例上报(metadata)简单但有延迟/过期；中心主动采集(如 Prometheus，第 15 节 Pull)实时但复杂 |
| 指标权重 vs 轮询 | 权重(智能)需指标+算法；轮询(简单)均衡但不考虑负载 |
| 客户端 vs 网关负载均衡 | 客户端(分散，第 4 节)灵活；网关(集中，入口层可做，见第 19 节稳定性)治理统一 |
| 指标快照 vs 实时 | 快照简单但过期；实时准确但开销大 |
| 一致性哈希 vs 最小负载 | 一致性哈希(会话保持/缓存友好)；最小负载(性能最优) |

### 常见坑/反模式

1. **指标过期导致误判**：实例上报的 cpu-usage 是历史快照，可能过期——负载均衡读到旧数据选错实例
2. **算法未实现(像 biz-client)**：只"读到指标"没真正实现选实例算法(TODO)——参考实现不完整(见小结置信度 Medium)
3. **指标采集开销**：频繁 JMX/采集拖累性能——要控制采集频率
4. **metadata 指标缺失**：实例没上报指标，负载均衡读到空/异常——要有兜底(回退轮询)
5. **指标口径不一致**：各实例上报的指标口径不同(CPU 计算方式)，不可比——要统一口径
6. **注册中心耦合指标**：把过多/过大的指标塞进 metadata，注册中心压力大——权衡放 metadata vs 独立监控(第 15 节 Prometheus)

### 生态位置

- **衔接**：第 4 节(客户端负载均衡)、第 12 节(动态权重)、第 13 节(Micrometer 指标)、第 15 节(Pull 监控)、第 19 节(网关稳定性；网关负载均衡为架构师发散)
- **注册中心**：Nacos(主流，第 11 节)承载指标元信息；Eureka 过时
- **LoadBalancer 双扩展点**：ServiceInstanceListSupplier + ReactorServiceInstanceLoadBalancer(第 11 节 KP-06)
- **性能优化维度 + 分布式问题维度**的交汇——监控指标(性能)服务负载均衡(分布式)

**架构师视角结论**：本篇不只是"用 JMX 读 CPU 做负载均衡"，而是"**设计基于实时指标的智能负载均衡**"——指标采集、上报链路、选实例算法、权重动态更新，让流量分布到最健康的实例，是"三高"下智能流量调度的一部分。
