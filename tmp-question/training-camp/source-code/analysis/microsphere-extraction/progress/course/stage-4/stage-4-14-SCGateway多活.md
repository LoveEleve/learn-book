...
- **参考实现**（docs 机制简述 + 本地实证——2026-08-12 重写：源码块照录降为机制）：**动态路由机制（docs:102-177）**——①**Holder 注册**：RouterFunctionHolder（装载配置路由构建的 RouterFunction）以 **scope="refresh" 代理**注册（docs:115-118——**ContextRefresher 刷新时代理重建**）；②**门面**：DelegatingRouterFunction 注入被代理 Holder（docs:124-129——刷新后 Supplier 重调 → 新路由生效——docs:136）；③**路由构建**：Binder 绑定 GatewayMvcProperties → routes/routesMap 遍历构建（docs:138-168——空则 NEVER_ROUTE）；④**合并**：RouterFunctionMapping 合并自定义 Bean + 配置规则（docs:172-177）；**本地实证**——`GatewayMvcPropertiesBeanDefinitionRegistrar` **`[本地实证：server-mvc `config/`——RouterFunctionHolder（:85 嵌套类）与 DelegatingRouterFunction（:103 嵌套类）为 Registrar 文件内嵌套类定义（docs 视作独立类）+ `RouterFunctionHolderFactory` 存在——docs 为早期实现快照]`**；**04 篇衔接**——refresh scope + ContextRefresher（04 篇 @RefreshScope 机制——**本篇是"refresh scope 代理路由"的具体应用**——HANDOVER 教训 4 现场延续）——**若我实现配置路由动态化会与此一致**（refresh scope 代理是 Spring 生态的标准热更新手段——Holder+门面解耦"配置构建"与"路由使用"）
...
**参考实现**：docs 意图照录 + Server MVC 5 类本地实证 + 3 源码块机制简述（2026-08-12 重写——照录降为机制）+ stage-3 19/20 交叉（Reactive 面）+ 04 篇衔接（refresh scope）。
