...
- **参考实现**（docs 机制简述 + 本地实证——2026-08-12 重写：源码块照录降为机制）：**默认装配三件（docs:125-199）**——①**LB Bean**：默认 RoundRobinLoadBalancer（docs:127——`@ConditionalOnMissingBean` 可覆盖），**Supplier 从 Factory 懒加载**（getLazyProvider——docs:134-135）；②**Supplier 双实现**：Reactive（`@ConditionalOnReactiveDiscoveryEnabled`——WebClient 类存在且 reactive.enabled 默认 true——docs:147-151）/Blocking（`@ConditionalOnBlockingDiscoveryEnabled`——docs:177-181）——**均 `withDiscoveryClient().withCaching().build()`**（发现 + 缓存——docs:166/190）；③**Factory**：LoadBalancerClientFactory（PROPERTY_NAME "loadbalancer.client.name"——docs:199——Ribbon 的 "ribbon.client.name" 对应物）；**本地实证**——`RoundRobinLoadBalancer`/`RandomLoadBalancer`/`ServiceInstanceListSupplier`/`LoadBalancerZoneConfig` **`[本地实证：spring-cloud-loadbalancer `core/` + `config/`]`**——**若我实现默认装配会与此一致**（"默认可用 + 条件化双实现 + 可覆盖"是标准装配哲学）
...
- **参考实现**（docs 机制简述 + 本地实证——2026-08-12 重写：源码块照录降为机制）：**机制四步（docs:251-359）**——①**@LoadBalanced 是 @Qualifier 派生注解**（docs:264——依赖注入的筛选条件）；②**收集**：`@LoadBalanced @Autowired List<RestTemplate>`（docs:293-295——**标记过的 RestTemplate 被注入收集**——@Qualifier 派生的依赖处理语义 docs:278-282）；③**定制回调**：SmartInitializingSingleton（docs:299-308——容器就绪后遍历 customizer）；④**注入拦截器**：RestTemplateCustomizer 把 LoadBalancerInterceptor 追加进 RestTemplate 拦截器列表（docs:317-322）——**"标记 → 收集 → 织入"三步**；**用法（docs:340-358）**——`@LoadBalanced @Bean RestTemplate` + `http://stores/stores` 虚拟主机名（**服务名寻址——拦截器解析服务名 → LoadBalancerClient 选实例 → 替换 host**）；**本地实证**——`LoadBalanced.java`/`LoadBalancerInterceptor.java` **`[本地实证：commons `client/loadbalancer/`]`**——**若我实现会与此一致**（@Qualifier 派生做"筛选标记"是干净方案——不污染业务注解）
