...
### 完整认知：REST API 客户端设计在真实架构中完整该讲什么

docs 覆盖了"RestTemplate 三层扩展 + 校验 + 异常 + Feign"。作为架构师，这个主题完整还该包含：

1. **客户端三件套的完整管理**：连接池管理(连接复用/池化/空闲清理)、超时(连接/读/写超时)、重试策略(与第 8 节容错衔接)——docs 只讲三层架构，这些是生产关键
2. **负载均衡与容错集成**：客户端调用走负载均衡(第 11/12 节) + 熔断限流(第 8 节)——客户端是容错/负载均衡的承载点
...
| 同步重试 vs 异步补偿 | 同步重试简单但放大流量(雪崩风险)；异步+熔断更稳(第 8 节) |

### 常见坑/反模式

1. **连接不复用/连接池满**：每次调用新建连接、或连接池耗尽——HTTP 连接要池化复用
2. **超时设置缺失**：没设连接/读超时，下游慢导致线程阻塞堆积(拖垮客户端)
3. **重试放大雪崩**：无脑重试(尤其配合熔断缺失)，下游故障时重试放大流量——重试要退避 + 熔断配合(第 8 节)
4. **序列化大对象/循环引用**：DTO 过大、循环引用导致序列化失败或性能差
5. **拦截器链顺序错误**：ClientHttpRequestInterceptor 顺序影响(如认证拦截必须在重试前/后)——洋葱式链(见 KP-04)
6. **忽略客户端指标/日志**：调用失败无日志/无指标，难排查——可观测性缺失(第 13 节)
7. **Feign 接口契约漂移**：客户端接口与服务端不一致(见第 3 节 API 契约)——契约要版本化

### 生态位置

- **与第 3 节(服务端)对称**：服务端定义契约，客户端消费契约——共同构成 REST API 治理
- **承载点**：客户端是负载均衡(第 11/12 节)、容错(第 8 节)、可观测(第 13 节)的落地位置
- **RestTemplate 三层架构 + 装饰器拦截链**是客户端扩展的核心模式
- 前置：第 3 节服务端、第 2 节工程模板；后置：第 8 节容错、第 11/12 节负载均衡

**架构师视角结论**：本篇不只是"怎么配 RestTemplate"，而是"**如何设计健壮的 API 客户端**"——连接/超时/重试/负载均衡/容错/可观测一体，是服务治理的消费侧。
