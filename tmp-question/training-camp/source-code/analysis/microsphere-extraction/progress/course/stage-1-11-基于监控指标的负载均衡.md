...
4. **负载均衡在调用链的位置**：客户端负载均衡(第 4 节 RestTemplate/Feign 走 LoadBalancer) vs 网关负载均衡(第 19 节)——两种位置
...
| 客户端 vs 网关负载均衡 | 客户端(分散，第 4 节)灵活；网关(集中，第 19 节)治理统一 |
...
- **衔接**：第 4 节(客户端负载均衡)、第 12 节(动态权重)、第 13 节(Micrometer 指标)、第 15 节(Pull 监控)、第 19 节(网关负载均衡)
