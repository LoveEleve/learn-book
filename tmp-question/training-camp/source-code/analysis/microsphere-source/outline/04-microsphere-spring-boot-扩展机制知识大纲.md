...
### 2.1 RunListener 生命周期适配 + 一次性监听幂等 [🔴 P1] [时间无关模式]
- **来源**：KP-308（SpringApplicationRunListenerAdapter）/ KP-302（OnceApplicationPreparedEventListener）
- **机制**：**RunListener 适配器模板**（8 回调空实现——子类只覆写需要的）+ Logging 生命周期日志变体（:157-159）+ FailureReport 失败报告；**按 contextId 幂等**（静态 Map 类级共享 :48/:96——多 context 防重）
- **my-xhs**：该用没用——启动钩子/埋点；官方 RunListener 是扩展点

### 2.2 条件评估报告 + 启动诊断 [🔴 P2] [时间无关模式]
...
- **来源**：KP-303（ListenableConfigurationPropertiesBindHandlerAdvisor）/ KP-306（EnableConfigurationPropertiesExtension）
- **机制**：**BindListener SPI 三回调**（onStart/onSuccess/onFailure——测试实证 :65/:69）+ **ConfigurationPropertiesBindHandlerAdvisor 官方扩展点**（wrap BindHandler :74）+ 属性变更事件（ConfigurationPropertiesBeanPropertyChangedEvent）+ @Enable 扩展外壳（开关属性）
...
### 4.1 生态能力 Actuator 化 + 监控线程池 [🔴 P2] [时间无关模式]
- **来源**：KP-312（ArtifactsEndpoint）/ KP-316（MonitoredThreadPoolTaskScheduler）
- **机制**：**内部工具 → Actuator 端点**（构件探测 /actuator/artifacts :18/:48——运维监控生产化路径）；**createExecutor 工厂钩子双轨设计**（:82-84——内部原对象 + 外部可监控委托）
- **my-xhs**：该用没用——classpath 审计端点/线程池监控

### 4.2 Boot 3 兼容层（同名同包重实现）[🔴 P2] [过时→Boot 3.2 官方]
- **来源**：KP-314（BootstrapContext）
