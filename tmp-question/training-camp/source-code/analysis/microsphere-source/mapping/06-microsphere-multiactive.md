...
- **my-xhs**：**已用（实证）**——`my-xhs-common/.../zone/loadbalancer/ZonePreferenceServiceInstanceListSupplier.java`（同名移植——与官方 SCL 同名风险在 my-xhs 同样存在）+ `ZoneLoadBalancerConfiguration.java`（LoadBalancer 定制装配——微球 CustomizedLoadBalancer* 对应物）；**差异**：微球默认关闭（customized=true）+ 客户端级条件（optimized-zone-preference）——my-xhs 装配方式待深读（**置信度 Medium——后续 dynamic 仓库提取时顺手核对 my-xhs loadbalancer 包**）
...
| KP-516 | 已用 | ZonePreferenceServiceInstanceListSupplier 同名移植 + ZoneLoadBalancerConfiguration（[待深读] Medium） |
