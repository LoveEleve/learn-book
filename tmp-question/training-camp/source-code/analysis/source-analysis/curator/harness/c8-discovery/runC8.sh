...
for jar in \
  ...
  $M2/org/apache/curator/curator-x-discovery/5.8.0/curator-x-discovery-5.8.0.jar \
...
echo "== 编译 =="
javac -cp "$CP" -d /tmp/c8harness HarnessC8.java || exit 1
echo "== 运行 =="
java -cp "/tmp/c8harness$CP" -Dorg.slf4j.simpleLogger.defaultLogLevel=warn HarnessC8
