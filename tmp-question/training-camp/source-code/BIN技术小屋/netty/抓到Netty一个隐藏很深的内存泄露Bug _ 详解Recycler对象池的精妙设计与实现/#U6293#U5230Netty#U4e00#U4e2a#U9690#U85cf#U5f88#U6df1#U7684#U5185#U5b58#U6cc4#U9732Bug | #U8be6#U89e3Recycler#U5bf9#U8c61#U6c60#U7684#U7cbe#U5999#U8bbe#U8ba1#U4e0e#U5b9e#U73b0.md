...
![](images/v2-ad3746423cfe11d936407e9883508243_1440w.jpg)

于是笔者将这个故事....哦不.....事故，详细的阐述出来分享给大家。

![](images/v2-6a16c9ae18908e2475e6a86ff13bb076_1440w.jpg)
...
> Issue11864: [github.com/netty/netty/](https://link.zhihu.com/?target=https%3A//github.com/netty/netty/issues/11864)
>
> PR: [github.com/netty/netty/](https://link.zhihu.com/?target=https%3A//github.com/netty/netty/pull/11865)

![](images/v2-54e897c394a0f9ac4587826e5cd680fd_1440w.jpg)
...
这位说话特别好听的 chrisvest 大佬也说了 我发现的这个 Bug 也间接证明了 Netty 要简化对象池设计的正确性和必要性。

![](images/v2-5d2c75995824e2b356f0ee3ced101f4f_1440w.jpg)

> 随口提一句，这个大牛 chrisvest 是大名鼎鼎的 [图数据库](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%9B%BE%E6%95%B0%E6%8D%AE%E5%BA%93&zhida_source=entity) Neo4j 的核心commitor，同时也是Netty Buffer相关 API 的设计者。
...
![](images/v2-cc529035530059776a61c80074eb958a_1440w.jpg)

## 1\. 池化思想的应用

在我们日常开发工作中我们经常会遇到各种池化技术的设计思想，比如 [连接池](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E8%BF%9E%E6%8E%A5%E6%B1%A0&zhida_source=entity) ，内存池，对象池，还有我们在业务开发过程中经常会缓存一些业务计算结果数据这也同样运用到了池化技术的设计思想，我们可以叫它为结果池。
...
还有我们在日常开发中遇到的一些 [计算逻辑](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E8%AE%A1%E7%AE%97%E9%80%BB%E8%BE%91&zhida_source=entity) 复杂的业务，我们通常会先从数据库中查询数据，然后经过复杂的计算得到结果，为了避免下次在重复计算，我们会将计算结果放入缓存中，我们可以称做结果池。也是一种池化思想。

再比如我们在**[?《Netty如何高效接收网络数据》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484244%26idx%3D1%26sn%3D831060fc38caa201d69f87305de7f86a%26chksm%3Dce77c513f9004c05b48f849ff99997d6d7252453135ae856a029137b88aa70b8e046013d596e%23rd)** 一文中提到的内存池，为了避免不必要的数据拷贝以及JVM垃圾回收对性能的影响，Netty 选择使用堆外内存存储网络通信数据。在 Netty 申请堆外内存之前，首先会在 JVM 堆中创建一个用于引用 native memory 的引用对象 DirectByteBuffer ，随后会使用 native 方法 `unsafe.allocateMemory` 通过底层 `malloc` [系统调用](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) 申请一块堆外内存。

这里就涉及到到两个重要开销：

- 在 JVM 堆中创建对象 DirectByteBuffer ，并为该对象申请分配 JVM [堆内存](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%A0%86%E5%86%85%E5%AD%98&zhida_source=entity) 。
- 通过 `malloc` 系统调用向 [操作系统](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F&zhida_source=entity) 申请 [堆外内存](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=4&q=%E5%A0%86%E5%A4%96%E5%86%85%E5%AD%98&zhida_source=entity) ，然后被 DirectByteBuffer 引用。但是堆外内存的申请和释放远比 [堆内内存](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%A0%86%E5%86%85%E5%86%85%E5%AD%98&zhida_source=entity) 申请和释放的开销要大很多。

而在 Netty 面对的 [高并发](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E9%AB%98%E5%B9%B6%E5%8F%91&zhida_source=entity) 网络通信场景下，申请堆外内存是一个非常频繁的操作，基于以上提到的两个重要性能开销，这种大量频繁的内存申请释放操作对程序的性能影响是巨大的，所以 Netty 就引入了内存池对内存相关的操作进行统一的管理。

## 2\. 对象池简介

以上内容的介绍就是池化思想的应用以及它所解决的问题，本文我们的主题是介绍对象池，对象池的引入是为了在需要大量创建对象以及销毁对象的场景下，将对象进行 [池化](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=7&q=%E6%B1%A0%E5%8C%96&zhida_source=entity) 以达到复用池中对象，避免大量地重复创建对象以及销毁对象的性能开销，
...
另外这里提前向大家透露一点的是我们下篇文章中即将要介绍的 Netty 发送数据流程涉及到的对象池的应用。我们都知道 Netty 是一个异步事件驱动的高性能网络框架，当在业务线程中处理完 [业务逻辑](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E4%B8%9A%E5%8A%A1%E9%80%BB%E8%BE%91&zhida_source=entity) 准备响应业务结果到客户端的时候，我们会向对应 channel 写入业务结果，此时业务线程会立即返回，这是一个异步的过程。
...
在 Java 程序中我们可以通过一个 new 关键字来创建对象，而当JVM遇到一条 new 的 [字节码](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%AD%97%E8%8A%82%E7%A0%81&zhida_source=entity) 指令后，会发生什么呢？

1. 首先 JVM 要去检查 new 指令后面的参数也就是创建对象所属的 Java 类是否能够在方法区的 [常量池](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%B8%B8%E9%87%8F%E6%B1%A0&zhida_source=entity) 中定位到类的符号引用，进而检查这个符号引用所代表的类是否已经加载，解析，初始化过。如果没有，就需要先执行类的加载过程。
2. 当通过类加载检查之后，就开始为对象分配内存，而对象所需内存大小其实在类加载完成后就已经确定了。JVM要做的事情就是将一块确定大小的内存区域从JVM堆中划分出来。

> 关于如何确定对象所需内存大小，对这方面细节感兴趣的同学可以回看下笔者的**[?《对象在JVM中的内存布局》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484304%26idx%3D1%26sn%3D54bf0d07e69c5621c145afaece8f50d6%26chksm%3Dce77c5d7f9004cc1249a03dfd0fb12b7d75171f1b87acea1fa44bbb11ca374b6f42a66fa274d%23rd)** 这篇文章。
...
2. 而为了避免这种划分内存时的同步锁定，JVM提供了另外一种方式就是每个线程先预先向JVM堆申请一块内存（本地线程分配缓存-TLAB），这样当线程创建对象的时候，先是从自己的TLAB中为对象分配内存，当自己的TLAB用完时，才会去JVM堆中 **同步分配** 。 我们可以通过 [虚拟机](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E6%9C%BA&zhida_source=entity) 参数 `-XX:+UseTLAB` 开启TLAB（默认）。 `-XX:-UseTLAB` 关闭TLAB。
...
2. 设置对象头。包括设置MarkWord中的对象运行时信息。以及通过类型指针引用关联到类的 [元数据](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%85%83%E6%95%B0%E6%8D%AE&zhida_source=entity) 信息。这些内容我们在**[?《对象在JVM中的内存布局》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484304%26idx%3D1%26sn%3D54bf0d07e69c5621c145afaece8f50d6%26chksm%3Dce77c5d7f9004cc1249a03dfd0fb12b7d75171f1b87acea1fa44bbb11ca374b6f42a66fa274d%23rd)** 一文中都有提到过，大家还记得吗？
3. 执行 [构造函数](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E6%9E%84%E9%80%A0%E5%87%BD%E6%95%B0&zhida_source=entity) 。这样一个真正可用的对象就被创建出来了。
...
然而在高并发的网络IO处理场景下，这些单个对象的创建和回收开销会被无限放大，于是Netty引入了一个轻量级的 [对象池](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=22&q=%E5%AF%B9%E8%B1%A1%E6%B1%A0&zhida_source=entity) Recycler 来负责将这些需要频繁创建的对象进行池化，统一分配，回收管理。
...
![](images/v2-f48ef7d89e32bb8e675fc5d9addd8276_1440w.jpg)
...
这里我们创建出来的对象池是专门用来管理PooledDirectByteBuf对象的（通过泛型指定对象池需要管理的具体对象）。 [泛型类](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E6%B3%9B%E5%9E%8B%E7%B1%BB&zhida_source=entity) `ObjectPool<T>` 是Netty为对象池设计的一个顶层抽象。对象池的行为功能均定义在这个泛型 [抽象类](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E6%8A%BD%E8%B1%A1%E7%B1%BB&zhida_source=entity) 中。我们可以通过 ObjectPool#newPool 方法创建指定的对象池。其参数 ObjectCreator 接口用来定义创建池化对象的行为。当对象池中需要创建新对象时，就会调用该接口方法 ObjectCreator#newObject 来创建对象。
...
前边我们介绍到每一个要被池化的对象都需要一个 [静态变量](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E9%9D%99%E6%80%81%E5%8F%98%E9%87%8F&zhida_source=entity) 来引用其对应的对象池。
...
我们按照这个思路先来看一下Recycler对象池的总体 [架构设计](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E6%9E%B6%E6%9E%84%E8%AE%BE%E8%AE%A1&zhida_source=entity) 图，从整体直观上来感受下它的设计，以及包含的一些重要模块。

![](images/v2-877f189afbe570f710fa4a8123458a60_1440w.jpg)
...
- 另一个重要的结构是WeakOrderQueue链表，head 指针指向WeakOrderQueue链表的头结点，cursor 指针指向链表的当前节点，prev 指针指向当前节点的前一个节点。WeakOrderQueue链表是用来存储其他线程帮助本线程回收的对象（我们称之为待回收对象）。其中WeakOrderQueue链表中的每一个节点对应一个其他线程，这个其他线程为本线程回收的 [对象存储](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%AF%B9%E8%B1%A1%E5%AD%98%E5%82%A8&zhida_source=entity) 在对应的WeakOrderQueue节点中。
...
![](images/v2-cb8b40b64fa0755dc88db81d8cdbdcf0_1440w.jpg)
...
![](images/v2-758cda38e051e77b17c50b2e76b425c6_1440w.jpg)
...
**Netty这里为了不引入多线程同步的开销，只会保证待回收对象的最终可见性** 。 因为如果要保证待回收对象的实时可见性，就要插入一些 [内存屏障指令](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%B1%8F%E9%9A%9C%E6%8C%87%E4%BB%A4&zhida_source=entity) ，执行这些内存屏障指令也是需要开销的。

事实上这里也并不需要保证实时可见性，创建线程暂时看不到WeakOrderQueue节点中的待回收对象也是没关系的，大不了就新创建一个对象。 **这里还是遵循 [无锁化](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=8&q=%E6%97%A0%E9%94%81%E5%8C%96&zhida_source=entity) 的设计思想** 。

> 维护线程之间操作的原子性，可见性都是需要开销的，我们在日常多线程 [程序设计](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E7%A8%8B%E5%BA%8F%E8%AE%BE%E8%AE%A1&zhida_source=entity) 中一定要根据业务场景来综合考虑，权衡取舍。尽量遵循我们这里多次强调的 **多线程无锁化设计思想** 。提高多线程的运行效率。避免引入不必要的同步开销。
...
在《4. 对象池Recycler的使用》小节我们介绍过池化对象的 [设计方法](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E8%AE%BE%E8%AE%A1%E6%96%B9%E6%B3%95&zhida_source=entity) ，其中我们提到池化对象中需要引用其在对象池中的Handler，这个Handler会在对象池创建对象的时候传入。池化对象类型中需要定义recycle方法，recycle方法清空池化对象的所有属性，并调用Handler的recycle方法将池化对象回收至对象池中。
...
![](images/v2-04c483bea2375d1f0ce004f5d2fae6af_1440w.jpg)
...
![](images/v2-532ebe2d65bd6925e78e1d9e754129f7_1440w.jpg)
...
![](images/v2-7cebb4b23e150032e4066f7c512511f9_1440w.jpg)
...
![](images/v2-def345752f136e1bea3ffca0be33ecbd_1440w.jpg)
...
为了方便大家理解这些 [容量控制](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=3&q=%E5%AE%B9%E9%87%8F%E6%8E%A7%E5%88%B6&zhida_source=entity) 的相关参数，笔者又在对象池架构设计图的基础上补充了容量控制相关的信息。大家可以对照上边介绍到的这些参数的含义形象体会下：

![](images/v2-25e076d5d47630de9975c8dbe0d140f4_1440w.jpg)

### 7.4 对象回收频率控制

对象池不能不考虑容量的限制而无脑的进行对象的回收，而是要对回收对象的频率进行限制。在我们日常架构设计和程序设计时，我们也一定要有托底的方案，比如限流，降级， [熔断](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E7%86%94%E6%96%AD&zhida_source=entity) 等托底方案。这样程序就不至于被突发的异常流量击垮。
...
对象回收频率 [控制参数](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E6%8E%A7%E5%88%B6%E5%8F%82%E6%95%B0&zhida_source=entity) 的初始化逻辑如下：
...
![](images/v2-2167c17bf4745e2d4ba2af667c3a1550_1440w.jpg)
...
- `AtomicInteger availableSharedCapacity` ：当前创建线程对应的所有回收线程可以帮助当前创建线程回收的对象总量。比如图中thread2, thread3, thread4 这三个回收线程总共可以帮助 thread1 回收对象的总量。availableSharedCapacity 在多个回收线程中是 **共享的** ，回收线程每回收一个对象它的值就会减1，当小于 LINK_CAPACITY(回收线程对应WeakOrderQueue节点的最小 [存储单元](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%AD%98%E5%82%A8%E5%8D%95%E5%85%83&zhida_source=entity) Link)时，回收线程将不能在为该stack回收对象了。该值的计算公式为前边介绍的 `max(maxCapacity / maxSharedCapacityFactor, LINK_CAPACITY)` 。

> 当创建线程从Stack结构中的WeakOrderQueue链表中转移待回收对象到数组栈中后，availableSharedCapacity 的值也会相应增加。说白了这个值就是用来指示回收线程还能继续回收多少对象。已达到控制回收线程回收对象的 [总体容量](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E6%80%BB%E4%BD%93%E5%AE%B9%E9%87%8F&zhida_source=entity) 。

- `int maxDelayedQueues` ： 一个线程对于对象池来说，它可以是创建线程，也可以是回收线程，当该创建线程作为 [回收线程](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=58&q=%E5%9B%9E%E6%94%B6%E7%BA%BF%E7%A8%8B&zhida_source=entity) 时，该值定义了最多可以为多少个创建线程回收对象。默认值为 CPU \* 2。比如图中 thread2 作为回收线程既可以帮 thread1 回收对象也可以帮助 thread3, thread4 回收对象。那么maxDelayedQueues 的值就是 3 。
...
![](images/v2-a2f4def4967433ca1621c3e9501de6de_1440w.jpg)
...
![](images/v2-5cd4d24a8e1117c4e06dce26bcbd2350_1440w.jpg)
...
![](images/v2-6103c2de96fe0d7fa52bb312f2a5745c_1440w.jpg)
...
![](images/v2-2e52eec697f8c97b3fe88aceb5850fde_1440w.jpg)
...
![](images/v2-70c7912189315721c7d57ffb44f8cd62_1440w.jpg)
...
**第二个问题：操作WeakOrderQueue链表的 [头结点](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=26&q=%E5%A4%B4%E7%BB%93%E7%82%B9&zhida_source=entity) 为什么是 [同步方法](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=3&q=%E5%90%8C%E6%AD%A5%E6%96%B9%E6%B3%95&zhida_source=entity) 呢？**
...
- `AtomicInteger availableSharedCapacity` ：这个字段前边已经介绍过多次了，它是 [多线程](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=43&q=%E5%A4%9A%E7%BA%BF%E7%A8%8B&zhida_source=entity) 共享的一个字段，可以被多个回收线程进行操作，表达的语义是所有回收线程总共可以帮助创建线程一共可以回收多少对象。对所有回收线程回收对象的总量进行限制。每创建一个Link节点，它的值就减少一个LINK_CAPACITY ，每释放一个Link节点，它的值就增加一个LINK_CAPACITY 。
- `Link link` ：Head结构封装的Link [链表](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=99&q=%E9%93%BE%E8%A1%A8&zhida_source=entity) 中的头结点。
...
如果头结点中存储的对象已经全部转移完毕，则更新head指针指向下一个Link节点，开始转移下一个Link节点。 **创建线程每次只会转移一个Link节点** 。如果Link链表是空的，没有转移成功一个对象，则 [transfer方法](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=3&q=transfer%E6%96%B9%E6%B3%95&zhida_source=entity) 返回false。

由于transfer方法体比较大，笔者将其按照上述逻辑步骤拆分开来为大家讲解：

![](images/v2-cfbd49484d1b6c327cbd79b1c5ee92d7_1440w.jpg)
...
![](images/v2-1016fbcec141d06a189d0bc347f572e3_1440w.jpg)
...
![](images/v2-60cef5707b9a230f83b5180630efe4a3_1440w.jpg)
...
> 重构commit： [github.com/netty/netty/](https://link.zhihu.com/?target=https%3A//github.com/netty/netty/commit/28b9834612638ffec4948c0c650d04f766f20690)
...
在 **4.1.71.Final** 版本发布之后，笔者想的是后面抽空看下重构后的对象池实现，哈哈，只要谓语动词出现—— ”想的是.....“ 类似这样的句式，估计就没有以后了，哈哈。笔者还是大意了，这个 Issue11864: **[?https://github.com/netty/netty/issues/11864](https://link.zhihu.com/?target=https%3A//github.com/netty/netty/issues/11864)** 在过了几个月之后在社区里又被讨论了起来。有人发现在 **4.1.71.Final** 对象池重构后的版本中笔者提到的这些问题还是存在的。

于是作者 chrisvest 又 提了一个 PR11996 最终在 **4.1.74.Final** 版本中修复了笔者提的这个 Issue11864。

> PR11996 ： [github.com/netty/netty/](https://link.zhihu.com/?target=https%3A//github.com/netty/netty/pull/11996)
>
...
![](images/v2-7a6cc671afd90ff18b026bd14f85e32c_1440w.jpg)
...
其实我们前边多少也提到过了，考虑到一种极端的情况就是当创建线程挂掉并且被GC回收之后，其实这个创建线程对应的Stack结构已经没有用了，存储在Stack结构中的池化对象永远不会再被使用到，此时回收线程完全就没有必要在为挂掉的创建线程回收对象了。而这个Stack结构如果没有任何 [引用链](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=9&q=%E5%BC%95%E7%94%A8%E9%93%BE&zhida_source=entity) 存在的话，随后也会被GC回收。那么这个Stack结构在WeakHashMap中对应的Entry也会被自动删除。如果这里不采用WeakHashMap，那么回收线程为该Stack回收的对象就会一直停留在回收线程中。
...
到这里关于Recycler对象池的整个设计与源码实现，笔者就为大家详细的剖析完毕了，在剖析的过程中，我们提炼出了很多多线程 [并发程序](https://zhida.zhihu.com/search?content_id=195490272&content_type=Article&match_order=1&q=%E5%B9%B6%E5%8F%91%E7%A8%8B%E5%BA%8F&zhida_source=entity) 的设计要点和注意事项。大家可以在日常开发工作中多多体会并实践。
...
编辑于 2022-03-18 14:50[Netty](https://www.zhihu.com/topic/19732975)[对象池](https://www.zhihu.com/topic/20097830)[Java](https://www.zhihu.com/topic/19561132)[程序员0基础入门大模型的学习路线！](https://zhuanlan.zhihu.com/p/31864213680)
