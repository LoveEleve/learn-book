从今天开始我们来聊聊 [Netty](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=Netty&zhida_source=entity) 的那些事儿，我们都知道Netty是一个高性能异步事件驱动的网络框架。

它的设计异常优雅简洁，扩展性高，稳定性强。拥有非常详细完整的用户文档。

同时内置了很多非常有用的模块基本上做到了开箱即用，用户只需要编写短短几行代码，就可以快速构建出一个具有 `高吞吐` ， `低延时` ， `更少的资源消耗` ， `高性能（非必要的内存拷贝最小化）` 等特征的高并发 [网络应用程序](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E7%BD%91%E7%BB%9C%E5%BA%94%E7%94%A8%E7%A8%8B%E5%BA%8F&zhida_source=entity) 。
...
![](images/v2-aa10bd85f52845f4de8ee3b095ffac3e_1440w.jpg)

- 当 `网络数据帧` 通过 [网络传输](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E7%BD%91%E7%BB%9C%E4%BC%A0%E8%BE%93&zhida_source=entity) 到达网卡时，网卡会将网络数据帧通过 `DMA的方式` 放到 `环形缓冲区RingBuffer` 中。
...
- 当 `DMA操作完成` 时，网卡会向CPU发起一个 `硬中断` ，告诉 `CPU` 有网络数据到达。CPU调用网卡驱动注册的 `硬中断响应程序` 。网卡硬中断响应程序会为网络数据帧创建内核 [数据结构](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) `sk_buffer` ，并将网络数据帧 `拷贝` 到 `sk_buffer` 中。然后发起 `软中断请求` ，通知 `内核` 有新的网络数据帧到达。
...
- 在 `ip_rcv函数` 中也就是上图中的 `网络层` ， `取出` 数据包的 `IP头` ，判断该数据包下一跳的走向，如果数据包是发送给本机的，则取出 [传输层](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E4%BC%A0%E8%BE%93%E5%B1%82&zhida_source=entity) 的协议类型（ `TCP` 或者 `UDP`)，并 `去掉` 数据包的 `IP头` ，将数据包交给上图中得 `传输层` 处理。
...
- 内核在接收网络数据包时所做的工作我们就介绍完了，现在我们把视角放到 [应用层](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E5%BA%94%E7%94%A8%E5%B1%82&zhida_source=entity) ，当我们程序通过 [系统调用](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) `read` 读取 `Socket接收缓冲区` 中的数据时，如果接收缓冲区中 `没有数据` ，那么应用程序就会在系统调用上 `阻塞` ，直到Socket接收缓冲区 `有数据` ，然后 `CPU` 将 `内核空间` （Socket接收缓冲区）的数据 `拷贝` 到 `用户空间` ，最后系统调用 `read返回` ，应用程序 `读取` 数据。
...
![](images/v2-9147efe7747fea705cfc3acdb1e1f9c4_1440w.jpg)
...
![](images/v2-9f1654bfc5715e275cc0ca261fc140b1_1440w.jpg)

> **为什么不直接使用 `Socket` [发送队列](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=7&q=%E5%8F%91%E9%80%81%E9%98%9F%E5%88%97&zhida_source=entity) 中的 `sk_buffer` 而是需要拷贝一份呢？** 因为 `TCP协议` 是支持 `丢包重传` 的，在没有收到对端的 `ACK` 之前，这个 `sk_buffer` 是不能删除的。内核每次调用网卡发送数据的时候，实际上传递的是 `sk_buffer` 的 `拷贝副本` ，当网卡把数据发送出去后， `sk_buffer` 拷贝副本会被释放。当收到对端的 `ACK` 之后， `Socket` 发送队列中的 `sk_buffer` 才会被真正删除。
...
>
> 从这里可以看到网络包的发送过程和接受过程是不同的，在介绍 [网络包](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=4&q=%E7%BD%91%E7%BB%9C%E5%8C%85&zhida_source=entity) 的接受过程时，我们提到是通过触发 `NET_RX_SOFTIRQ` 类型的软中断在内核线程 `ksoftirqd` 中执行 `内核网络协议栈` 接受数据。而在网络数据包的发送过程中是 `用户线程的内核态` 在执行 `内核网络协议栈` ，只有当线程的 `CPU quota` 用尽时，才触发 `NET_TX_SOFTIRQ` 软中断来发送数据。
>
...
- 现在发送流程终于到了网卡真实发送数据的阶段，前边我们讲到无论是用户线程的内核态还是触发 `NET_TX_SOFTIRQ` 类型的软中断在发送数据的时候最终会调用到网卡的驱动程序函数 `dev_hard_start_xmit` 来发送数据。在网卡驱动程序函数 `dev_hard_start_xmit` 中会将 `sk_buffer` 映射到网卡可访问的 `内存 DMA 区域` ，最终网卡驱动程序通过 `DMA` 的方式将 `数据帧` 通过 [物理网卡](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E7%BD%91%E5%8D%A1&zhida_source=entity) 发送出去。
...
前边我们提到了在网络包接收过程中涉及到的性能开销，现在介绍完了网络包的发送过程，我们来看下在数据包发送过程中的 [性能开销](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=6&q=%E6%80%A7%E8%83%BD%E5%BC%80%E9%94%80&zhida_source=entity) ：
...
![](images/v2-6b4f4b9969a88660f4a74a7ff88657df_1440w.jpg)
...
![](images/v2-bc3efc14ed2a97c01f46fcaf0216f710_1440w.jpg)
...
- 当 `Socket` 的接收缓冲区中有数据的时候， `阻塞` 和 `非阻塞` 的表现是一样的，都会进入第二阶段 `等待` 数据从 `内核空间` 拷贝到 `用户空间` ，然后 `系统调用返回` 。
  ![](images/v2-82a61df046de31add3c1a68a1c5a7406_1440w.jpg)
...
![](images/v2-597111a5a02c723f1e0bef57e30809ef_1440w.jpg)
...
![](images/v2-5b81acbcda49fdee380bcc6eea6ede73_1440w.jpg)

## IO模型

在进行网络IO操作时，用什么样的IO模型来读写数据将在很大程度上决定了网络框架的IO性能。所以IO模型的选择是构建一个高性能网络框架的基础。

在《UNIX 网络编程》一书中介绍了五种IO模型： `阻塞IO`,`非阻塞IO`,`IO多路复用`,`信号驱动IO`,`异步IO` ，每一种 [IO模型](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=7&q=IO%E6%A8%A1%E5%9E%8B&zhida_source=entity) 的出现都是对前一种的升级优化。
...
![](images/v2-ef420eb131bc1905146a3050d26b7f68_1440w.jpg)
...
- `Socket` 接收缓冲区中 `有数据` ，用户线程在 `内核态` 会将 `内核空间` 中的 [数据拷贝](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=9&q=%E6%95%B0%E6%8D%AE%E6%8B%B7%E8%B4%9D&zhida_source=entity) 到 `用户空间` ， **注意** 这个数据拷贝阶段，应用程序是 `阻塞的` ，当数据拷贝完成，系统调用返回。
...
![](images/v2-db7d267fe82e765550690c1cfaa10124_1440w.jpg)

基于以上 `非阻塞IO` 的特点，我们就不必像 `阻塞IO` 那样为每个请求分配一个线程去处理连接上的读写了。

我们可以利用 **一个线程或者很少的线程** ，去 `不断地轮询` 每个 `Socket` 的接收缓冲区是否有数据到达，如果没有数据， `不必阻塞` 线程，而是接着去 `轮询` 下一个 `Socket` 接收缓冲区，直到轮询到数据后，处理连接上的读写，或者交给 [业务线程池](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E4%B8%9A%E5%8A%A1%E7%BA%BF%E7%A8%8B%E6%B1%A0&zhida_source=entity) 去处理， [轮询线程](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E8%BD%AE%E8%AF%A2%E7%BA%BF%E7%A8%8B&zhida_source=entity) 则 `继续轮询` 其他的 `Socket` 接收缓冲区。
...
`select` 是 [操作系统内核](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=3&q=%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F%E5%86%85%E6%A0%B8&zhida_source=entity) 提供给我们使用的一个 `系统调用` ，它解决了在 `非阻塞IO模型` 中需要不断的发起 `系统IO调用` 去轮询 `各个连接上的Socket` [接收缓冲区](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=28&q=%E6%8E%A5%E6%94%B6%E7%BC%93%E5%86%B2%E5%8C%BA&zhida_source=entity) 所带来的 `用户空间` 与 `内核空间` 不断切换的 `系统开销` 。

`select` 系统调用将 `轮询` 的操作交给了 `内核` 来帮助我们完成，从而避免了在 `用户空间` 不断的发起轮询所带来的的系统性能开销。

![](images/v2-6339d5b8dc57fa7f1bdbab9c0733d90b_1440w.jpg)
...
![](images/v2-87037cda610c87f03dc348b6564b726d_1440w.jpg)
...
- 当内核将修改后的 `fd` 数组返回给用户线程后，用户线程解除 `阻塞` ，由用户线程开始遍历 `fd` 数组然后找出 `fd` 数组中值为 `1` 的 `Socket` [文件描述符](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=7&q=%E6%96%87%E4%BB%B6%E6%8F%8F%E8%BF%B0%E7%AC%A6&zhida_source=entity) 。最后对这些 `Socket` 发起系统调用读取数据。
...
- `maxfdp1 ：` select传递给内核监听的文件描述符集合中数值最大的文件描述符 `+1` ，目的是用于限定 [内核遍历](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=2&q=%E5%86%85%E6%A0%B8%E9%81%8D%E5%8E%86&zhida_source=entity) 范围。比如： `select` 监听的文件描述符集合为 `{0,1,2,3,4}` ，那么 `maxfdp1` 的值为 `5` 。
...
- `struct sock` 中的 `等待队列` 中存放的是系统IO调用发生阻塞的 `进程fd` ，以及相应的 `回调函数` 。 **记住这个地方，后边介绍 [epoll](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=7&q=epoll&zhida_source=entity) 的时候我们还会提到！**
...
![](images/v2-a995fafcdbf85b2386ce6e5d552a8148_1440w.jpg)
...
![](images/v2-d5cb1c23588163582ec549f20f3dfc9e_1440w.jpg)
...
![](images/v2-31ec01cf26bbe9758339f55ac876a33b_1440w.jpg)
...
1. 首先要在epoll内核中创建一个表示 `Socket连接` 的数据结构 `struct epitem` ，而在 `epoll` 中为了综合性能的考虑，采用一颗红黑树来管理这些海量 `socket连接` 。所以 `struct epitem` 是一个红黑树节点。
   ![](images/v2-1a317d1dbe82fa47c9850e8dc2e8f783_1440w.jpg)
...
![](images/v2-83c27c98dfe73be40091d8efb937d7f3_1440w.jpg)
...
> `container_of` 在Linux内核中是一个常用的宏，用于从包含在某个结构中的指针获得结构本身的指针，通俗地讲就是通过 [结构体变量](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E7%BB%93%E6%9E%84%E4%BD%93%E5%8F%98%E9%87%8F&zhida_source=entity) 中某个成员的首地址进而获得整个结构体变量的首地址。
>
...
2. 如果 `eventpoll->rdllist` 就绪队列中没有 `IO就绪` 的 `epitem` ，则会创建等待项 `wait_queue_t` ，将用户进程的 `fd` 关联到 `wait_queue_t->private` 上，并在等待项 `wait_queue_t->func` 上注册回调函数 `default_wake_function` 。最后将等待项添加到 `epoll` 中的等待队列中。用户进程让出CPU，进入 `阻塞状态` 。
   ![](images/v2-28404480280ae5fb32cb892bdb5df228_1440w.jpg)
...
1. **前边做了那么多的知识铺垫，下面终于到了 `epoll` 的整个工作流程了：**
   ![](images/v2-4c2b54dac880410ff6764f50c03d5981_1440w.jpg)
...
经过上边对 `epoll` 工作过程的详细解读，我们知道，当我们监听的 `socket` 上有数据到来时，软中断会执行 `epoll` 的回调函数 `ep_poll_callback`,在 [回调函数](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=25&q=%E5%9B%9E%E8%B0%83%E5%87%BD%E6%95%B0&zhida_source=entity) 中会将 `epoll` 中描述 `socket信息` 的数据结构 `epitem` 插入到 `epoll` 中的就绪队列 `rdllist` 中。随后用户进程从 `epoll` 的等待队列中被唤醒， `epoll_wait` 将 `IO就绪` 的 `socket` 返回给用户进程，随即 `epoll_wait` 会清空 `rdllist` 。
...
根据以上 `epoll` 的性能优势，它是目前为止各大主流网络框架，以及 [反向代理](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E5%8F%8D%E5%90%91%E4%BB%A3%E7%90%86&zhida_source=entity) 中间件使用到的网络IO模型。
...
![](images/v2-d5d614d9829782732a8b5c1b909e5310_1440w.jpg)
...
- 信号IO 在大量 IO 操作时可能会因为 [信号队列](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=1&q=%E4%BF%A1%E5%8F%B7%E9%98%9F%E5%88%97&zhida_source=entity) 溢出导致没法通知
...
![](images/v2-90df39a2a9d1c5743e787a9f612fc3fc_1440w.jpg)

`IO多路复用模型` 就像是我们在饭店门口排队等待叫号。叫号器就好比 `select,poll,epoll` 可以统一管理全部顾客的 `吃饭就绪` 事件，客户好比是 `socket` 连接，谁可以去吃饭了，叫号器就通知谁。

![](images/v2-c51da49cd8a9f02c762d2c2b0f5c2881_1440w.jpg)
...
![](images/v2-a7a99882e9a0ed2366785970c1109d7c_1440w.jpg)

`异步IO` 的系统调用需要操作系统内核来支持，目前只有 `Window` 中的 `IOCP` 实现了非常成熟的 `异步IO机制` 。

而 `Linux` 系统对 `异步IO机制` 实现的不够成熟，且与 `NIO` 的性能相比提升也不明显。

> 但 [Linux kernel](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=2&q=Linux+kernel&zhida_source=entity) 在5.1版本由Facebook的大神Jens Axboe引入了新的异步IO库 `io_uring` 改善了原来Linux native AIO的一些性能问题。性能相比 `Epoll` 以及之前原生的 `AIO` 提高了不少，值得关注。
...
![](images/v2-27c611f3c9857cbb612a22a73965150a_1440w.jpg)
...
>
> [用户线程](https://zhida.zhihu.com/search?content_id=189320171&content_type=Article&match_order=50&q=%E7%94%A8%E6%88%B7%E7%BA%BF%E7%A8%8B&zhida_source=entity) 注册 `write事件` 的时机是只有当用户发送的数据 `无法一次性` 全部写入 `buffer` 时，才会去注册 `write事件` ，等待 `buffer重新可写` 时，继续写入剩下的发送数据、如果用户线程可以一股脑的将发送数据全部写入 `buffer` ，那么也就无需注册 `write事件` 到 `从Reactor` 中。
...
![](images/v2-e2cca98c30ece048d8c2abc8367762c6_1440w.jpg)
...
发布于 2022-01-10 19:56[Netty](https://www.zhihu.com/topic/19732975)[Java](https://www.zhihu.com/topic/19561132)[网络编程](https://www.zhihu.com/topic/19611616)[一文告诉你人工智能纯小白学习路线！](https://zhuanlan.zhihu.com/p/31863323446)
