...
![](images/v2-9a4c5fa3c7d12ff1c3fb22e835c578ec_1440w.jpg)

大家第一眼看到这幅流程图，是不是脑瓜子嗡嗡的呢？

![](images/v2-f9f318804943349fccc4767f263daa82_1440w.jpg)

大家先不要惊慌，问题不大，本文笔者的目的就是要让大家清晰的理解这幅流程图，从而深刻的理解Netty Reactor的启动全流程，包括其中涉及到的各种代码设计实现细节。

![](images/v2-fa9b0bbe5420cf77513d692fd508bf73_1440w.jpg)

在上篇文章 **[《聊聊Netty那些事儿之Reactor在Netty中的实现(创建篇)》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%23rd)** 中我们详细介绍了Netty服务端核心引擎组件 `主从Reactor组模型 NioEventLoopGroup` 以及 `Reactor模型 NioEventLoop` 的创建过程。最终我们得到了netty Reactor模型的运行骨架如下：
...
![](images/v2-115b23422516d31bf9012cef313763e1_1440w.jpg)
...
![](images/v2-2e0e96d08db63b1c79abc318b05c1272_1440w.png)

`EventLoopGroup` Reactor线程组在不同IO模型下的实现：

![](images/v2-f86a80d20f65ce2c538fadaf173a339a_1440w.png)
...
相关的底层Socket选项， [netty](https://zhida.zhihu.com/search?content_id=190200225&content_type=Article&match_order=5&q=netty&zhida_source=entity) 全部枚举在ChannelOption类中，笔者这里就不一一列举了，在本系列后续相关的文章中，笔者还会为大家详细的介绍这些参数的作用。
...
![](images/v2-81a5ac2baa9ca92b094fc33dac153207_1440w.jpg)
...
当netty服务端启动成功之后，最终我们会得到如下结构的阵型，开始 [枕戈待旦](https://zhida.zhihu.com/search?content_id=190200225&content_type=Article&match_order=1&q=%E6%9E%95%E6%88%88%E5%BE%85%E6%97%A6&zhida_source=entity) ，准备接收客户端的连接，Reactor开始运转。
...
![](images/v2-b2626cd159185e9ff03792bb13f01dbf_1440w.jpg)
...
- 首先调用 `newSocket` 创建JDK NIO 原生 `ServerSocketChannel` ，这里调用了 `SelectorProvider#openServerSocketChannel` 来创建JDK NIO 原生 `ServerSocketChannel` ，我们在上篇文章 **[《聊聊Netty那些事儿之Reactor在Netty中的实现(创建篇)》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%23rd)** 中详细的介绍了 `SelectorProvider` 相关内容，当时是用 `SelectorProvider` 来创建 `Reactor` 中的 `Selector` 。大家还记得吗？？
...
`NioServerSocketChannel` 的整体构建过程介绍完了，现在我们来按照 [继承层次](https://zhida.zhihu.com/search?content_id=190200225&content_type=Article&match_order=2&q=%E7%BB%A7%E6%89%BF%E5%B1%82%E6%AC%A1&zhida_source=entity) 再回过头来看下 `NioServerSocketChannel` 的层次构建，来看下每一层都创建了什么，封装了什么，这些信息都是 `Channel` 的核心信息，所以有必要了解一下。

![](images/v2-810b671c700e63ae8f181a30b96e3aec_1440w.jpg)
...
- 设置JDK NIO原生 `ServerSocketChannel` 为 [非阻塞模式](https://zhida.zhihu.com/search?content_id=190200225&content_type=Article&match_order=1&q=%E9%9D%9E%E9%98%BB%E5%A1%9E%E6%A8%A1%E5%BC%8F&zhida_source=entity) ， 配合IO多路复用模型。
...
> `Unsafe` 为 `Channel接口` 的一个内部接口，用于定义实现对Channel底层的各种操作， `Unsafe接口` 定义的操作行为只能由Netty框架的 `Reactor线程` 调用， [用户线程](https://zhida.zhihu.com/search?content_id=190200225&content_type=Article&match_order=1&q=%E7%94%A8%E6%88%B7%E7%BA%BF%E7%A8%8B&zhida_source=entity) 禁止调用。
...
- 为 `NioServerSocketChannel` 分配独立的 `pipeline` 用于IO事件编排。 `pipeline` 其实是一个 `ChannelHandlerContext` 类型的 [双向链表](https://zhida.zhihu.com/search?content_id=190200225&content_type=Article&match_order=1&q=%E5%8F%8C%E5%90%91%E9%93%BE%E8%A1%A8&zhida_source=entity) 。头结点 `HeadContext`,尾结点 `TailContext` 。 `ChannelHandlerContext` 中包装着 `ChannelHandler` 。
...
![](images/v2-027a933c487838a299264f720175492f_1440w.jpg)

---

到了这里 `NioServerSocketChannel` 就创建完毕了，我们来回顾下它到底包含了哪些核心信息。

![](images/v2-d95533bab8af3803ad1f3d66b30465c3_1440w.jpg)
...
![](images/v2-28a693da6e51fe52ad94975ee153d6e2_1440w.jpg)
...
![](images/v2-3023dd4a52e558658c10450cbf46513d_1440w.jpg)

此时 `NioServerSocketChannel` 中的 `pipeline` 结构如下图所示：

![](images/v2-9c41ccac7b08704d96244a36dbd079ec_1440w.jpg)
...
![](images/v2-d38ab67b6bdc73c95ef3aa51836e928a_1440w.jpg)
...
Netty通过 `next()` 方法根据上篇文章 **[《聊聊Netty那些事儿之Reactor在Netty中的实现(创建篇)》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%23rd)** 提到的 `channel到reactor的绑定策略` ，从 `ReactorGroup` 中选取一个Reactor进行注册绑定。之后 `Channel` 生命周期内的所有 `IO 事件` 都由这个 `Reactor` 负责处理，如 `accept、connect、read、write` 等 IO 事件。
...
![](images/v2-285a115cd619ca9fe0f53b9bbae41b15_1440w.jpg)
...
![](images/v2-3305ee387ad1ba11afc6ca91824a749f_1440w.jpg)
...
- `Reactor线程` 的核心工作之前介绍过： `轮询所有注册其上的Channel中的IO就绪事件` ， `处理对应Channel上的IO事件` ， `执行异步任务` 。Netty将这些核心工作封装在 `io.netty.channel.nio.NioEventLoop#run` 方法中。
  ![](images/v2-ee15389f1fdfe1b1de5b8c123cfbab0d_1440w.jpg)
...
![](images/v2-5a7cebbebeae6ca420b077e106206f88_1440w.jpg)

至此 `NioServerSocketChannel` 的注册工作正式拉开帷幕~~

![](images/v2-f29250e3ea3dd871497fc7a2d93d4235_1440w.jpg)
...
![](images/v2-8f1bb99665b922d63b2ba575b837172f_1440w.jpg)

此时 `Main Reactor` 中的任务队列 `taskQueue` 结构变化为：

![](images/v2-b3d813eabcbf6cac4453fafd67a47cf8_1440w.jpg)
...
在本小节《Netty服务端的启动》的最开始，我们介绍了服务端启动的 [入口函数](https://zhida.zhihu.com/search?content_id=190200225&content_type=Article&match_order=2&q=%E5%85%A5%E5%8F%A3%E5%87%BD%E6%95%B0&zhida_source=entity) `io.netty.bootstrap.AbstractBootstrap#doBind` ，在函数的最开头调用了 `initAndRegister()` 方法用来创建并初始化 `NioServerSocketChannel` ，之后便会将 `NioServerSocketChannel` 注册到 `Main Reactor` 中。
...
![](images/v2-8006cbc8c5e2b353cbd7c33cedfd7d7d_1440w.jpg)
...
![](images/v2-4ea583facde06b7226e1fb2b4bbb00cc_1440w.jpg)
...
- `outbound事件` 则是 [反向传播](https://zhida.zhihu.com/search?content_id=190200225&content_type=Article&match_order=1&q=%E5%8F%8D%E5%90%91%E4%BC%A0%E6%92%AD&zhida_source=entity) ，从 `TailContext` 开始反向向前传播直到 `HeadContext` 。

> `inbound事件` 只能被 `pipeline` 中的 `ChannelInboundHandler` 响应处理 `outbound事件` 只能被 `pipeline` 中的 `ChannelOutboundHandler` 响应处理

![](images/v2-1010aebeab872a5c0d12520dcc85aad8_1440w.jpg)
...
发布于 2022-01-19 11:04[Netty](https://www.zhihu.com/topic/19732975)[Java](https://www.zhihu.com/topic/19561132)[网络编程](https://www.zhihu.com/topic/19611616)[企业新增长，知乎给出知+内容解决方案](https://zhuanlan.zhihu.com/p/257912974)
