...
在上篇文章 [《聊一聊 Netty 数据搬运工 ByteBuf 体系的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s/KOjTICm2qZ7wKVCgQXhHwg%3Ftoken%3D1985029116%26lang%3Dzh_CN) 中，笔者详细地为大家介绍了 ByteBuf 整个体系的设计，其中笔者觉得 Netty 对于引用计数的设计非常精彩，因此将这部分设计内容专门独立出来。

---

Netty 为 ByteBuf 引入了引用计数的机制，在 ByteBuf 的整个设计体系中，所有的 ByteBuf 都会继承一个 [抽象类](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=1&q=%E6%8A%BD%E8%B1%A1%E7%B1%BB&zhida_source=entity) AbstractReferenceCountedByteBuf ， 它是对接口 ReferenceCounted 的实现。
...
每个 ByteBuf 的内部都维护了一个叫做 refCnt 的 [引用计数](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=3&q=%E5%BC%95%E7%94%A8%E8%AE%A1%E6%95%B0&zhida_source=entity) ，我们可以通过 `refCnt()` 方法来获取 ByteBuf 当前的引用计数 refCnt。当 ByteBuf 在其他上下文中被引用的时候，我们需要通过 `retain()` 方法将 ByteBuf 的引用计数加 1。另外我们也可以通过 `retain(int increment)` 方法来指定 refCnt 增加的大小（increment）。

有对 ByteBuf 的引用那么就有对 ByteBuf 的释放，每当我们使用完 ByteBuf 的时候就需要手动调用 `release()` 方法将 ByteBuf 的引用计数减 1 。当引用计数 refCnt 变成 0 的时候，Netty 就会通过 `deallocate` 方法来释放 ByteBuf 所引用的内存资源。这时 `release()` 方法会返回 true, 如果 refCnt 还不为 0 ，那么就返回 false 。同样我们也可以通过 `release(int decrement)` 方法来指定 refCnt 减少多少（ [decrement](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=3&q=decrement&zhida_source=entity) ）。

## 1\. 为什么要引入引用计数

”在其他上下文中引用 ByteBuf “ 是什么意思呢 ？ 比如我们在线程 1 中创建了一个 ByteBuf，然后将这个 ByteBuf 丢给 [线程](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=2&q=%E7%BA%BF%E7%A8%8B&zhida_source=entity) 2 进行处理，线程 2 又可能丢给线程 3， 而每个线程都有自己的上下文处理逻辑，比如对 ByteBuf 的处理，释放等操作。这样就使得 ByteBuf 在事实上形成了在多个线程上下文中被共享的情况。
...
另外相比于 JDK [DirectByteBuffer](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=1&q=DirectByteBuffer&zhida_source=entity) 需要依赖 GC 机制来释放其背后引用的 Native Memory, Netty 更倾向于手动及时释放 DirectByteBuf 。因为 JDK DirectByteBuffer 的释放需要等到 GC 发生，由于 DirectByteBuffer 的对象实例所占的 JVM [堆内存](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=1&q=%E5%A0%86%E5%86%85%E5%AD%98&zhida_source=entity) 太小了，所以一时很难触发 GC, 这就导致被引用的 [Native Memory](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=2&q=Native+Memory&zhida_source=entity) 的释放有了一定的延迟，严重的情况会越积越多，导致 OOM 。而且也会导致进程中对 DirectByteBuffer 的申请操作有非常大的延迟。
...
在探测到 [内存泄露](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=4&q=%E5%86%85%E5%AD%98%E6%B3%84%E9%9C%B2&zhida_source=entity) 发生之后，后续 Netty 就会通过 `reportLeak()` 将内存泄露的相关信息以 `error` 的日志级别输出到日志中。
...
事实上 Netty 对引用计数的设计非常讲究，绝非如此简单，甚至有些复杂，其背后隐藏着大大的性能考究以及对复杂并发问题的全面考虑，在性能与 [线程安全](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=1&q=%E7%BA%BF%E7%A8%8B%E5%AE%89%E5%85%A8&zhida_source=entity) 问题之间的反复权衡。
...
4.1.16.Final 的设计简洁清晰，在我们看来完全没有任何问题，但 Netty 对性能的考究完全没有因此止步，由于在 x86 架构下 [XADD 指令](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=1&q=XADD+%E6%8C%87%E4%BB%A4&zhida_source=entity) 的性能要高于 CMPXCHG 指令， compareAndSet 方法底层是通过 CMPXCHG 指令实现的，而 getAndAdd 方法底层是 XADD 指令。
...
如果 Netty 想在同时享受 XADD 指令带来的性能提升之外，又要解决上述提到的并发安全问题，就要重新对引用计数进行设计。首先我们的要求是继续采用 XADD 指令来实现引用计数的更新，但这就会带来 [多线程](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=1&q=%E5%A4%9A%E7%BA%BF%E7%A8%8B&zhida_source=entity) 并发修改所引起的 ByteBuf 语义改变。
...
> 大部分 [性能优化](https://zhida.zhihu.com/search?content_id=247125975&content_type=Article&match_order=1&q=%E6%80%A7%E8%83%BD%E4%BC%98%E5%8C%96&zhida_source=entity) 的套路都是相同的，我们通常不能一上来就奢求一个大而全的针对全局的优化方案，这是不可能的，也是十分低效的。往往最有效的，可以立竿见影的优化方案都是针对局部热点进行专门优化。
...
发布于 2024-08-20 11:37・广东[Netty](https://www.zhihu.com/topic/19732975)[Java](https://www.zhihu.com/topic/19561132)[我的硕士论文狱记：从崩溃到重生的1300天](https://zhuanlan.zhihu.com/p/1954165718468784796)

[

我的硕士论文狱记：从崩溃到重生的1300天 作为一个曾经在海外被硕士论文按在地上摩擦的“学术幸存者”，看到这个问题，我DNA里的PTSD瞬间被激活了！今天就来聊聊那段“白天焦虑、晚上...

](https://zhuanlan.zhihu.com/p/1954165718468784796)
