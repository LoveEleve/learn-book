...
![](images/v2-bfafabf04d8e4db4b723641f6506a645_1440w.jpg)
...
> 详细内容可回看**[?《从内核角度看IO模型的演变》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483737%26idx%3D1%26sn%3D7ef3afbb54289c6e839eed724bb8a9d6%26chksm%3Dce77c71ef9004e08e3d164561e3a2708fc210c05408fa41f7fe338d8e85f39c1ad57519b614e%23rd)**

后续我们又围绕着Netty的主从Reactor网络IO线程模型，在**[?《Reactor模型在Netty中的实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%23rd)** 一文中详细阐述了Netty的主从Reactor模型的创建，以及介绍了Reactor模型的关键组件。搭建了Netty的核心骨架如下图所示：

![](images/v2-59ed6fce1b185a30620f32e2e425fa45_1440w.jpg)

在核心骨架搭建完毕之后，我们随后又在**[?《详细图解Reactor启动全流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484005%26idx%3D1%26sn%3D52f51269902a58f40d33208421109bc3%26chksm%3Dce77c422f9004d340e5b385ef6ba24dfba1f802076ace80ad6390e934173a10401e64e13eaeb%23rd)** 一文中阐述了Reactor启动的全流程，一个非常重要的核心组件 [NioServerSocketChannel](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=NioServerSocketChannel&zhida_source=entity) 开始在这里初次亮相，承担着一个网络框架最重要的任务--高效接收网络连接。我们介绍了NioServerSocketChannel的创建，初始化，向Main Reactor注册并监听OP_ACCEPT事件的整个流程。在此基础上，Netty得以整装待发， [枕戈待旦](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E6%9E%95%E6%88%88%E5%BE%85%E6%97%A6&zhida_source=entity) 开始迎接海量的客户端连接。

![](images/v2-85d0d00f2ea669f5a3fcb035c1c8cc1d_1440w.jpg)

随后紧接着我们在**[?《Netty如何高效接收网络连接》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484184%26idx%3D1%26sn%3D726877ce28cf6e5d2ac3225fae687f19%26chksm%3Dce77c55ff9004c493b592288819dc4d4664b5949ee97fed977b6558bc517dad0e1f73fab0f46%23rd)** 一文中详细介绍了Netty高效接收客户端网络连接的全流程，在这里Netty的核心重要组件NioServerSocketChannel开始正是登场，在NioServerSocketChannel中我们创建了客户端连接 [NioSocketChannel](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=NioSocketChannel&zhida_source=entity) ，并详细介绍了NioSocketChannel的初始化过程，随后通过在NioServerSocketChannel的pipeline中触发ChannelRead事件，并最终在ServerBootstrapAcceptor中将客户端连接NioSocketChannel注册到Sub Reactor中开始监听客户端连接上的OP_READ事件，准备接收客户端发送的网络数据也就是本文的主题内容。
...
![](images/v2-d8251cc5292751c41a8391a92effce0e_1440w.jpg)

客户端发起系统IO调用向服务端发送数据之后，当网络数据到达服务端的网卡并经过内核协议栈的处理，最终数据到达Socket的 [接收缓冲区](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E6%8E%A5%E6%94%B6%E7%BC%93%E5%86%B2%E5%8C%BA&zhida_source=entity) 之后，Sub Reactor轮询到NioSocketChannel上的 `OP_READ事件` 就绪，随后Sub Reactor线程就会从JDK Selector上的阻塞轮询API `selector.select(timeoutMillis)` 调用中返回。转而去处理NioSocketChannel上的 `OP_READ事件` 。
...
这里需要重点强调的是，当前的 [执行线程](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E6%89%A7%E8%A1%8C%E7%BA%BF%E7%A8%8B&zhida_source=entity) 现在已经变成了Sub Reactor，而Sub Reactor上注册的正是netty客户端NioSocketChannel负责处理连接上的读写事件。

所以这里入口函数的参数 `AbstractNioChannel ch` 则是IO就绪的客户端连接 `NioSocketChannel` 。

开头通过 `ch.unsafe()` 获取到的NioUnsafe操作类正是NioSocketChannel中对底层JDK NIO SocketChannel的Unsafe底层操作类。实现类型为 `NioByteUnsafe` 定义在下图 [继承结构](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E7%BB%A7%E6%89%BF%E7%BB%93%E6%9E%84&zhida_source=entity) 中的 `AbstractNioByteChannel` 父类中。

![](images/v2-41e68e0e42113295b84dfba2e297abec_1440w.jpg)
...
这时候就是考验我们大局观的时候了，在前边的文章介绍中我们提到Netty的IO模型为主从Reactor [线程组](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E7%BA%BF%E7%A8%8B%E7%BB%84&zhida_source=entity) 模型，在Sub Reactor Group中包含了多个Sub Reactor专门用于监听处理客户端连接上的IO事件。

为了能够高效有序的处理全量客户端连接上的读写事件，Netty将 [服务端](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=3&q=%E6%9C%8D%E5%8A%A1%E7%AB%AF&zhida_source=entity) 承载的全量客户端连接分摊到多个Sub Reactor中处理，同时也能保证 `Channel上IO处理的线程安全性` 。

其中一个Channel只能分配给一个固定的Reactor。一个Reactor负责处理多个Channel上的IO就绪事件，Reactor与Channel之间的对应关系如下图所示：

![](images/v2-5bffbd794b2baf5c564c3bd475299d7b_1440w.jpg)
...
- 当 `lastBytesRead > 0` ：表示在本次read loop中从NioSocketChannel中读取到了数据，会在NioSocketChannel的pipeline中触发ChannelRead事件。进而在pipeline中负责IO处理的ChannelHandelr中响应，处理网络请求。
  ![](images/v2-dd64bb57110f41de54460d69c98bd230_1440w.jpg)
...
![](images/v2-68027f77b885c31440d1c71881dda0e2_1440w.jpg)
...
> 有些小伙伴可能对Netty中的一些传播事件触发的时机，或者事件之间的区别理解的不是很清楚，概念容易混淆。在后面的文章中笔者也会从源码的角度出发给大家说清楚Netty中定义的所有异步事件，以及这些事件之间的区别和联系和触发时机， [传播机制](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E4%BC%A0%E6%92%AD%E6%9C%BA%E5%88%B6&zhida_source=entity) 。
...
经过前边对 [网络数据](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=13&q=%E7%BD%91%E7%BB%9C%E6%95%B0%E6%8D%AE&zhida_source=entity) 接收的核心逻辑介绍，笔者在把这张流程图放出来，大家可以结合这张图在来回想下主干核心逻辑。
...
![](images/v2-991276d4696a993c13092f88c229d1d6_1440w.jpg)
...
> 这里再次强调下当前执行线程为Sub Reactor [线程](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=9&q=%E7%BA%BF%E7%A8%8B&zhida_source=entity) ，处理连接数据读取逻辑是在NioSocketChannel中。

首先通过 `config()` 获取客户端NioSocketChannel的Channel配置类NioSocketChannelConfig。

通过 `pipeline()` 获取NioSocketChannel的pipeline。我们在**[?《详细图解Netty Reactor启动全流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484005%26idx%3D1%26sn%3D52f51269902a58f40d33208421109bc3%26chksm%3Dce77c422f9004d340e5b385ef6ba24dfba1f802076ace80ad6390e934173a10401e64e13eaeb%26token%3D1943348780%26lang%3Dzh_CN%23rd)** 一文中提到的Netty服务端模板所举的示例中，NioSocketChannelde pipeline中只有一个EchoChannelHandler。
...
在上篇文章**[?《抓到Netty一个Bug，顺带来透彻地聊一下Netty是如何高效接收网络连接》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484184%26idx%3D1%26sn%3D726877ce28cf6e5d2ac3225fae687f19%26chksm%3Dce77c55ff9004c493b592288819dc4d4664b5949ee97fed977b6558bc517dad0e1f73fab0f46%26scene%3D178%26cur_album_id%3D2217816582418956300%23rd)** 中，笔者为了阐述上篇文章中提到的Netty在接收网络连接时的Bug时，简单和大家介绍了下这个RecvByteBufAllocator。

在上篇文章提到的NioServerSocketChannelConfig中，这里的RecvByteBufAllocator类型为ServerChannelRecvByteBufAllocator。

![](images/v2-ec6c7d5afa185df8a02719ae5cf955d3_1440w.png)
...
![](images/v2-f23b8ae3e8c3d177a5b8ba45e3de768d_1440w.png)
...
如果我们把用于接收数据用的ByteBuffer看做一个桶的话，那么 [小数据](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E5%B0%8F%E6%95%B0%E6%8D%AE&zhida_source=entity) 用大桶装或者大数据用小桶装肯定是不合适的，所以我们需要根据接收数据的大小来动态调整桶的容量。而AdaptiveRecvByteBufAllocator的作用正是用来根据每次接收数据的容量大小来动态调整ByteBuffer的容量的。
...
而真正具体执行 [内存分配](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%88%86%E9%85%8D&zhida_source=entity) 动作的是这里的ByteBufAllocator类型为PooledByteBufAllocator。它会根据AdaptiveRecvByteBufAllocator动态调整出来的大小去真正的申请内存分配ByteBuffer。
...
- `maybeMoreDataSupplier.get()` ：true表示本次读取从NioSocketChannel中读取数据，ByteBuffer满载而归。说明可能NioSocketChannel中还有数据没读完。fasle表示ByteBuffer还没有装满，说明NioSocketChannel中已经没有数据可读了。- `respectMaybeMoreData = true` 表示要对可能还有更多数据进行处理的这种情况要 `respect` 认真对待,如果本次循环读取到的数据已经装满 `ByteBuffer` ，表示后面可能还有数据，那么就要进行读取。如果 `ByteBuffer` 还没装满表示已经没有数据可读了那么就退出循环。
  ![](images/v2-076979fdff4f4817d749e37ccaf3181d_1440w.png)
...
而ByteBuffer动态自适应 [扩缩容](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=2&q=%E6%89%A9%E7%BC%A9%E5%AE%B9&zhida_source=entity) 机制依赖于AdaptiveRecvByteBufAllocator类的实现。让我们先回到AdaptiveRecvByteBufAllocator类的创建起点开始说起~~

### 4.1 AdaptiveRecvByteBufAllocator的创建

在前文**[?《Netty是如何高效接收网络连接》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484184%26idx%3D1%26sn%3D726877ce28cf6e5d2ac3225fae687f19%26chksm%3Dce77c55ff9004c493b592288819dc4d4664b5949ee97fed977b6558bc517dad0e1f73fab0f46%26scene%3D178%26cur_album_id%3D2217816582418956300%23rd)** 中我们提到，当Main Reactor监听到OP_ACCPET事件活跃后，会在NioServerSocketChannel中accept完成三次握手的客户端连接。并创建NioSocketChannel，伴随着NioSocketChannel的创建其对应的配置类NioSocketChannelConfig类也会随之创建。
...
AdaptiveRecvByteBufAllocator 主要的作用就是为接收数据的 `ByteBuffer` 进行 [扩容缩容](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E6%89%A9%E5%AE%B9%E7%BC%A9%E5%AE%B9&zhida_source=entity) ，那么每次怎么扩容？扩容多少？怎么缩容？缩容多少呢？？
...
- 当索引容量小于 `512` 时， `SIZE_TABLE` 中定义的容量索引是从 `16开始` 按 `16` 递增。
  ![](images/v2-74f6055c8269ad8bdad60d779821326b_1440w.jpg)

- 当索引容量大于 `512` 时， `SIZE_TABLE` 中定义的容量索引是按前一个索引容量的2倍递增。
  ![](images/v2-1a74875367fee56e7433abb836e188f3_1440w.jpg)
...
- `DEFAULT_INITIAL` ：表示ByteBuffer的 [初始化容量](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E5%88%9D%E5%A7%8B%E5%8C%96%E5%AE%B9%E9%87%8F&zhida_source=entity) 。默认为 `2048` 。
...
我们可以通过 **[二分查找](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=3&q=%E4%BA%8C%E5%88%86%E6%9F%A5%E6%89%BE&zhida_source=entity)** 在SIZE_TABLE中找出 `第一个` 容量大于等于DEFAULT_MINIMUM的容量索引 `minIndex` 。
...
在我们使用Java进行日常开发过程中，在为对象分配内存空间的时候我们都会选择在JVM堆中为对象分配内存，这样做对我们Java开发者特别的友好，我们只管使用就好而不必过多关心这块申请的内存如何回收，因为JVM堆完全受Java [虚拟机](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E6%9C%BA&zhida_source=entity) 控制管理，Java虚拟机会帮助我们回收不再使用的内存。

但是JVM在进行垃圾回收时候的 `stop the world` 会对我们应用程序的性能造成一定的影响。

除此之外我们在**[?《聊聊Netty那些事儿之从内核角度看IO模型》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483737%26idx%3D1%26sn%3D7ef3afbb54289c6e839eed724bb8a9d6%26chksm%3Dce77c71ef9004e08e3d164561e3a2708fc210c05408fa41f7fe338d8e85f39c1ad57519b614e%23rd)** 一文中介绍IO模型的时候提到，当数据达到网卡时，网卡会通过DMA的方式将数据拷贝到内核空间中，这是 `第一次拷贝` 。当 [用户线程](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=2&q=%E7%94%A8%E6%88%B7%E7%BA%BF%E7%A8%8B&zhida_source=entity) 在用户空间发起系统IO调用时，CPU会将内核空间的数据再次拷贝到用户空间。这是 `第二次拷贝` 。

于此不同的是当我们在JVM中发起IO调用时，比如我们使用JVM [堆内存](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E5%A0%86%E5%86%85%E5%AD%98&zhida_source=entity) 读取 `Socket接收缓冲区` 中的数据时， **会多一次内存拷贝** ，CPU在 `第二次拷贝` 中将数据从 [内核空间](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=3&q=%E5%86%85%E6%A0%B8%E7%A9%BA%E9%97%B4&zhida_source=entity) 拷贝到用户空间时，此时的用户空间站在JVM角度是 `堆外内存` ，所以还需要将堆外内存中的数据拷贝到 `堆内内存` 中。这就是 `第三次内存拷贝` 。

同理当我们在JVM中发起IO调用向 `Socket发送缓冲区` 写入数据时，JVM会将IO数据先 `拷贝` 到 `堆外内存` ，然后才能发起系统IO调用。

**那为什么操作系统不直接使用JVM的 `堆内内存` 进行 `IO操作` 呢？**

因为JVM的内存布局和操作系统分配的内存是不一样的，操作系统不可能按照JVM规范来读写数据，所以就需要 `第三次拷贝` 中间做个转换将堆外内存中的数据拷贝到 [JVM堆](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=4&q=JVM%E5%A0%86&zhida_source=entity) 中。
...
2. 在进行IO操作的时候，会多产生一次由堆外内存到 [堆内内存](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=5&q=%E5%A0%86%E5%86%85%E5%86%85%E5%AD%98&zhida_source=entity) 的拷贝。
...
所以Netty在进行 I/O 操作时都是使用的堆外内存，可以避免数据从 JVM 堆内存到堆外内存的拷贝。但是由于堆外内存不受JVM的管理，所以就需要额外关注对内存的使用和释放，稍有不慎就会造成 [内存泄露](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E6%B3%84%E9%9C%B2&zhida_source=entity) ，于是Netty就引入了 **内存池** 对 `堆外内存` 进行统一管理。
...
> 由于本文的主线是介绍Sub Reactor处理 `OP_READ事件` 的完整过程，所以这里只介绍主线相关的内容，这里只是简单介绍下在接收数据的时候为什么会用 `PooledByteBufAllocator` 来为 `ByteBuffer` [分配内存](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=9&q=%E5%88%86%E9%85%8D%E5%86%85%E5%AD%98&zhida_source=entity) 。而内存池的 [架构设计](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=1&q=%E6%9E%B6%E6%9E%84%E8%AE%BE%E8%AE%A1&zhida_source=entity) 比较复杂，所以笔者后面会单独写一篇关于Netty内存管理的文章。
...
这样设计的就比较灵活，具体内存分配的工作交给具体的 `ByteBufAllocator`,可以使用内存池的分配方式 `PooledByteBufAllocator` ，也可以不使用 [内存池](https://zhida.zhihu.com/search?content_id=192825390&content_type=Article&match_order=15&q=%E5%86%85%E5%AD%98%E6%B1%A0&zhida_source=entity) 的分配方式 `UnpooledByteBufAllocator` 。具体的内存可以采用JVM堆内内存（HeapBuffer），也可以使用堆外内存（DirectBuffer）。
...
编辑于 2022-02-23 10:40[Netty](https://www.zhihu.com/topic/19732975)[Java](https://www.zhihu.com/topic/19561132)[NIO](https://www.zhihu.com/topic/19651878)[嵌入式要卷成下一个Java了吗？](https://zhuanlan.zhihu.com/p/671200301)
