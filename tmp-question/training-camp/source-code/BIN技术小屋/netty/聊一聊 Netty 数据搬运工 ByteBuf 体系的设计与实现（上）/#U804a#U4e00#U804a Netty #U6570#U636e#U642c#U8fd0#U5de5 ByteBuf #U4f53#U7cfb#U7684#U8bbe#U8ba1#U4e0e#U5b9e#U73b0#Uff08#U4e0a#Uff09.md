...
时光芿苒，岁月如梭，好久没有给大家更新 Netty 相关的文章了，在断更 Netty 的这段日子里，笔者一直在持续更新 [Linux 内存管理相关的文章](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/mp/appmsgalbum%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26action%3Dgetalbum%26album_id%3D2559805446807928833%26scene%3D173%26from_msgid%3D%26from_itemidx%3D%26count%3D3%26nolastread%3D1%26scene%3D21%23wechat_redirect) ，目前为止，算是将 Linux [内存管理子系统](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E7%AE%A1%E7%90%86%E5%AD%90%E7%B3%BB%E7%BB%9F&zhida_source=entity) 相关的主干源码较为完整的给大家呈现了出来，同时也结识了很多喜欢内核的读者，经常在后台留言讨论一些代码的设计细节，在这个过程中，我们相互分享，相互学习，浓浓的感受到了大家对技术那份纯粹的热爱，对于我自己来说，也是一种激励，学习，提高的机会。

之前系列文章的视角一直是停留在内核态，笔者试图从 Linux 内核的角度来为大家揭秘内存管理的本质，那么从今天开始，我们把视角在往上挪一挪，从内核态转换到用户态，继续沿着内存管理这条主线，来看一看用户态的内存管理是如何进行的。

接下来笔者计划用三篇文章的篇幅为大家剖析一下 Netty 的内存管理模块，本文是第一篇，主要是围绕 Netty 内存管理的外围介绍一下 ByteBuf 的 [总体设计](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E6%80%BB%E4%BD%93%E8%AE%BE%E8%AE%A1&zhida_source=entity) 。
...
- 从 JVM 内存区域布局的角度来看，Netty 的 ByteBuf 主要分为 [HeapByteBuf](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=HeapByteBuf&zhida_source=entity) （堆内） 和 [DirectByteBuf](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=DirectByteBuf&zhida_source=entity) （堆外）这两种类型。
- 从内存管理的角度来看，Netty 的 ByteBuf 又分为 [PooledByteBuf](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=PooledByteBuf&zhida_source=entity) （池化）和 UnpooledByteBuf（非池化）两种子类型。一种是被内存池统一管理，另一种则和普通的 ByteBuf 一样，用的时候临时创建，不用的时候释放。
- 从内存访问的角度来看，Netty 又将 ByteBuf 分为了 [UnsafeByteBuf](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=UnsafeByteBuf&zhida_source=entity) 和普通的 ByteBuf。UnsafeByteBuf 主要是依赖 Unsafe 类提供的底层 API 来直接对内存地址进行操作。而普通 ByteBuf 对内存的操作主要是依赖 NIO 中的 ByteBuffer。
...
- 从内存占用统计的角度来说，Netty 又近一步将 ByteBuf 分为了 [InstrumentedByteBuf](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=InstrumentedByteBuf&zhida_source=entity) 和普通的 ByteBuf，其中 InstrumentedByteBuf 会带有内存占用相关 Metrics 的统计供我们进行监控，而普通的 ByteBuf 则不带有热任何 Metrics。
- 从零拷贝的角度来看，Netty 又引入了 [CompositeByteBuf](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=CompositeByteBuf&zhida_source=entity) ，目的是为多个 ByteBuf 在聚合的时候提供一个统一的逻辑视图，将多个 ByteBuf 聚合成一个逻辑上的 CompositeByteBuf，而传统的聚合操作则是首先要分配一个大的 ByteBuf，然后将需要聚合的多个 ByteBuf 中的内容在拷贝到新的 ByteBuf 中。CompositeByteBuf 避免了分配大段内存以及内存拷贝的开销。注意这里的 [零拷贝](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=2&q=%E9%9B%B6%E6%8B%B7%E8%B4%9D&zhida_source=entity) 指的是 Netty 在用户态层面自己实现的避免内存拷贝的设计，而不是 OS 层面上的零拷贝。
- 另外 Netty 的 ByteBuf 支持引用计数以及自动地内存泄露探测，如果有 [内存泄露](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=2&q=%E5%86%85%E5%AD%98%E6%B3%84%E9%9C%B2&zhida_source=entity) 的情况，Netty 会将具体发生泄露的位置报告出来。
...
![](images/v2-18da03bbdbc3d485e334822325c40f7f_1440w.jpg)
...
由于 JDK ByteBuffer 只设计了一个 position 指针，所以我们在读写 ByteBuffer 的时候需要不断的调整 position 的位置。比如，利用 flip() ，rewind()，compact()，clear() 等方法不断的进行 [读写模式](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E8%AF%BB%E5%86%99%E6%A8%A1%E5%BC%8F&zhida_source=entity) 的切换。
...
![](images/v2-9f1226939efee594dd6efe1a4eb84ac0_1440w.jpg)
...
![](images/v2-9d0c25e14b81a3485e9a88f0f4b95109_1440w.jpg)
...
![](images/v2-36879398b51042366ec3178e697b6680_1440w.jpg)
...
从上面列举的这些读写 ByteBuffer 场景可以看出，当我们在操作 ByteBuffer 的时候，需要时刻保持头脑清醒，对 ByteBuffer 中哪些部分是可读的，哪些部分是可写的要有一个清醒的认识，稍不留神就会出错。在复杂的 [编解码](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E7%BC%96%E8%A7%A3%E7%A0%81&zhida_source=entity) 逻辑中，如果使用 ByteBuffer 的话，就需要不断的进行读写模式的切换，切的切的人就傻了。
...
另外 JDK 的 ByteBuffer 不支持动态按需自适应扩容，当一个 ByteBuffer 被创建出来之后，它的容量就固定了。但实际上，我们很难在一开始就能准确的评估出到底需要多大的 ByteBuffer。分配的容量大了，会造成浪费。分配的容量小了，我们又需要每次在写入的时候判断剩余容量是否足够，如果不足，又需要手动去申请一个更大的 ByteBuffer，然后在将原有 ByteBuffer 中的 [数据迁移](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E8%BF%81%E7%A7%BB&zhida_source=entity) 到新的 ByteBuffer 中，想想都麻烦。

还有就是当多个 JDK 的 ByteBuffer 在面对合并聚合的场景，总是要先创建一个更大的 ByteBuffer，然后将原有的多个 ByteBuffer 中的内容在拷贝到新的 ByteBuffer 中。这就涉及到了 [内存分配](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%88%86%E9%85%8D&zhida_source=entity) 和拷贝的开销。
...
在之前介绍 JDK ByteBuffer 整体设计的时候，笔者是以 HeapByteBuffer 为例将 ByteBuffer 的整个设计体系串联起来的，那么本文笔者将会用 DirectByteBuf 为大家串联 Netty ByteBuf 的 [设计体系](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=4&q=%E8%AE%BE%E8%AE%A1%E4%BD%93%E7%B3%BB&zhida_source=entity) 。
...
![](images/v2-239ff4b53079478fa799ee4914d4063b_1440w.jpg)
...
其底层依赖的是一个 [抽象方法](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E6%8A%BD%E8%B1%A1%E6%96%B9%E6%B3%95&zhida_source=entity) `_getByte` ，由 AbstractByteBuf 具体的子类负责实现。比如，在 UnpooledDirectByteBuf 类的实现中，直接将 \_getByte 操作代理给其底层依赖的 JDK DirectByteBuffer。
...
Netty 另外还提供了批量读取 Bytes 的操作，比如我们可以通过 `getBytes` 方法将 ByteBuf 中的数据读取到一个 [字节数组](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E5%AD%97%E8%8A%82%E6%95%B0%E7%BB%84&zhida_source=entity) `byte[]` 中，也可以读取到另一个 ByteBuf 中。
...
同理，真正负责读取数据的方法 \_getInt 方法需要由 AbstractByteBuf 具体的子类实现，但这里和 \_getByte 不同的是，\_getInt 需要考虑字节序的问题， **由于网络协议采用的是 [大端字节序](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E5%A4%A7%E7%AB%AF%E5%AD%97%E8%8A%82%E5%BA%8F&zhida_source=entity) 传输，所以 Netty 的 ByteBuf 默认也是大端字节序** 。
...
在 UnpooledDirectByteBuf 的实现中，首先通过其依赖的 JDK DirectByteBuffer 以 [大端序](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E5%A4%A7%E7%AB%AF%E5%BA%8F&zhida_source=entity) 读取一个 Int 数据，然后通过 `ByteBufUtil.swapInt` 切换成 [小端序](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E5%B0%8F%E7%AB%AF%E5%BA%8F&zhida_source=entity) 返回。
...
![](images/v2-e66aeeff00cf279dae8902c28748b5f6_1440w.jpg)
...
![](images/v2-600dd5d120f5e2fdb401930f0c0ce740_1440w.jpg)

image.png

另外一种情况是 `readerIndex = writerIndex` 的情况，这种情况下表示 ByteBuf 中已经没有可读字节了。

![](images/v2-f9da1202014c5094ea070fe2b5e4c346_1440w.jpg)

image.png

既然 ByteBuf 中已经没有任何可读字节了，自然也就不需要将可读字节拷贝到 ByteBuf 的开头了，直接将 readerIndex 和 writerIndex 重新调整为 0 即可。

![](images/v2-46054f1715e38b45dbc328d494a3be53_1440w.jpg)
...
Netty 设计的这个丢弃字节的方法在解码的场景非常有用，由于 TCP 是一个面向流的网络协议，它只会根据滑动窗口的大小进行字节流的发送，所以我们在 [应用层](https://zhida.zhihu.com/search?content_id=246881835&content_type=Article&match_order=1&q=%E5%BA%94%E7%94%A8%E5%B1%82&zhida_source=entity) 接收到的数据可能是一个半包也可能是一个粘包，反正不会是一个完整的数据包。
...
![](images/v2-4d0c7d133d619c53655e816e11ba75c1_1440w.jpg)
...
- 如果 minNewCapacity 在 `[129 , 256]` 这段范围内 ， 那么扩容后的 newCapacity 就是 256 。
  ![](images/v2-78cf4ffd4e477b63aa30c9e60c5012ac_1440w.jpg)
...
![](images/v2-5fea6cdf85632c64d52b897d04528309_1440w.jpg)

image.png

当索引容量 `大于 512` 时，SIZE_TABLE 中定义的容量是按前一个索引容量的 `2 倍` 递增。

![](images/v2-2d8c994cd5ba44c2b1889403fb6e573a_1440w.jpg)
...
![](images/v2-43da690241c4e80628453caa0e20bcbd_1440w.jpg)
...
![](images/v2-24bd0d90d531f14409603577a19fe63e_1440w.jpg)
...
![](images/v2-d7ed9ca1bf3f21c97721273e60da879b_1440w.jpg)
...
![](images/v2-9a6613f1bb88dd5420349bcabfee0cc9_1440w.jpg)
...
![](images/v2-b9aadeff345121a4f597873e7a1432b0_1440w.jpg)
...
![](images/v2-025ad8896f378afe37188dc566956be0_1440w.jpg)
...
![](images/v2-5f27dbbf9f905c3cac9ad27bb5a353ba_1440w.jpg)
...
![](images/v2-62d132521af0819bd9dc101e50ad0183_1440w.jpg)
...
编辑于 2024-08-14 09:28・广东[Netty](https://www.zhihu.com/topic/19732975)[Java](https://www.zhihu.com/topic/19561132)[网络编程](https://www.zhihu.com/topic/19611616)[汉码未来|Java培训出来的如何找工作？](https://zhuanlan.zhihu.com/p/672068265)
