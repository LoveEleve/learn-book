> 本文基于 Netty 4.1.112.Final, [Kafka](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=Kafka&zhida_source=entity) 3.9.0 版本进行讨论

在业务开发的场景中，我们经常会遇到很多定时任务的需求。比如，生成业务报表，周期性对账，同步数据，订单支付超时处理等。针对业务场景中定时任务逻辑复杂，执行时间长的特点，市面上已经有很多成熟的任务调度中间件可供我们选择。比如： [ElasticJob](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=ElasticJob&zhida_source=entity), XXL-JOB, [PowerJob](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=PowerJob&zhida_source=entity) 等等。

而在 [中间件](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=2&q=%E4%B8%AD%E9%97%B4%E4%BB%B6&zhida_source=entity) 的场景中，同样也存在很多定时任务的需求。比如，网络连接的心跳检测，网络请求超时或失败的重试机制，网络连接断开之后的重连机制。和业务场景不同的是，这些中间件场景的定时任务特点是逻辑简单，执行时间非常短，而且对时间精度的要求比较低。比如，心跳检测以及失败重试这些定时任务，其实晚执行个几十毫秒或者 100 毫秒也无所谓。
...
各大中间件设计了时间轮来调度具有上述 4 种特征的定时任务，而本文主要讨论的就是 [时间轮](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=2&q=%E6%97%B6%E9%97%B4%E8%BD%AE&zhida_source=entity) 的设计与实现。但在这之前我们需要搞清楚时间轮这个设计需求是怎么产生的，我们先从 [JDK](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=JDK&zhida_source=entity) 中的任务调度组件开始聊起，看看 JDK 中的这些任务调度组件为什么不能满足中间件的场景。

![](images/v2-51275d4a8bf7ba7abb83a3acdce794e7_1440w.jpg)

本文概要.png

## 1\. JDK 中的任务调度组件

说到定时任务，我们第一时间就能想到的调度组件就是 JDK 中的 [Timer](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=Timer&zhida_source=entity) ，为什么这么说呢，因为笔者刚参加工作时的第一个任务就是用 Timer 实现的，当时对 Java 一无所知，完全零基础。主管交给我一个定时任务的需求，两眼抹黑。于是带着清澈而又稚嫩的眼神到网上一顿搜索，找到了这个 Timer，如获至宝。
...
TimerThread 会不断的从 TaskQueue 中获取堆顶任务，如果堆顶任务的 ExecutionTime 已经达到 —— `executionTime <= currentTime`, 则执行任务。如果该任务是一个 [周期性任务](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E5%91%A8%E6%9C%9F%E6%80%A7%E4%BB%BB%E5%8A%A1&zhida_source=entity) ，则将任务重新放入到 TaskQueue 中。
...
1. 首先用于组织延时任务的 TaskQueue 本质上是一个小根堆。对于堆这种 [数据结构](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) 来说，添加，删除一个延时任务时，堆都要向上，向下调整以便满足小根堆的特性。单次操作的 [时间复杂度](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E6%97%B6%E9%97%B4%E5%A4%8D%E6%9D%82%E5%BA%A6&zhida_source=entity) 为 `O(logn)` 。显然在面对海量定时任务的添加，删除时，性能上还是差点意思。
2. Timer 调度框架中只有一个 TimerThread [线程](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=2&q=%E7%BA%BF%E7%A8%8B&zhida_source=entity) 来负责延时任务的调度，执行。在面对海量任务的时候，通常会显得力不从心。
3. 另外一个严重问题是，当延时任务在执行的过程中出现异常时， Timer 并不会捕获，会导致 TimerThread 终止。这样一来，TaskQueue 中的其他延时任务将永远不会得到执行。
4. Timer 依赖于系统的 [绝对时间](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E7%BB%9D%E5%AF%B9%E6%97%B6%E9%97%B4&zhida_source=entity) ，如果系统时间本身不准确，那么延时任务的调度就可能会出问题。

### 1.2 DelayQueue

DelayQueue 是 JDK 提供的一个 [延时队列](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E5%BB%B6%E6%97%B6%E9%98%9F%E5%88%97&zhida_source=entity) ，我们可以利用它来延时获取队列中的元素，它的实现其实和 Timer 中的 TaskQueue 很类似，其底层都是一个优先级队列。
...
ScheduledThreadPoolExecutor 是 [多线程](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E5%A4%9A%E7%BA%BF%E7%A8%8B&zhida_source=entity) 版本的 Timer ，它是在 DelayQueue 的基础上增加了多线程调度延时任务的能力。ScheduledThreadPoolExecutor 中负责组织管理延时任务的是 DelayedWorkQueue，它也是一个小根堆实现的优先级队列，延时任务 ScheduledFutureTask 按照到期时间由近及远的组织在 DelayedWorkQueue 中。DelayedWorkQueue 的第一个元素是到期时间最近的 ScheduledFutureTask。

[业务线程](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E4%B8%9A%E5%8A%A1%E7%BA%BF%E7%A8%8B&zhida_source=entity) 可以通过 schedule, scheduleAtFixedRate, scheduleWithFixedDelay 方法将延时任务 ScheduledFutureTask 添加到 DelayedWorkQueue 中。

![](images/v2-031c01e397cd199f1520b019058c74ed_1440w.jpg)

image.png

ScheduledThreadPoolExecutor 负责调度延时任务的是一个 [线程池](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E7%BA%BF%E7%A8%8B%E6%B1%A0&zhida_source=entity) ，里边包含了多个 worker 调度线程，每个 worker 线程负责从 DelayedWorkQueue 中获取已经到期的 ScheduledFutureTask，然后执行。如果 DelayedWorkQueue 中没有任务到期，那么 worker 线程就会在 DelayedWorkQueue 上阻塞等待，直到有到期的任务出现。
...
时间轮的设计灵感来自于我们日常生活中用的钟表，钟表有秒针，分针，时针，共三个指针，60 个刻度。秒针每走一个刻度就是一秒，秒针走完一个 [时钟周期](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E6%97%B6%E9%92%9F%E5%91%A8%E6%9C%9F&zhida_source=entity) （60s），分针走一个刻度就是一分钟，当分针走完一个时钟周期（60m），时针走一个刻度就是一个小时。

![](images/v2-ba92ef0add7ef4529e4cb0e06cf9fcf6_1440w.jpg)

image.png

比如我们要在今天的 10 点 10 分 0 秒这个时刻去开一个重要的会议，那么当钟表的秒针指向 0 这个刻度，分针指向 10 这个刻度，时针指向 10 这个刻度的时候，闹钟就会响起，提醒我们去执行开会这个延时任务。

如果我们能把钟表里的刻度抽象成一个数据结构，用这个数据结构来存放对应刻度的延时任务的话，那么当钟表的时针，分针，秒针指向某个刻度的时候，我们就去执行这个刻度对应的延时任务，这样一来，一种新的延时任务调度思路就出来了，这也是时间轮的 [设计理念](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E8%AE%BE%E8%AE%A1%E7%90%86%E5%BF%B5&zhida_source=entity) 。

![](images/v2-435ac929ccb79e36d96df1eb106e43e3_1440w.jpg)

image.png

如上图所示，Netty 将钟表的刻度抽象成了一个 HashedWheelBucket 的数据结构，钟表的表盘被抽象成一个 HashedWheelBucket 类型的 [环形数组](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E7%8E%AF%E5%BD%A2%E6%95%B0%E7%BB%84&zhida_source=entity) ，钟表中有 60 个刻度，而 Netty 的时间轮 [HashedWheelTimer](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=HashedWheelTimer&zhida_source=entity) 一共有 512 个刻度。
...
钟表中一共有三个指针，分别是秒针，分针，时针。而 HashedWheelTimer 中只有一个 tick 指针，tick 每隔 tickDuration (100ms) 走一个刻度，也就是说 Netty 时间轮的时钟精度就是 100 ms, 定时任务的 [调度延时](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=4&q=%E8%B0%83%E5%BA%A6%E5%BB%B6%E6%97%B6&zhida_source=entity) 有时会在 100ms 左右。如果你接受不了这么大的调度误差，那么可以将 tickDuration 适当调小一些，但最小不能低于 1ms 。
...
HashedWheelBucket 是一个具有头尾指针的 [双向链表](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E5%8F%8C%E5%90%91%E9%93%BE%E8%A1%A8&zhida_source=entity) ，链表中存储的元素类型为 HashedWheelTimeout 用于封装定时任务。HashedWheelBucket 中的 head 指向双向链表中的第一个 HashedWheelTimeout ， tail 指向双向链表中的最后一个 HashedWheelTimeout。
...
![](images/v2-7936f450fd5c1ea5a1274f5c5cffbdf4_1440w.jpg)
...
![](images/v2-6a47a32886801ea3853e403091564940_1440w.jpg)
...
![](images/v2-d0a21587e26cb3fecd81be8c798b07c3_1440w.jpg)
...
![](images/v2-25c560bdafe0091b4e1c3b7c08828b2b_1440w.jpg)
...
![](images/v2-04da93a2cf65c11d707358879db82bcc_1440w.jpg)
...
从上面的警告信息我们可以看出，时间轮是一种共享的资源，既然是一种系统资源，那么就和内存资源一样（ [ByteBuf](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=ByteBuf&zhida_source=entity) ）都存在资源泄露的风险。当我们使用完时间轮但忘记调用它的 `stop` 方法进行关闭的时候，就发生了资源泄露。
...
> 关于 ResourceLeakTracker 的实现原理，感兴趣的读者可以回看下笔者之前的文章 [《Netty 如何自动探测内存泄露的发生》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s/ZU87D8HnZKp4OJPBg7AecA)
...
![](images/v2-0d95bbbcf4340168da27e7e21c8dbc1d_1440w.jpg)

image.png

如上图所示，当我们向时间轮添加一个延时任务时，并不是大家想象的那样，直接将延时任务添加到时间轮中，而是首先添加到一个叫做 timeouts 的 [MpscQueue](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=MpscQueue&zhida_source=entity) 中。
...
![](images/v2-3aa8dbf2a068e4e8420e160b59f6727e_1440w.jpg)
...
![](images/v2-2f44cf93cab49790a1e21e945cc00d01_1440w.jpg)
...
当时间轮要关闭的时候，我们就需要将 workerState 更新为 WORKER_STATE_SHUTDOWN。当 workerThread 检测到时间轮的状态不是 WORKER_STATE_STARTED 的时候，就会退出 `do ... while` 循环，停止时间轮的转动。
...
而 Kafka 中的 [多层时间轮](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E5%A4%9A%E5%B1%82%E6%97%B6%E9%97%B4%E8%BD%AE&zhida_source=entity) 设计则完美地解决了上面的这两个缺陷，那怎么解决的呢 ？ 下面就让我们带着这两个疑问来深入探究一下 Kafka 多层时间轮的设计与实现。

## 5\. Kafka 多层时间轮的设计

如下图所示，Kafka 时间轮的设计模型可以说是和 Netty 时间轮的设计非常相似。

![](images/v2-ac86358847f30ccd93f62a9f3eae6b45_1440w.jpg)
...
Kafka 则使用 TimerTaskList 结构来描述时间轮中的刻度，它是一个带有头结点的 [双向循环链表](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=%E5%8F%8C%E5%90%91%E5%BE%AA%E7%8E%AF%E9%93%BE%E8%A1%A8&zhida_source=entity) 。

![](images/v2-42cc3aa142dc74859aa00e791cc09894_1440w.jpg)
...
![](images/v2-35071906a1821649d9a744668614ed3b_1440w.jpg)
...
![](images/v2-d7b748e33c68a38fef52a0a1ae6e0451_1440w.jpg)
...
![](images/v2-87b49feaf87534f61aceb3b4859475d5_1440w.jpg)
...
![](images/v2-7147b3733b96a3a3079299826004de89_1440w.jpg)
...
2. 第二层时间轮的时钟周期是 400ms ， 可以满足延时 50ms 的任务，于是将该任务重新添加到第二层时间轮的 bucket\[2\] 中。然后将 bucket\[2\] 添加到 DelayQueue 中。
   ![](images/v2-c665e518cb2ecb63c1ae664a9d9af24c_1440w.jpg)
...
taskExecutor 用于执行多层时间轮中的延时任务，它是一个单线程的 [FixedThreadPool](https://zhida.zhihu.com/search?content_id=251927362&content_type=Article&match_order=1&q=FixedThreadPool&zhida_source=entity) 。
...
发布于 2024-12-25 09:28・广东[Kafka](https://www.zhihu.com/topic/20012159)[Netty](https://www.zhihu.com/topic/19732975)[Java](https://www.zhihu.com/topic/19561132)[毕业两年双飞一本科班想转java 无后端经验 还可以转吗 有什么建议？](https://www.zhihu.com/question/9492625783/answer/1924114705322080009)

[

总有人问：半路出家学 Java，真的能行吗？答案是肯定的，但这条路需要你做好打持久战的准备，系统规划每一步成长。尤...

](https://www.zhihu.com/question/9492625783/answer/1924114705322080009)
