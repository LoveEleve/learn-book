...
![](images/v2-7b63429c560d05bb770fb386ff129b99_1440w.jpg)
...
那么在探讨Netty服务端如何接收连接之前，我们需要弄清楚 `Reactor线程` 的 [运行机制](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=1&q=%E8%BF%90%E8%A1%8C%E6%9C%BA%E5%88%B6&zhida_source=entity) ，它是如何监听并处理 `Channel` 上的 `IO就绪事件` 的。
...
![](images/v2-6003e2700c918d55eafa4d73a519b9f2_1440w.jpg)

上图所展示的就是Reactor整个工作体系的全貌，主要分为如下几个重要的工作模块：

1. Reactor线程在Selector上阻塞获取IO就绪事件。在这个模块中首先会去检查当前是否有异步任务需要执行，如果有异步需要执行，那么不管当前有没有IO [就绪事件](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=13&q=%E5%B0%B1%E7%BB%AA%E4%BA%8B%E4%BB%B6&zhida_source=entity) 都不能阻塞在Selector上，随后会去非阻塞的轮询一下Selector上是否有IO就绪事件，如果有，正好可以和异步任务一起执行。优先处理IO就绪事件，在执行异步任务。
...
> 这里需要注意的是netty只会自动注册 `OP_READ` 事件，而 `OP_WRITE事件` 是在当Socket写入 [缓冲区](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=1&q=%E7%BC%93%E5%86%B2%E5%8C%BA&zhida_source=entity) 以满无法继续写入发送数据时由用户自己注册。
...
1. 在最后Netty会判断本次 `Reactor线程` 的唤醒是否是由于触发了 **[JDK epoll 空轮询 BUG](https://link.zhihu.com/?target=https%3A//bugs.java.com/bugdatabase/view_bug.do%3Fbug_id%3D6670302)** 导致的，如果触发了该BUG，则重建 `Selector` 。绕过JDK BUG，达到解决问题的目的。

> 正常情况下Reactor线程从Selector中被唤醒有两种情况：

- 轮询到有IO就绪事件发生。
- 有异步任务或者定时任务需要执行。 而 **[JDK epoll 空轮询 BUG](https://link.zhihu.com/?target=https%3A//bugs.java.com/bugdatabase/view_bug.do%3Fbug_id%3D6670302)** 会在上述两种情况都没有发生的时候， `Reactor线程` 会意外的从 `Selector` 中被唤醒，导致CPU空转。

> JDK epoll 空轮询 BUG： [bugs.java.com/bugdataba](https://link.zhihu.com/?target=https%3A//bugs.java.com/bugdatabase/view_bug.do%3Fbug_id%3D6670302)

好了， `Reactor线程` 的总体运行结构框架我们现在已经了解了，下面我们来深入到这些核心处理模块中来各个击破它们~~

## 1\. Reactor线程轮询IO就绪事件

在 **[《聊聊Netty那些事儿之Reactor在Netty中的实现(创建篇)》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%23rd)** 一文中，笔者在讲述主从Reactor线程组 `NioEventLoopGroup` 的创建过程的时候，提到一个构造器参数 `SelectStrategyFactory` 。
...
![](images/v2-5d59cb1b33e3339c9ec2222037be2a39_1440w.jpg)
...
![](images/v2-5fe1c98e0b5cc864565f53ba9a1a6c36_1440w.jpg)
...
在回答这个问题之前，我们在回顾下 **[《聊聊Netty那些事儿之Reactor在Netty中的实现(创建篇)》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%23rd)** 一文中在讲述 `Reactor的创建` 时提到， `Reactor线程` 除了要轮询 `Channel` 上的 `IO就绪事件` ，以及处理 `IO就绪事件` 外，还有一个任务就是负责执行Netty框架中的 `异步任务` 。
...
![](images/v2-7ab841668d045ef640402e3c0d8f711e_1440w.jpg)
...
![](images/v2-02a364d57b6b1aaa33001f2242d590d8_1440w.jpg)
...
对于 `execute方法` 我想大家一定不会陌生，在上篇文章 **[《详细图解Netty Reactor启动全流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484005%26idx%3D1%26sn%3D52f51269902a58f40d33208421109bc3%26chksm%3Dce77c422f9004d340e5b385ef6ba24dfba1f802076ace80ad6390e934173a10401e64e13eaeb%23rd)** 中我们在介绍 `Reactor线程的启动` 时介绍过该方法。
...
针对execute方法中的这个唤醒条件`!addTaskWakesUp && immediate` ， **[netty](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=3&q=netty&zhida_source=entity) 这里要表达的语义是** ：当immediate参数为true的时候表示该异步任务需要立即执行，addTaskWakesUp 默认设置为false 表示不仅只有addTask方法可以唤醒Reactor，还有其他方法比如这里的execute方法也可以唤醒。但是当设置为true时，语义就变为只有addTask才可以唤醒Reactor，即使execute方法里的immediate = true也不能唤醒Reactor，因为执行的是 [execute方法](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=7&q=execute%E6%96%B9%E6%B3%95&zhida_source=entity) 而不是addTask方法。
...
当 `nextWakeupNanos = AWAKE` 时表示当前Reactor正处于苏醒状态，既然是苏醒状态也就没有必要去执行 `selector.wakeup()` 重复唤醒Reactor了，同时也能省去这一次的 [系统调用](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=3&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) 开销。
...
![](images/v2-c96445afe38223148776d7d45868878e_1440w.jpg)
...
![](images/v2-643d5da0f6ebdd233ce8ab3b244b54d4_1440w.jpg)
...
![](images/v2-e1b27cae99979aae03d8ba050b89c29b_1440w.png)

不知大家还记不记得我们在 **[《聊聊Netty那些事儿之Reactor在Netty中的实现(创建篇)》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%23rd)** 一文中介绍Reactor `NioEventLoop类` 在创建 `Selector` 的过程中提到，出于对 `JDK NIO Selector` 中 `selectedKeys 集合` 的 `插入` 和 `遍历` 操作性能的考虑Netty将自己用数组实现的 `SelectedSelectionKeySet 集合` 替换掉了 `JDK NIO Selector` 中 `selectedKeys` 的 `HashSet` 实现。
...
通过获取 `HashSet` 的 [迭代器](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=1&q=%E8%BF%AD%E4%BB%A3%E5%99%A8&zhida_source=entity) ，开始逐个处理 `IO就绪` 的 `Channel` 。
...
在上篇文章 **[《详细图解Netty Reactor启动全流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484005%26idx%3D1%26sn%3D52f51269902a58f40d33208421109bc3%26chksm%3Dce77c422f9004d340e5b385ef6ba24dfba1f802076ace80ad6390e934173a10401e64e13eaeb%26token%3D773025719%26lang%3Dzh_CN%23rd)** 中我们在讲 `NioServerSocketChannel` 向 `Main Reactor` 注册的时候，通过this指针将自己作为 `SelectionKey` 的 `attachment属性` 注册到 `Selector` 中。 **这一步完成了Netty自定义 `Channel` 和 `JDK NIO Channel` 的绑定** 。
...
- 一种是我们熟悉的 `Channel` ，无论是服务端使用的 `NioServerSocketChannel` 还是客户端使用的 `NioSocketChannel` 都属于 `AbstractNioChannel` 。 `Channel` 上的 `IO事件` 是由 [Netty框架](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=5&q=Netty%E6%A1%86%E6%9E%B6&zhida_source=entity) 负责处理，也是本小节我们要重点介绍的
...
> 这里大家只需记住各个 `IO事件` 在对应 `Channel` 中的处理入口，后续文章我们会详细分析这些 [入口函数](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=1&q=%E5%85%A5%E5%8F%A3%E5%87%BD%E6%95%B0&zhida_source=entity) 。
...
- `JDK NIO 原生 Selector` 存放 `IO就绪的SelectionKey` 的集合为 `HashSet类型` 的 `selectedKeys` 。而Netty为了优化对 `selectedKeys 集合` 的 `遍历效率` 采用了自己实现的 `SelectedSelectionKeySet类型` ，从而用对 `数组` 的遍历代替用 `HashSet` 的 [迭代器遍历](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=1&q=%E8%BF%AD%E4%BB%A3%E5%99%A8%E9%81%8D%E5%8E%86&zhida_source=entity) 。
...
- 另一个是有超时时间限制的 `runAllTasks(long timeoutNanos)方法` 。当 `ioRatio != 100` 时， `Reactor线程` 执行 `异步任务` 会有时间限制，优先一股脑的处理完 `IO就绪事件` 统计出执行 `IO任务` 耗时 `ioTime` 。根据公式 `ioTime * (100 - ioRatio) / ioRatio)` 计算出 `Reactor线程` 执行 `异步任务` 的超时时间。在超时时间限定范围内，执行有限的 `异步任务` 。
  ![](images/v2-9fef4b38a526e8706400259a4a04f910_1440w.jpg)
...
- **由于系统调用 `System.nanoTime()` 需要一定的 [系统开销](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E5%BC%80%E9%94%80&zhida_source=entity) ，所以每执行完 `64` 个 `异步任务` 的时候才会去检查一下 `执行时间` 是否到达了 `deadline`** 。如果到达了执行截止时间 `deadline` 则退出停止执行 `异步任务` 。如果没有到达 `deadline` 则继续从普通 [任务队列](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=34&q=%E4%BB%BB%E5%8A%A1%E9%98%9F%E5%88%97&zhida_source=entity) 中取出任务循环执行下去。
...
![](images/v2-b238aed549870b163d6d018ea30d79f8_1440w.jpg)
...
> 我们在日常开发中也可以借鉴Netty这种处理问题的思路，比如在 [项目开发](https://zhida.zhihu.com/search?content_id=190956327&content_type=Article&match_order=1&q=%E9%A1%B9%E7%9B%AE%E5%BC%80%E5%8F%91&zhida_source=entity) 中，当我们发现我们无法保证彻底的解决一个问题时，或者为了解决这个问题导致我们的投入产出比不高时，我们就该考虑是不是应该换一种思路去绕过这个问题，从而达到同样的效果。 _\*解决问题的最高境界就是不解决它，巧妙的绕过去~~~~~！！_ \*
...
发布于 2022-01-29 09:56[Netty](https://www.zhihu.com/topic/19732975)[Java](https://www.zhihu.com/topic/19561132)[网络编程](https://www.zhihu.com/topic/19611616)[有了豆包学习搭子，作文、翻译、讲解，学习轻松无压力](http://www.doubao.com/download/desktop?ug_apk_token=LmYXY&ad_platform_id=zhihu_feed_lead&ug_callback_url=https%3A%2F%2Fsugar.zhihu.com%2Fplutus_adreaper_callback%3Fsi%3D56abca6a-a41e-47b4-bed8-7e268601184b%26os%3D3%26zid%3D1629%26zaid%3D3764983%26zcid%3D3766026%26cid%3D3766026%26event%3D__EVENTTYPE__%26value%3D__EVENTVALUE__%26score%3D__EVENTSCORE__%26ts%3D__TIMESTAMP__%26cts%3D__TS__%26mh%3D17156ba06675b54a36f061d6b208ffef%26adv%3D784532%26ocg%3D0%26cp%3D0%26ocs%3D0%26aic%3D0%26atp%3D0%26ct%3D0%26ed%3DGiBNJgVzfCMmUW9XFyEvRA8xBGxJICwkOhh0FlwxKw1ZY0gnWzUoISkYf1UXISFcCiIEeR4yNyw9GGJBRCUlVFZ0DngJcmN3eh