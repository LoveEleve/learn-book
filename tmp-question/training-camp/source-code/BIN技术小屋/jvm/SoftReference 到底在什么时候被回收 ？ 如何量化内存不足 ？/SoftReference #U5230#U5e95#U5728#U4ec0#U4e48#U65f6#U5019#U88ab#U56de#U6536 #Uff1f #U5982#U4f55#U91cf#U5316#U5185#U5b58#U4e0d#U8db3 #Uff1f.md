> 本文基于 [OpenJDK17](https://zhida.zhihu.com/search?content_id=244484172&content_type=Article&match_order=1&q=OpenJDK17&zhida_source=entity) 进行讨论， [垃圾回收器](https://zhida.zhihu.com/search?content_id=244484172&content_type=Article&match_order=1&q=%E5%9E%83%E5%9C%BE%E5%9B%9E%E6%94%B6%E5%99%A8&zhida_source=entity) 为 ZGC。
...
大家在网上或者在其他讲解 JVM 的书籍中多多少少会看到这样一段关于 SoftReference 的描述 —— “当 SoftReference 所引用的 [referent](https://zhida.zhihu.com/search?content_id=244484172&content_type=Article&match_order=1&q=referent&zhida_source=entity) 对象在整个堆中没有其他强引用的时候，发生 GC 的时候，如果此时内存充足，那么这个 referent 对象就和其他强引用一样，不会被 GC 掉，如果此时内存不足，系统即将 OOM 之前，那么这个 referent 对象就会被当做垃圾回收掉”。
...
在 ZGC 开始的时候，首先会创建一个 ZDriverGCScope 对象，这里主要进行一些 GC 的准备工作，比如更新 GC 的相关统计信息，设置并行 GC [线程](https://zhida.zhihu.com/search?content_id=244484172&content_type=Article&match_order=1&q=%E7%BA%BF%E7%A8%8B&zhida_source=entity) 个数，以及本小节的重点，初始化 SoftReference 的处理策略 —— \_soft_reference_policy。
...
2. 引起 GC 的原因是 —— `_metadata_GC_clear_soft_refs` ，也就是在 [元数据](https://zhida.zhihu.com/search?content_id=244484172&content_type=Article&match_order=1&q=%E5%85%83%E6%95%B0%E6%8D%AE&zhida_source=entity) 分配失败的时候触发的 Full GC，元空间内存不足，情况就很严重了，所以要无条件清理 SoftReference。
...
JVM 首先会获取我们通过 ` -Xmx` 参数指定的最大堆 —— MaxHeapSize，然后在通过 `Universe::heap()->used_at_last_gc()` 获取上一次 GC 之后 JVM 堆占用的空间，两者相减，就得到了当前 JVM 堆的最大剩余内存空间，并将 [单位转换](https://zhida.zhihu.com/search?content_id=244484172&content_type=Article&match_order=1&q=%E5%8D%95%E4%BD%8D%E8%BD%AC%E6%8D%A2&zhida_source=entity) 为 `MB` 。
...
发布于 2024-06-15 19:30・广东[Java 虚拟机（JVM）](https://www.zhihu.com/topic/19566470)[JVM垃圾回收](https://www.zhihu.com/topic/21850482)[Java](https://www.zhihu.com/topic/19561132)[HFSS仿真研修班首期招生！0基础两个月快速掌握电磁仿真](https://zhuanlan.zhihu.com/p/1921565122821855103)

[

为推动中国智造强国战略，仿真秀以“为行业选才育才”为宗旨，2022年正式成立仿真高研院开展仿真工程师职业培训。 截止目前 已经有超3000学员报名学习，顺利毕业！学员通过两个月集中培...
