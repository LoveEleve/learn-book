> 本文基于 [OpenJDK17](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=OpenJDK17&zhida_source=entity) 进行讨论

在 [JDK NIO](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=JDK+NIO&zhida_source=entity) 针对 [堆外内存](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E5%A0%86%E5%A4%96%E5%86%85%E5%AD%98&zhida_source=entity) 的分配场景中，我们经常会看到 [System.gc](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=System.gc&zhida_source=entity) 的身影，比如当我们通过 `FileChannel#map` 对文件进行 [内存映射](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E6%98%A0%E5%B0%84&zhida_source=entity) 的时候，如果 JVM 进程 [虚拟内存空间](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98%E7%A9%BA%E9%97%B4&zhida_source=entity) 中的虚拟内存不足，JVM 在 native 层就会抛出 `OutOfMemoryError ` 。
...
通常情况下我们应当避免在应用程序中主动调用 `System.gc` ，因为这会导致 JVM 立即触发一次 Full GC，使得整个 JVM 进程陷入到 [Stop The World](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=Stop+The+World&zhida_source=entity) 阶段，对性能会有很大的影响。
...
当 GC 结束之后，JVM 会唤醒 ReferenceHandler [线程](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E7%BA%BF%E7%A8%8B&zhida_source=entity) 去执行 pending 队列中的这些 Cleaner，在 Cleaner 中会释放其背后引用的 Native Memory。

但在现实的 NIO 使用场景中，DirectByteBuffer 却很难触发 GC，因为 DirectByteBuffer 的实例实在太小了（在 JVM 堆中的内存占用），而且通常情况下这些实例是被应用程序长期持有的，很容易就会晋升到老年代。

即使 DirectByteBuffer 实例已经没有任何引用关系了，由于它的实例足够的小，一时很难把老年代撑爆，所以需要等很久才能触发一次 Full GC，在这之前，这些没有任何引用关系的 DirectByteBuffer 实例将会持续在老年代中堆积，其背后所引用的大片 [Native Memory](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=6&q=Native+Memory&zhida_source=entity) 将一直不会得到释放。
...
而位于冰山下面的大一片 Native Memory ，JVM 是察觉不到的， 这也是 Full GC 迟迟不会触发的原因，因此导致了大量的 DirectByteBuffer 实例的堆积，背后引用的一大片 Native Memory 一直得不到释放，严重的情况下可能会导致 [内核](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E5%86%85%E6%A0%B8&zhida_source=entity) 的 OOM，当前进程会被 kill 。
...
Java 堆在 JVM 源码中使用 CollectedHeap 类型来描述，该类型为整个 JVM 堆结构类型的 [基类](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E5%9F%BA%E7%B1%BB&zhida_source=entity) ，具体的实现类型取决于我们选择的 [垃圾回收器](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E5%9E%83%E5%9C%BE%E5%9B%9E%E6%94%B6%E5%99%A8&zhida_source=entity) 。比如，当我们选择 [ZGC](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=ZGC&zhida_source=entity) 作为垃圾回收器时，JVM 堆的类型是 ZCollectedHeap，选择 [G1](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=G1&zhida_source=entity) 作为垃圾回收器时，JVM 堆的类型则是 G1CollectedHeap。
...
GCConfig 是 JVM 专门用于封装 GC 相关信息的类，具体创建堆的行为 —— create_heap()，则封装在 GCConfig 类中的 \_arguments 属性中（GCArguments 类型）。这里也是一样，不同的垃圾回收器对应不同的 GCArguments，比如，ZGC 对应的是 ZArguments，G1 对应的是 G1Arguments。典型工厂， [策略模式](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E7%AD%96%E7%95%A5%E6%A8%A1%E5%BC%8F&zhida_source=entity) 的应用，不同的 GCArguments 负责创建不用类型的 JVM 堆。
...
`select_gc()` 的逻辑其实非常简单，核心就是遍历一个叫做 `IncludedGCs ` 的数组，该数组里包含的是当前 JVM 版本中所支持的所有垃圾回收器集合。比如，当我们通过 [command line](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=command+line&zhida_source=entity) 指定了 `-XX:+UseZGC` 的时候，相关的 GC 参数 UseZGC 就会为 true，其他的 GC 参数都为 false，如果 JVM 在遍历 `IncludedGCs ` 数组的时候发现，当前遍历元素的 GC 参数为 true，那么就会将对应的 \_arguments （zArguments）返回。
...
- 如果我们选择的垃圾回收器是 CMS（已在 Java 9 中废弃），G1，Shenandoah，并且在 command line 中设置了 `-XX:+ExplicitGCInvokesConcurrent` 的话，那么在调用 System.gc 则会立即触发一次 **Concurrent Full GC** ，JVM 进程不会陷入 Stop The World 阶段， [业务线程](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E4%B8%9A%E5%8A%A1%E7%BA%BF%E7%A8%8B&zhida_source=entity) 和 GC 线程可以 [并发运行](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E5%B9%B6%E5%8F%91%E8%BF%90%E8%A1%8C&zhida_source=entity) ，而且调用 System.gc 的线程在触发 **Concurrent Full GC** 之后就立即返回了，不需要等到 GC 结束。
...
这样设计也是合理的，因为毕竟 Full Gc 会让整个 JVM 进程陷入 Stop The World 阶段，所有 Java 线程必须到达 SafePoint 之后 Full Gc 才会执行，而我们通过 JNI 进入到 Native 方法的实现之后，由于 Native 代码不会访问 Java 对象、不会调用 Java 方法，不再执行任何 [字节码](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E5%AD%97%E8%8A%82%E7%A0%81&zhida_source=entity) 指令，所以 Java 虚拟机的 [堆栈](https://zhida.zhihu.com/search?content_id=241515687&content_type=Article&match_order=1&q=%E5%A0%86%E6%A0%88&zhida_source=entity) 不会发生改变，因此 Native 方法本身就是一个 SafePoint。在 Full Gc 没有结束之前，Java 线程会一直停留在这个 SafePoint 中。
...
发布于 2024-04-01 19:39・广东[Java](https://www.zhihu.com/topic/19561132)[Java 虚拟机（JVM）](https://www.zhihu.com/topic/19566470)[JVM](https://www.zhihu.com/topic/19562400)
