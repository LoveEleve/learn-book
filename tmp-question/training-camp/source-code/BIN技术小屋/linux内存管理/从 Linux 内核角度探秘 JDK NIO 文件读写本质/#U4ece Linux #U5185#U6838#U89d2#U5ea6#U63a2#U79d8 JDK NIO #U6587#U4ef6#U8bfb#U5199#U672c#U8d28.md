...
笔者在 **[《从 Linux 内核角度看 IO 模型的演变》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483737%26idx%3D1%26sn%3D7ef3afbb54289c6e839eed724bb8a9d6%26chksm%3Dce77c71ef9004e08e3d164561e3a2708fc210c05408fa41f7fe338d8e85f39c1ad57519b614e%23rd)** 一文中曾对 Socket 文件在内核中的相关 [数据结构](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) 为大家做了详尽的阐述。

![](images/v2-98ad60af2db4faaf74ae5140aadc2a6c_1440w.jpg)

Socket内核结构.png

又在此基础之上介绍了针对 [socket](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=socket&zhida_source=entity) 文件的相关操作及其对应在内核中的处理流程：

![](images/v2-705089a1d31bddab3f774051225bbcbe_1440w.jpg)

系统IO调用结构.png

并与 epoll 的 [工作机制](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%B7%A5%E4%BD%9C%E6%9C%BA%E5%88%B6&zhida_source=entity) 进行了串联：

![](images/v2-dc10253c2cb03b80b0292d5ad0740c20_1440w.jpg)

数据到来epoll_wait流程.png

通过这些内容的串联介绍，我想大家现在一定对 socket 文件非常熟悉了，在我们利用 socket 文件接口在与内核进行网络数据读取，发送的相关交互的时候，不可避免的涉及到一个新的问题，就是我们如何在用户空间设计一个 [字节缓冲区](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%AD%97%E8%8A%82%E7%BC%93%E5%86%B2%E5%8C%BA&zhida_source=entity) 来高效便捷的存储管理这些需要和 socket 文件进行交互的网络数据。

于是笔者又在 **[《一步一图带你深入剖析 JDK NIO ByteBuffer 在不同字节序下的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247485497%26idx%3D1%26sn%3Deb4afe6764b2b976fb80f6dc5c6fd68a%26chksm%3Dce77ce7ef900476865864e09bb6f0688ca784afc396084ecc90a894bfd733692049c332edd11%26token%3D927203489%26lang%3Dzh_CN%23rd)** 一文中带大家从 JDK NIO Buffer 的顶层设计开始，详细介绍了 NIO Buffer 中的顶层抽象设计以及行为定义，随后我们选取了在 [网络应用程序](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E7%BD%91%E7%BB%9C%E5%BA%94%E7%94%A8%E7%A8%8B%E5%BA%8F&zhida_source=entity) 中比较常用的 ByteBuffer 来详细介绍了这个Buffer具体类型的实现，并以 HeapByteBuffer 为例说明了JDK NIO 在不同字节序下的 ByteBuffer 实现。

![](images/v2-711ee25263c4a8dbc2563453daafa762_1440w.jpg)
...
![](images/v2-8e2353a39f487ff127df487d5e111ae6_1440w.jpg)
...
> JDK NIO 中的 FileChannel 比较特殊，它只能是阻塞的，不能设置非阻塞模式。FileChannel的读写方法均是 [线程安全](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E7%BA%BF%E7%A8%8B%E5%AE%89%E5%85%A8&zhida_source=entity) 的。
>
> 注意：下面的例子并不是 [最佳实践](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%9C%80%E4%BD%B3%E5%AE%9E%E8%B7%B5&zhida_source=entity) ，之所以这里引入 HeapByteBuffer 是为了将上篇文章的内容和本文衔接起来。事实上，对于 IO 的操作一般都会选择 DirectByteBuffer ，关于 DirectByteBuffer 的相关内容笔者会在后面的文章中详细为大家介绍。
...
我们首先利用 RandomAccessFile 在内核中打开指定的文件 file-read-write.txt 并获取到它的 [文件描述符](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%96%87%E4%BB%B6%E6%8F%8F%E8%BF%B0%E7%AC%A6&zhida_source=entity) fd = 5000。

![](images/v2-5481cfc15b43adba6e86cbcd9558c082_1440w.jpg)

image.png

随后我们在 JVM 堆中开辟一块 4k 大小的 [虚拟内存](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98&zhida_source=entity) heapByteBuffer，用来读取文件中的数据。

![](images/v2-03d632e007a91a11e6d431df14bd6ec4_1440w.jpg)

image.png

> 操作系统在管理内存的时候是将内存分为一页一页来管理的，每页大小为 4k ，我们在操作内存的时候一定要记得进行页对齐，也就是偏移位置以及读取的内存大小需要按照 4k 进行对齐。具体为什么？文章后边会从内核角度详细为大家介绍。

最后通过 `FileChannel#read` 方法触发底层 [系统调用](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) read。进行文件读取。
...
![](images/v2-226ac8c14ec610dd6588eb13ff5d6cd9_1440w.jpg)
...
内核将文件的 IO 操作根据是否使用内存（ [页高速缓存](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E9%A1%B5%E9%AB%98%E9%80%9F%E7%BC%93%E5%AD%98&zhida_source=entity) page cache）做磁盘热点数据的缓存，将文件 IO 分为：Buffered IO 和 Direct IO 两种类型。
...
而 Java 在 JDK 10 之前一直是不支持 Direct IO 的，到了 JDK 10 才开始支持 Direct IO。但是在 JDK 10 之前我们可以使用第三方的 Direct IO 框架 **[?Jaydio](https://link.zhihu.com/?target=https%3A//github.com/smacke/jaydio)** 来通过 Direct IO 的方式对文件进行读写操作。

> Jaydio GitHub ： [github.com/smacke/jaydi](https://link.zhihu.com/?target=https%3A//github.com/smacke/jaydio)

下面笔者就带大家从内核角度深度剖析下这两种 IO 类型各自的特点：

### 3.1 Buffered IO

大部分 [文件系统](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F&zhida_source=entity) 默认的文件 IO 类型为 Buffered IO，当进程进行文件读取时，内核会首先检查文件对应的页高速缓存 page cache 中是否已经缓存了文件数据，如果有则直接返回，如果没有才会去磁盘中去读取文件数据，而且还会根据非常精妙的预读算法来预先读取后续若干文件数据到 page cache 中。这样等进程下一次顺序读取文件时，想要的数据已经预读进 page cache 中了，进程直接返回，不用再到磁盘中去龟速读取了，这样一来就极大地提高了 IO 性能。

比如一些著名的 [消息队列中间件](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%B6%88%E6%81%AF%E9%98%9F%E5%88%97%E4%B8%AD%E9%97%B4%E4%BB%B6&zhida_source=entity) Kafka, RocketMq 对消息日志文件进行顺序读取的时候，访问速度接近于内存。这就是 Buffered IO 中页高速缓存 page cache 的功劳。在本文的后面，笔者会为大家详细的介绍这一部分内容。

![](images/v2-4140c9c0aae9f3df2b0e3127ff3e31e7_1440w.jpg)

image.png

如果我们使用在上篇文章 **[《一步一图带你深入剖析 JDK NIO ByteBuffer 在不同字节序下的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247485497%26idx%3D1%26sn%3Deb4afe6764b2b976fb80f6dc5c6fd68a%26chksm%3Dce77ce7ef900476865864e09bb6f0688ca784afc396084ecc90a894bfd733692049c332edd11%26token%3D927203489%26lang%3Dzh_CN%23rd)** 中介绍的 HeapByteBuffer 来接收 NIO 读取文件数据的时候，整个文件读取的过程分为如下几个步骤：
...
1. 随后 NIO 会在 **用户态** 调用系统调用 read 向内核发起文件读取的请求。此时发生 **第一次 [上下文切换](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E4%B8%8A%E4%B8%8B%E6%96%87%E5%88%87%E6%8D%A2&zhida_source=entity)** 。
2. 用户进程随即转到内核态运行，进入 [虚拟文件系统](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F&zhida_source=entity) 层，在这一层内核首先会查看读取文件对应的页高速缓存 page cache 中是否含有请求的文件数据，如果有直接返回，避免一次磁盘 IO。并根据内核预读算法从磁盘中 **异步预读** 若干文件数据到 page cache 中（文件顺序读取高性能的关键所在）。

> 在内核中，一个文件对应一个 page cache 结构，注意：这个 page cache 在内存中只会有一份。

1. 如果进程请求数据不在 page cache 中，则会进入文件系统层，在这一层调用块 [设备驱动](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E8%AE%BE%E5%A4%87%E9%A9%B1%E5%8A%A8&zhida_source=entity) 程序触发真正的磁盘 IO。并根据内核预读算法 **同步预读** 若干文件数据。请求的文件数据和预读的文件数据将被一起填充到 page cache 中。
2. 在块设备驱动层完成真正的磁盘 IO。在这一层会从磁盘中读取进程请求的文件数据以及内核预读的文件数据。
3. 磁盘控制器 DMA 将从磁盘中读取的数据拷贝到页高速缓存 page cache 中。 **发生第一次数据拷贝** 。
4. 随后 CPU 将 page cache 中的数据拷贝到 NIO 在用户空间临时创建的缓冲区 DirectByteBuffer 中， **发生 [第二次数据](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E7%AC%AC%E4%BA%8C%E6%AC%A1%E6%95%B0%E6%8D%AE&zhida_source=entity) 拷贝** 。
5. 最后系统调用 read 返回。进程从 [内核态](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=3&q=%E5%86%85%E6%A0%B8%E6%80%81&zhida_source=entity) 切换回 [用户态](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=2&q=%E7%94%A8%E6%88%B7%E6%80%81&zhida_source=entity) 。 **发生第二次上下文切换** 。
6. NIO 将 DirectByteBuffer 中临时存放的文件数据拷贝到 JVM 堆中的 HeapBytebuffer 中。 **发生第三次 [数据拷贝](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=6&q=%E6%95%B0%E6%8D%AE%E6%8B%B7%E8%B4%9D&zhida_source=entity)** 。
...
另外还有一种情况是，当我们在随机读取文件的时候，也不希望内核使用 page cache。因为这样违反了程序 [局部性原理](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%B1%80%E9%83%A8%E6%80%A7%E5%8E%9F%E7%90%86&zhida_source=entity) ，当我们随机读取文件的时候，内核预读进 page cache 中的数据将很久不会再次得到访问，白白浪费 page cache 空间不说，还额外增加了预读的磁盘 IO。

基于以上两点原因，我们很自然的希望内核能够提供一种机制可以绕过 page cache 直接对磁盘进行读写操作。这种机制就是本小节要为大家介绍的 Direct IO。

下面是内核采用 Direct IO 读取文件的工作流程：

![](images/v2-8494fc297145cc5ae55f151aed99d84d_1440w.jpg)

image.png

Direct IO 和 Buffered IO 在进入内核虚拟文件系统层之前的流程全部都是一样的。区别就是进入到虚拟文件系统层之后，Direct IO 会绕过 page cache 直接来到文件系统层通过 direct_io 调用来到块驱动设备层，在块设备驱动层调用 \_\_blockdev_direct_IO 对磁盘内容直接进行读写。

- 和 Buffered IO 一样，在系统调用 read 进入内核以及 Direct IO 完成从内核返回的时候各自会发生一次上下文切换。 **共两次上下文切换**
- 磁盘控制器 DMA 从磁盘中读取数据后直接拷贝到 [用户空间缓冲区](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E7%94%A8%E6%88%B7%E7%A9%BA%E9%97%B4%E7%BC%93%E5%86%B2%E5%8C%BA&zhida_source=entity) DirectByteBuffer 中。 **只发生一次 DMA 拷贝**
- 随后 NIO 将 DirectByteBuffer 中临时存放的数据拷贝到 JVM 堆 HeapByteBuffer 中。 **发生第二次数据拷贝** 。
- 注意块设备驱动层的 \_\_blockdev_direct_IO 需要等到所有的 Direct IO 传送数据完成之后才会返回， **这里的传送指的是直接从磁盘拷贝到用户空间缓冲区中** ，当 Direct IO 模式下的 read() 或者 write() 系统调用返回之后，进程就可以安全放心地去读取用户缓冲区中的数据了。
...
> 对于 Java 程序员来说， [file_operations](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=2&q=file_operations&zhida_source=entity) 大家可以把它当做内核针对文件相关 [操作定义](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%93%8D%E4%BD%9C%E5%AE%9A%E4%B9%89&zhida_source=entity) 的一个公共接口（其实就是一个 [函数指针](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%87%BD%E6%95%B0%E6%8C%87%E9%92%88&zhida_source=entity) ），它只是一个接口。具体的实现根据不同的文件类型有所不同。

比如我们在 **[《聊聊Netty那些事儿之从内核角度看IO模型》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483737%26idx%3D1%26sn%3D7ef3afbb54289c6e839eed724bb8a9d6%26chksm%3Dce77c71ef9004e08e3d164561e3a2708fc210c05408fa41f7fe338d8e85f39c1ad57519b614e%23rd)** 一文中详细介绍过的 Socket 文件。针对 Socket 文件类型，这里的 file_operations 指向的是 socket_file_ops。
...
![](images/v2-4da3d511c1efacfdd0f6e1340c14fee9_1440w.jpg)

进程中管理文件列表结构.png

而本小节中我们讨论的是对普通文件的操作，针对普通文件的操作定义在具体的文件系统中，这里我们以 Linux 中最为常见的 [ext4](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=ext4&zhida_source=entity) 文件系统为例说明：
...
![](images/v2-878539701295252c2b25d13c770bb819_1440w.jpg)
...
所以在虚拟文件系统 VFS 中，\_\_vfs_read 调用的是 new_sync_read 方法，在该方法中会对系统调用传进来的参数进行重新封装。比如：

- struct file \*filp ： 要读取文件的 struct file 结构。
- char \_\_user \*buf ：用户空间的 Buffer，这里指的我们例子中 NIO 创建的临时 DirectByteBuffer。
- size_t count ：进行读取的 [字节数](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%AD%97%E8%8A%82%E6%95%B0&zhida_source=entity) 。也就是我们传入的用户态缓冲区 DirectByteBuffer 剩余可容纳的容量大小。
...
当应用程序期望使用自定义的 [缓存算法](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E7%BC%93%E5%AD%98%E7%AE%97%E6%B3%95&zhida_source=entity) 从而可以在用户空间实现更加高效更加可控的缓存逻辑时（比如数据库等应用程序），这时应该使用直接 Direct IO。在随机读取，随机写入的场景中也是比较适合用 Direct IO。

操作系统进程在接下来使用 read() 或者 write() 系统调用去读写文件的时候使用的是 Direct IO 方式，所传输的数据均不经过文件对应的高速缓存 page cache （这里就是网上常说的内核缓冲区）。

我们都知道操作系统是将内存分为一页一页的单位进行组织管理的，每页大小 4K ，那么同样文件中的数据在磁盘中的组织形式也是按照一块一块的单位来组织管理的，每 [块大小](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%9D%97%E5%A4%A7%E5%B0%8F&zhida_source=entity) 也是 4K ，所以我们在使用 Direct IO 读写数据时必须要按照文件在磁盘中的组织单位进行磁盘块大小对齐，缓冲区的大小也必须是磁盘块大小的整数倍。具体表现在如下几点：
...
由于磁盘文件中的数据是按照块为单位来组织管理的，所以文件系统其实就是一个块设备，通过 ext4_direct_IO 绕过 page cache 直接来到了文件系统的块设备驱动层，最终在块设备驱动层调用 \_\_blockdev_direct_IO 来完成磁盘的读写操作。

> 注意：块设备驱动层的 \_\_blockdev_direct_IO 需要等到所有的 Direct IO 传送数据完成之后才会返回，这里的传送指的是直接从磁盘拷贝到用户空间缓冲区中，当 Direct IO 模式下的 read() 或者 write() 系统调用返回之后，进程就可以安全放心地去读取用户缓冲区中的数据了。
...
2. 如果 page cache 中不存在文件数据的缓存页，就需要通过 page_cache_sync_readahead 方法从磁盘中读取数据并缓存到 page cache 中。于此同时还需要 **同步** 预读若干相邻的 [数据块](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E5%9D%97&zhida_source=entity) 到 page cache 中。这样在下一次顺序读取的时候，直接就可以从 page cache 中读取了。
...
![](images/v2-cf761e3b66ad93f4a2a5cfc06405d90c_1440w.jpg)
...
笔者在 **[《一文聊透对象在 JVM 中的内存布局，以及内存对齐和压缩指针的原理及应用》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484304%26idx%3D1%26sn%3D54bf0d07e69c5621c145afaece8f50d6%26chksm%3Dce77c5d7f9004cc1249a03dfd0fb12b7d75171f1b87acea1fa44bbb11ca374b6f42a66fa274d%23rd)** 文章中为大家介绍 CPU 的高速缓存时曾提到过，根据 [摩尔定律](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%91%A9%E5%B0%94%E5%AE%9A%E5%BE%8B&zhida_source=entity) ：芯片中的晶体管数量每隔 18 个月就会翻一番。导致 CPU 的性能和处理速度变得越来越快，而提升 CPU 的运行速度比提升内存的运行速度要容易和便宜的多，所以就导致了 CPU 与内存之间的速度差距越来越大。

CPU 与内存之间的速度差异到底有多大呢？ 我们知道 [寄存器](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%AF%84%E5%AD%98%E5%99%A8&zhida_source=entity) 是离 CPU 最近的，CPU 在访问寄存器的时候速度近乎于 0 个 [时钟周期](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%97%B6%E9%92%9F%E5%91%A8%E6%9C%9F&zhida_source=entity) ，访问速度最快，基本没有时延。而访问内存则需要 50 - 200 个时钟周期。
...
![](images/v2-ceb2d280e3d0665242097cdc48f28424_1440w.jpg)
...
> 程序局部性原理表现为：时间局部性和空间局部性。时间局部性是指如果程序中的某条指令一旦执行，则不久之后该指令可能再次被执行；如果某块数据被访问，则不久之后该数据可能再次被访问。空间局部性是指一旦程序访问了某个 [存储单元](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%AD%98%E5%82%A8%E5%8D%95%E5%85%83&zhida_source=entity) ，则不久之后，其附近的存储单元也将被访问。
...
> page cache 中缓存的不仅有基于文件的缓存页，还会缓存 [内存映射文件](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E6%98%A0%E5%B0%84%E6%96%87%E4%BB%B6&zhida_source=entity) ，以及磁盘块设备文件。这里大家只需要有这个概念就行，本文我们主要聚焦于基于文件的缓存页。在笔者后面的文章中，我们还会再次介绍到这些剩余类型的缓存页。
...
![](images/v2-778f135005bcc22dabe066c14a756de9_1440w.jpg)
...
在 page cache 结构 struct address_space 中有一个类型为 struct radix_tree_root 的字段 page_tree，它表示的是 radix_tree 的 [根节点](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%A0%B9%E8%8A%82%E7%82%B9&zhida_source=entity) 。
...
![](images/v2-4718b586405615876ad36b6a4fff4b13_1440w.jpg)

image.png

`void __rcu  *slots[RADIX_TREE_MAP_SIZE]` ：radix_tree 树中的每个节点中包含一个 slots ，它是一个包含 64 个指针的数组，每个指针指向它的下一层节点或者缓存页描述符 struct page。

radix_tree 将缓存页全部存放在它的 [叶子结点](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%8F%B6%E5%AD%90%E7%BB%93%E7%82%B9&zhida_source=entity) 中，所以它的叶子结点类型为 struct page。其余的节点类型为 radix_tree_node。最底层的 radix_tree_node 节点中的 slots 指向缓存页描述符 struct page。
...
这里大家可能已经注意到了在 struct radix_tree_node 结构中还有一个 long 型的 tags [二维数组](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E4%BA%8C%E7%BB%B4%E6%95%B0%E7%BB%84&zhida_source=entity) `tags[RADIX_TREE_MAX_TAGS][RADIX_TREE_TAG_LONGS]` 。那么这个二维数组到底是用来干嘛的呢？我们接着往下看~~
...
![](images/v2-b53005f03acac306dbaea5d93cb2471c_1440w.jpg)
...
![](images/v2-00809ab35436197ab05ba3f427104fa1_1440w.jpg)

image.png

只要一个缓存页（叶子结点）被标记，那么从这个叶子结点一直到 radix_tree 根节点的路径将会全部被标记。这就好比你在一盆清水中滴入一滴墨水，不久之后整盆水就会变为黑色。

![](images/v2-6e947c12a835289bab0a107743405e99_1440w.jpg)
...
经过上小节相关内容的介绍，我们了解到在 radix_tree 中每个节点 radix_tree_node 包含一个大小为 64 的 [指针数组](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%8C%87%E9%92%88%E6%95%B0%E7%BB%84&zhida_source=entity) slots 用于指向它的子节点或者缓存页描述符（叶子节点）。
...
![](images/v2-d94816f6d3d062801c8e3a59140cab7a_1440w.jpg)

image.png

而如果一颗 radix_tree 的深度为 2，那么它就可以缓存 64 \* 64 = 4096 个文件页，总共可以缓存 16M 的文件内容。

![](images/v2-ac8381c2676b03bdc3b1ffd502dcdd34_1440w.jpg)
...
如果一颗 radix_tree 的深度为 2（不包括叶子节点），那么它就可以缓存 64 \* 64 = 4096 个文件页，表示的索引范围为 0 - 4095，在这种情况下，缓存页索引 offset 的低 12 位可以分成 两个 6 位的字段，高位的字段用来表示第一层节点的 slots 数组的下标，低 [位字段](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E4%BD%8D%E5%AD%97%E6%AE%B5&zhida_source=entity) 用于表示第二层节点的 slots 数组下标。
...
通过以上根据缓存页索引 offset 的查找过程，我们看出内核在 page cache 查找缓存页的 [时间复杂度](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%97%B6%E9%97%B4%E5%A4%8D%E6%9D%82%E5%BA%A6&zhida_source=entity) 和 radix_tree 的深度有关。
...
- 进程在读取文件数据的时候都是逐页进行读取的，因此在预读文件页的时候内核并不会考虑页内偏移，而是根据请求数据在文件内部的页偏移进行读取。
  ![](images/v2-a72080bf50198afab2acd8ce8f4e31dd_1440w.jpg)
...
- 预读窗口（ahead window）：预读窗口的页面都是内核正在预读的文件页，它们此时并不在 page cache 中。这些页面并不是进程请求的文件页，但是内核根据空间局部性原理假定它们迟早会被进程请求。预读窗口内的页面紧跟着当前窗口后面，并且内核会动态调整预读窗口的大小（有点类似于 TCP 中的滑动窗口）。
  ![](images/v2-b2168578cdbcd927ebbdf4ca22dcde8f_1440w.jpg)
...
- 当进程第一次访问文件时，并且请求的第一个文件页在文件中的 [偏移量](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E5%81%8F%E7%A7%BB%E9%87%8F&zhida_source=entity) 为 0 时表示进程从头开始读取文件，那么内核就会认为进程想要顺序的访问文件，随后内核就会从文件的第一页开始创建一个新的当前窗口，初始的当前窗口总是 2 的次幂，窗口具体大小与进程的读操作所请求的页数有一定的关系。请求页数越大，当前窗口就越大，直到最大值 ra->ra_pages 。
...
当内核通过以上介绍的预读算法确定了预读窗口的大小之后，就开始调用 \_\_do_page_cache_readahead 从磁盘去预读指定的页数到 page cache 中。

### 9.3 \_\_do_page_cache_readahead
...
![](images/v2-8aa55cdc97516040ad0f3e37114a5ace_1440w.jpg)
...
> 再次强调一下，本文所举示例中用到的 HeapByteBuffer 只是为了与上篇文章 **[《一步一图带你深入剖析 JDK NIO ByteBuffer 在不同字节序下的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247485497%26idx%3D1%26sn%3Deb4afe6764b2b976fb80f6dc5c6fd68a%26chksm%3Dce77ce7ef900476865864e09bb6f0688ca784afc396084ecc90a894bfd733692049c332edd11%26token%3D927203489%26lang%3Dzh_CN%23rd)** 介绍的内容做出呼应，并不是最佳实践。笔者会在后续的文章中一步一步为大家展开这块内容的最佳实践。

### 11.1 Buffered IO

![](images/v2-3ace7080594e2ce55bff74759070b186_1440w.jpg)
...
![](images/v2-65bb4c62606497dd561619abda7cf8e5_1440w.jpg)
...
由于 ext4_file_operations 中只定义了.write_iter 函数指针，所以在 \_\_vfs_write 函数中流程进入 else if {......} 分支来到 new_sync_write 函数中：
...
我们看到在文件系统 ext4 中调用的是 \_\_generic_file_write_iter 方法。内核针对文件写入的所有逻辑都封装在这里。
...
![](images/v2-f4f5a92aeb45012a975392b2350f330b_1440w.jpg)
...
为了解决这种场景，于是 linux 内核引入了 ext3, ext4 等 [日志文件系统](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%97%A5%E5%BF%97%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F&zhida_source=entity) 。而日志文件系统比非日志文件系统在磁盘中多了一块 Journal 区域，Journal 区域就是存放管理文件元数据和文件 [数据操作](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E6%93%8D%E4%BD%9C&zhida_source=entity) 日志的磁盘区域。
...
但是这里不能直接进行拷贝，因为此时从 page cache 中取出的缓存页 page 是 [物理地址](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%9C%B0%E5%9D%80&zhida_source=entity) ， **而在内核中是不能够直接操作物理地址的，只能操作虚拟地址** 。
...
其实这三个问题中涉及到的具体数值，内核都提供了参数供我们来配置。这些参数的 [配置文件](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E9%85%8D%E7%BD%AE%E6%96%87%E4%BB%B6&zhida_source=entity) 存在于 `proc/sys/vm` 目录下：

![](images/v2-35c4626211a29f4573938b24f7302075_1440w.jpg)
...
![](images/v2-445e5d921720fcd7fa408984fc167b24_1440w.png)
...
bdi_writeback 有个 [成员变量](https://zhida.zhihu.com/search?content_id=212946140&content_type=Article&match_order=1&q=%E6%88%90%E5%91%98%E5%8F%98%E9%87%8F&zhida_source=entity) struct delayed_work dwork，bdi_writeback 就是把 delayed_work 结构挂到 bdi_wq 队列上的。
...
![](images/v2-52cf6f969a9a6b1f41a76c7735e35f41_1440w.png)

image.png

> 系统的可用内存 = 空闲内存 + 可回收内存。可以通过 free 命令的 available 项查看。

![](images/v2-a56ed5a064033f1cf2634ca076cc8186_1440w.png)
...
![](images/v2-7a136d554102bd7b5a478cc2cdbb2e23_1440w.png)

image.png

> dirty_background_bytes 的优先级大于 drity_background_ratio 的优先级。

dirty_ratio ： dirty_background\_\* 相关的内核配置参数均是内核通过唤醒 flusher 线程来异步回写脏页。下面要介绍的 dirty\_\* 配置参数，均是由用户进程 **同步** 回写脏页。表示内存中的脏页太多了，用户进程自己都看不下去了，不用等内核 flusher 线程唤醒， **用户进程自己主动去回写脏页到磁盘中** 。当脏页占用系统可用内存的比例达到 dirty_ratio 配置的值时，用户进程同步回写脏页。默认值为：20 。

![](images/v2-4c5934ab1c4e74b003e9d8e93c5c3f95_1440w.png)
...
![](images/v2-239e42844e38dafed481666dd6edb7d1_1440w.png)
...
![](images/v2-6b882da0de8d3856d9618e46c5c5b0d2_1440w.png)
...
![](images/v2-b8689548bc59526fef5c88357468a666_1440w.jpg)
...
编辑于 2022-09-06 11:12[Linux](https://www.zhihu.com/topic/19554300)[Java](https://www.zhihu.com/topic/19561132)[JDK](https://www.zhihu.com/topic/19619153)[有了豆包学习搭子，作文、翻译、讲解，学习轻松无压力](http://www.doubao.com/download/desktop?ug_apk_token=LmYXY&ad_platform_id=zhihu_feed_lead&ug_callback_url=https%3A%2F%2Fsugar.zhihu.com%2Fplutus_adreaper_callback%3Fsi%3D62b288ac-409d-4ee1-ab69-256ae30df181%26os%3D3%26zid%3D1629%26zaid%3D3764983%26zcid%3D3766026%26cid%3D3766026%26event%3D__EVENTTYPE__%26value%3D__EVENTVALUE__%26score%3D__EVENTSCORE__%26ts%3D__TIMESTAMP__%26cts%3D__TS__%26mh%3D17156ba06675b54a36f061d6b208ffef%26adv%3D784532%26ocg%3D0%26cp%3D0%26ocs%3D0%26aic%3D0%26atp%3D0%26ct%3D0%26ed%3DGiBNJgVzfCMmUW9XFyEvRA8xBGxJICwkOhh0FlwxKw1ZY0gnWzUoISkYf1UXISFcCiIEeR4yNyw9GGJBRCUlVFZ0DngJcmN3ehFkUwBkfwNZY1YkXSo-fX4DPhIMZ3QGWHUOegFwTvnNPS8Upjg%3D&cb=https%3A%2F%2Fsugar.zhihu.com%2Fplutus_adreaper_callback%3Fsi%3D62b288ac-409d-4ee1-ab69-256ae30df181%26os%3D3%26zid%3D1629%26zaid%3D3764983%26zcid%3D3766026%26cid%3D3766026%26event%3D__EVENTTYPE__%26value%3D__EVENTVALUE__%26score%3D__EVENTSCORE__%26ts%3D__TIMESTAMP__%26cts%3D__TS__%26mh%3D17156ba06675b54a36f061d6b208ffef%26adv%3D784532%26ocg%3D0%26cp%3D0%26ocs%3D0%26aic%3D0%26atp%3D0%26ct%3D0%26ed%3DGiBNJgVzfCMmUW9XFyEvRA8xBGxJICwkOhh0FlwxKw1ZY0gnWzUoISkYf1UXISFcCiIEeR4yNyw9GGJBRCUlVFZ0DngJcmN3ehFkUwBkfwNZY1YkXSo-fX4DPhIMZ3QGWHUOegFwTvnNPS8Upjg%3D&ug_semver=v1.0.0&spu=biz%3D0%26ci%3D3766026%26si%3Dc0379927-e8fb-4940-91e5-d06473d07207%26ts%3D1787017812%26zid%3D1629)

[

学生党学习搭子-豆包AI！不仅可以输出中英文作文、英语翻译、作文修改润色，还能有海量题目讲解

](http://www.doubao.com/download/desktop?ug_apk_token=LmYXY&ad_platform_id=zhihu_feed_lead&ug_callback_url=https%3A%2F%2Fsugar.zhihu.com%2Fplutus_adreaper_callback%3Fsi%3D62b288ac-409d-4ee1-ab69-256ae30df181%26os%3D3%26zid%3D1629%26zaid%3D3764983%26zcid%3D3766026%26cid%3D3766026%26event%3D__EVENTTYPE__%26value%3D__EVENTVALUE__%26score%3D__EVENTSCORE__%26ts%3D__TIMESTAMP__%26cts%3D__TS__%26mh%3D17156ba06675b54a36f061d6b208ffef%26adv%3D784532%26ocg%3D0%26cp%3D0%26ocs%3D0%26aic%3D0%26atp%3D0%26ct%3D0%26ed%3DGiBNJgVzfCMmUW9XFyEvRA8xBGxJICwkOhh0FlwxKw1ZY0gnWzUoISkYf1UXISFcCiIEeR4yNyw9GGJBRCUlVFZ0DngJcmN3ehFkUwBkfwNZY1YkXSo-fX4DPhIMZ3QGWHUOegFwTvnNPS8Upjg%3D&cb=https%3A%2F%2Fsugar.zhihu.com%2Fplutus_adreaper_callback%3Fsi%3D62b288ac-409d-4ee1-ab69-256ae30df181%26os%3D3%26zid%3D1629%26zaid%3D3764983%26zcid%3D3766026%26cid%3D3766026%26event%3D__EVENTTYPE__%26value%3D__EVENTVALUE__%26score%3D__EVENTSCORE__%26ts%3D__TIMESTAMP__%26cts%3D__TS__%26mh%3D17156ba06675b54a36f061d6b208ffef%26adv%3D784532%26ocg%3D0%26cp%3D0%26ocs%3D0%26aic%3D0%26atp%3D0%26ct%3D0%26ed%3DGiBNJgVzfCMmUW9XFyEvRA8xBGxJICwkOhh0FlwxKw1ZY0gnWzUoISkYf1UXISFcCiIEeR4yNyw9GGJBRCUlVFZ0DngJcmN3ehFkUwBkfwNZY1YkXSo-fX4DPhIMZ3QGWHUOegFwTvnNPS8Upjg%3D&ug_semver=v1.0.0&spu=biz%3D0%26ci%3D3766026%26si%3Dc0379927-e8fb-4940-91e5-d06473d07207%26ts%3D1787017812%26zid%3D1629)
