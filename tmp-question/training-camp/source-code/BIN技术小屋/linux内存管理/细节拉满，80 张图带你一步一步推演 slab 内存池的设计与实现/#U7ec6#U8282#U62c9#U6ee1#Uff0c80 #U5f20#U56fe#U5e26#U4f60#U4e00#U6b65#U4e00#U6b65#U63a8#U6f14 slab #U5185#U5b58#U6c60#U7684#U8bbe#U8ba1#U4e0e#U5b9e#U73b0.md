...
在之前的几篇内存管理系列文章中，笔者带大家从宏观角度完整地梳理了一遍 Linux 内存分配的整个链路，本文的主题依然是内存分配，这一次我们会从微观的角度来探秘一下 Linux 内核中用于零散小 [内存块](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%9D%97&zhida_source=entity) 分配的内存池 —— [slab 分配器](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=slab+%E5%88%86%E9%85%8D%E5%99%A8&zhida_source=entity) 。
...
![](images/v2-8886102625d3c83c0c33b961292c6643_1440w.jpg)

物理内存分配 api.png

![](images/v2-73f6ab77a6ffe23c86c2bf3cacd4092c_1440w.jpg)

物理内存释放 api.png

其中物理内存分配在内核中的全链路流程如下图所示：

![](images/v2-56aa0960b3fdded486023a2e2f2f451c_1440w.jpg)

物理内存分配全流程.png

在 Linux 内核中，真正负责物理内存分配的核心是 **[伙伴系统](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247487228%26idx%3D1%26sn%3D85e44fa5b090b29ab23ca6abf98da221%26chksm%3Dce77c8bbf90041ad4958a3871a880a3f6d812e282530ea047d9eaf76d9f03aafb4b987a64ae9%23rd)** ，在我们从总体上熟悉了物理内存分配的全链路流程之后，随后我们继续来到了伙伴系统的入口 get_page_from_freelist 函数，它的完整流程如下：

![](images/v2-d13546400ac621ab383e8e6f03e32f41_1440w.jpg)

image.png

内核通过 get_page_from_freelist 函数，挨个遍历检查各个 [NUMA 节点](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=NUMA+%E8%8A%82%E7%82%B9&zhida_source=entity) 中的物理内存区域是否有足够的空闲内存可以满足本次的内存分配要求，当找到符合内存分配标准的物理内存区域 zone 之后，接下来就会通过 rmqueue 函数进入到该物理内存区域 zone 对应的伙伴系统中分配物理内存。

![](images/v2-822cc95a0cb81bab326c5d6409bf533d_1440w.jpg)

image.png

那么内核既然已经有了伙伴系统，那么为什么还需要一个 slab 内存池呢 ？下面就让我们从这个疑问开始，正式拉开本文的帷幕~~~

![](images/v2-98cc0694e47de69b38f4ee84f0bdc85a_1440w.jpg)

## 2\. 既然有了伙伴系统，为什么还需要 Slab?

从上篇文章 **[《深度剖析 Linux 伙伴系统的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247487228%26idx%3D1%26sn%3D85e44fa5b090b29ab23ca6abf98da221%26chksm%3Dce77c8bbf90041ad4958a3871a880a3f6d812e282530ea047d9eaf76d9f03aafb4b987a64ae9%26token%3D2104970935%26lang%3Dzh_CN%23rd)** 第一小节 “1. 伙伴系统的核心 [数据结构](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) ” 的介绍中我们知道，内核中的伙伴系统管理内存的最小单位是 [物理内存页](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%86%85%E5%AD%98%E9%A1%B5&zhida_source=entity) page。

伙伴系统会将它所属物理内存区 zone 里的 [空闲内存](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=2&q=%E7%A9%BA%E9%97%B2%E5%86%85%E5%AD%98&zhida_source=entity) 划分成不同尺寸的 [物理内存块](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%86%85%E5%AD%98%E5%9D%97&zhida_source=entity) ，这里的尺寸必须是 2 的次幂，物理内存块可以是由 1 个 page 组成，也可以是 2 个 page，4 个 page........ 1024 个 page 组成。

内核将这些相同尺寸的内存块用一个内核数据结构 struct [free_area](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=free_area&zhida_source=entity) 中的 [双向链表](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E5%8F%8C%E5%90%91%E9%93%BE%E8%A1%A8&zhida_source=entity) free_list 串联组织起来。
...
![](images/v2-639344d9c20975671ece96578c1ecfda_1440w.jpg)
...
但是此时我们在 free_area\[order + n\] 链表中找到的 [空闲内存块](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E7%A9%BA%E9%97%B2%E5%86%85%E5%AD%98%E5%9D%97&zhida_source=entity) 的尺寸是 2 ^ (order + n) 大小，而我们需要的是 2 ^ order 尺寸的内存块，于是内核会将这 2 ^ (order + n) 大小的内存块逐级减半分裂，将每一次分裂后的内存块插入到相应的 free_area 数组里对应的 free_list\[MIGRATE_MOVABLE\] 链表中，并将最后分裂出的 2 ^ order 尺寸的内存块分配给进程使用。
...
这种小内存在内核中的使用场景非常之多，比如，内核中那些经常使用，需要频繁申请释放的一些核心数据结构对象： [task_struct](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=task_struct&zhida_source=entity) 对象， [mm_struct](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=mm_struct&zhida_source=entity) 对象，struct page 对象， [struct file](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=struct+file&zhida_source=entity) 对象，socket 对象等。
...
3. 调用伙伴系统的操作会对 CPU 高速缓存 L1Cache 中的 Instruction Cache（指令高速缓存）和 Data Cache （数据高速缓存）有污染，因为对伙伴系统的长链路调用，相关的一些指令和数据必然会填充到 Instruction Cache 和 Data Cache 中，从而将频繁使用的一些指令和数据挤压出去，造成 [缓存污染](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E7%BC%93%E5%AD%98%E6%B1%A1%E6%9F%93&zhida_source=entity) 。而在内核空间中越浪费这些缓存资源，那么在用户空间中的进程就会越少的得到这些缓存资源，造成性能的下降。 slab 对象池极大的减少了对伙伴系统的调用，防止了不必要的 L1Cache 污染。
...
1. 当我们使用 fork() [系统调用](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) 创建进程的时候，内核需要使用 task_struct 专属的 slab 对象池分配 task_struct 对象。
...
1. 当服务端 [网络应用程序](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E7%BD%91%E7%BB%9C%E5%BA%94%E7%94%A8%E7%A8%8B%E5%BA%8F&zhida_source=entity) 使用 accpet 系统调用接收客户端的连接时，内核需要使用 [struct socket](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=struct+socket&zhida_source=entity) 专属的 slab 对象池为新进来的客户端连接分配 socket 对象。
...
![](images/v2-a0e9eebe9aeec5b362c3635ccac331ff_1440w.jpg)
...
![](images/v2-f92a40add168f5846fdcff9053a598fb_1440w.jpg)

image.png

在 **[《深入理解 Linux 虚拟内存管理》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486732%26idx%3D1%26sn%3D435d5e834e9751036c96384f6965b328%26chksm%3Dce77cb4bf900425d33d2adfa632a4684cf7a63beece166c1ffedc4fdacb807c9413e8c73f298%26token%3D668918070%26lang%3Dzh_CN%23rd)** 一文中介绍的虚拟 [内存地址](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%9C%B0%E5%9D%80&zhida_source=entity) 空间相关的对象：
...
由于 slab 的实现非常复杂，slab 中拥有多种存储对象的队列，队列管理开销比较大，slab 元数据比较臃肿，对 NUMA 架构的支持臃肿繁杂（slab 引入时内核还没支持 NUMA），这样导致 slab 内部为了维护这些自身 [元数据管理](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E5%85%83%E6%95%B0%E6%8D%AE%E7%AE%A1%E7%90%86&zhida_source=entity) 结构就得花费大量的内存空间，这在配置有超大容量内存的服务器上，内存的浪费是非常可观的。
...
![](images/v2-5f36e13b066bcc05330b95b2f5f28e89_1440w.jpg)
...
下面笔者就带大家从一个最简单的物理内存页 page 开始，我们一步一步的推演 slab 的整个 [架构设计](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E6%9E%B6%E6%9E%84%E8%AE%BE%E8%AE%A1&zhida_source=entity) 与实现。
...
![](images/v2-9847ba7a1905aadefcce3655413dccbf_1440w.jpg)

image.png

但是在一个工业级的对象池设计中，我们不能这么简单粗暴的搞，因为对象的 object size 可以是任意的，并不是内存对齐的，CPU 访问一块没有进行对齐的内存比访问对齐的 [内存速度](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E9%80%9F%E5%BA%A6&zhida_source=entity) 要慢一倍。
...
![](images/v2-70bfab9c8c0021e5704f7053867946a0_1440w.jpg)
...
![](images/v2-61d4722281de0e2e999c9317ec6f8451_1440w.jpg)
...
![](images/v2-46972c02c39af6c9f8d5cf1a110c66ce_1440w.jpg)
...
- 如果对象尺寸 object size 是通过填充 padding 之后，才与 word size 对齐。内核会巧妙的利用对象右边的这段 padding 填充区域作为 red zone。只需要额外的在对象内存区域的左侧填充一段 red zone 即可。
  ![](images/v2-cf257664dbaccc3ce145e28e12404cd3_1440w.jpg)
...
![](images/v2-d09da6f7583629fcb2bbdead7af9ef6e_1440w.jpg)
...
![](images/v2-febfa4c1359019976a568cc0a2b32197_1440w.jpg)

image.png

这种通过在对象内存 [区域填充](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E5%8C%BA%E5%9F%9F%E5%A1%AB%E5%85%85&zhida_source=entity) 特定字节表示对象的特殊状态的行为，在 slab 中有一个专门的术语叫做 SLAB_POISON （SLAB 中毒）。POISON 这个术语起的真的是只可意会不可言传，其实就是表示 slab 对象的一种状态。
...
![](images/v2-90dc833db7d48aaee9a03eb237c8bcbf_1440w.jpg)
...
![](images/v2-1156b499f753d724756048498da21655_1440w.jpg)
...
![](images/v2-79dc89a3befb971cd0c7cea1cf0e35ba_1440w.jpg)
...
所以在内核中，都是用 struct page 结构来表示 slab，如果 slab 背后依赖的是多个物理内存页，那就使用在 **[《深度剖析 Linux 伙伴系统的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247487228%26idx%3D1%26sn%3D85e44fa5b090b29ab23ca6abf98da221%26chksm%3Dce77c8bbf90041ad4958a3871a880a3f6d812e282530ea047d9eaf76d9f03aafb4b987a64ae9%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文中 " 5.3.2 设置复合页 compound_page " 小节提到的复合页 compound_page 来表示。

![](images/v2-c32e48f795f521e403cbcf5de9acbadf_1440w.jpg)
...
**在笔者当前所在的 [内核版本](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E5%86%85%E6%A0%B8%E7%89%88%E6%9C%AC&zhida_source=entity) 5.4 中** ，内核是使用 struct page 来表示 slab 的，但是考虑到 struct page 结构已经非常庞大且复杂，为了减少 struct page 的内存占用以及提高可读性，内核在 5.17 版本中专门为 slab 引入了一个管理结构 struct slab，将原有 struct page 中 slab 相关的字段全部删除，转移到了 struct slab 结构中。这一点，大家只做了解即可。

## 6\. slab 的总体架构设计

![](images/v2-7ff95712961dcb8d3a57727dcdf9a8fd_1440w.jpg)
...
![](images/v2-654e482df3062682a1b29cc722086800_1440w.jpg)
...
![](images/v2-8795cb01da80dd27285fe7384beea610_1440w.jpg)

image.png

如果一个 slab 中的对象部分被分配出去使用，部分却未被分配仍然在 slab 中缓存，那么内核就会将该 slab 视为一个 partial slab。

![](images/v2-6a80dd600e62e269138b41fe1fe8e493_1440w.jpg)
...
- 当 flags 设置了 SLAB_HWCACHE_ALIGN 时，表示 slab 中的对象需要按照 CPU 硬件高速缓存行 cache line (64 字节) 进行对齐。
  ![](images/v2-2c28f4f4c6679c62afbb02dea35fd5f8_1440w.jpg)
...
![](images/v2-4efaae3ac25dc228884162efa0ab6555_1440w.jpg)
...
![](images/v2-f7358de64058d91ba9d3338e3cae8a6a_1440w.jpg)
...
在 `cat /proc/slabinfo` 命令显示的这些系统中所有的 slab cache，内核会将这些 slab cache 用一个双向链表统一串联起来。链表的 [头结点](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E5%A4%B4%E7%BB%93%E7%82%B9&zhida_source=entity) 指针保存在 struct kmem_cache 结构的 list 中。
...
![](images/v2-574c9a6026aa7ed2065737211b65d3a3_1440w.jpg)

image.png

系统中所有的这些 slab cache 占用的内存总量，我们可以通过 `cat /proc/meminfo` 命令查看：

![](images/v2-ae928fe027918b16ec1dcae0fad75e4f_1440w.jpg)
...
![](images/v2-cf4fe9326de84f678e42862ebd6aadb1_1440w.jpg)
...
下面我们来看一下 slab cache 它的 cpu 本地缓存 kmem_cache_cpu 结构的 [详细设计](https://zhida.zhihu.com/search?content_id=225811486&content_type=Article&match_order=1&q=%E8%AF%A6%E7%BB%86%E8%AE%BE%E8%AE%A1&zhida_source=entity) 细节：
...
![](images/v2-49cf68c3cc225ddeb9dc0512e3f09f6c_1440w.jpg)
...
![](images/v2-8f0c121b41aa745320dbe1d3904881d4_1440w.jpg)
...
![](images/v2-fbaaa34c0b86988e4dc6f14765e7da6e_1440w.jpg)
...
![](images/v2-eb460a14b292abe1c24cf1d8668bb1f6_1440w.jpg)
...
![](images/v2-ae0f4b7d18421070fbc2550a413bc3a3_1440w.jpg)
...
![](images/v2-ffb97263ed89a4f403d18b5eaee84f26_1440w.jpg)
...
![](images/v2-478e4d9996670fba9da2d29c97413080_1440w.jpg)

image.png

### 7.2 从本地 cpu 缓存 partial 列表中分配

![](images/v2-5e4d8dec9197e41a2edbdcfb2a505dfe_1440w.jpg)
...
![](images/v2-68e5bf3bba781cc014ba8df5bf99054e_1440w.jpg)
...
![](images/v2-120a287ccdf877e596e0bd993a216368_1440w.jpg)

image.png

### 7.3 从 NUMA 节点缓存中分配

![](images/v2-03374c56d7d9268f6862cae8768d4791_1440w.jpg)
...
![](images/v2-46dca32f2bf07308beced7c212d0ff3b_1440w.jpg)
...
![](images/v2-ed2dbb439b0b9c99a70487289a459de1_1440w.jpg)
...
![](images/v2-dbaac3050285d12deca8f13910d57d93_1440w.jpg)

image.png

### 7.4 从伙伴系统中重新申请 slab

![](images/v2-b73bdbb258a633b30af997c981e9e8e2_1440w.jpg)
...
![](images/v2-78b270dc47300fc1c14e889d0615b51b_1440w.jpg)
...
![](images/v2-e2bb1fcafcd0dbfd053bd57f49041203_1440w.jpg)
...
![](images/v2-78855f46daf5a626394841021f4c628d_1440w.jpg)
...
![](images/v2-6214540334716dd6384127870baa82c3_1440w.jpg)

image.png

### 8.2 释放对象所属 slab 在 cpu 本地缓存 partial 列表中

![](images/v2-8e5846331b167892cfd90a66778a467c_1440w.jpg)
...
![](images/v2-e8c64f6726689c88abf235bdddfb766c_1440w.jpg)
...
![](images/v2-7b0b9984a2c1bbf12edbdbe5ecfcaf5f_1440w.jpg)
...
![](images/v2-5b5a33152ecf87620258910716a1414d_1440w.jpg)
...
![](images/v2-6b1952f5f8cf1a6256597524dbef99f0_1440w.jpg)
...
![](images/v2-59b48b0fdca9b6c927b1fc2ab47ccfdb_1440w.jpg)
...
![](images/v2-bff83236f543aa459fbd47c879f2bdcf_1440w.jpg)
...
![](images/v2-8d13f77a6aacce5546fd79ca5f6f54ec_1440w.jpg)
...
![](images/v2-b97e84f3f737e5a6d1423c0ecb4301e9_1440w.jpg)
...
发布于 2023-04-05 10:01・广东[Linux 内核](https://www.zhihu.com/topic/19614193)[slab块分配器](https://www.zhihu.com/topic/25758381)[内存管理](https://www.zhihu.com/topic/19579205)[求一个手机配件行业的进销存软件，我都被库存管理弄得头疼?](https://www.zhihu.com/question/428806960/answer/104016748301)

[可以试试用百草进销存软件，很简单易用，10分钟上手。管手机配件库存非常合适。手机电脑都能用云进销存，手机版、电脑版都用，数据互联互通，习惯操作哪一个就用哪一个。简单易用软件...](https://www.zhihu.com/question/428806960/answer/104016748301)
