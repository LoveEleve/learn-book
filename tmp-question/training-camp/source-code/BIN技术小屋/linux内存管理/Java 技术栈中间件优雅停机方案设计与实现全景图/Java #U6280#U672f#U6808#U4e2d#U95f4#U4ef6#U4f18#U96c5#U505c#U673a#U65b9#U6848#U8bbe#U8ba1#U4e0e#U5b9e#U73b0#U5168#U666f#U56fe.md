...
在上篇文章 **[?我为 Netty 贡献源码 | 且看 Netty 如何应对 TCP 连接的正常关闭，异常关闭，半关闭场景](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247485060%26idx%3D1%26sn%3D736360af6eb3a4db496de2d6665ebd3c%26chksm%3Dce77c0c3f90049d5e44692c2cf837e8d85bb28758b243d505c43c48ce703da1edadfc19360b1%23rd)** 中笔者为大家详细介绍了 Netty 在处理连接关闭时的完整过程，并详细介绍了 Netty 如何应对 TCP 连接在关闭时会遇到的各种场景。

在连接关闭之后，接下来就轮到 Netty 的谢幕时刻了，本文笔者会为大家详尽 Java 技术栈中间件中关于优雅停机方案的详细设计和实现。

笔者会从日常开发工作中常见的版本发布，服务上下线的场景聊起，引出服务优雅启停的需求，并从这个需求出发，一步一步带大家探究各个 [中间件](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=2&q=%E4%B8%AD%E9%97%B4%E4%BB%B6&zhida_source=entity) 里的优雅停机的相关设计。

熟悉笔者文风的读者朋友应该知道，笔者肯定不会只是简单的介绍，要么不讲，要讲就要把整个技术体系的前世今生给大家讲清楚，讲明白。

基于这目的，笔者会先从支持 [优雅停机](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=3&q=%E4%BC%98%E9%9B%85%E5%81%9C%E6%9C%BA&zhida_source=entity) 的底层技术基石--内核中的 [信号量](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E4%BF%A1%E5%8F%B7%E9%87%8F&zhida_source=entity) 开始聊起。

![](images/v2-4a62ad74a50676f75ef17cbc479a0724_1440w.jpg)

image.png

从内核层我们接着会聊到 JVM 层，在 JVM 层一探优雅停机底层的技术玄机。

![](images/v2-e94db51c665eca1fd53b76ab87c74e28_1440w.jpg)

image.png

随后我们会从 JVM 层一路奔袭到 Spring 然后到 Dubbo。在这个过程中，笔者还会带大家一起 Shooting Dubbo 在优雅停机下的一个 Bug，并为大家详细介绍修复过程。

![](images/v2-d2b916c43d93144312cb0d7f0a0a803c_1440w.jpg)

image.png

最后由 Dubbo 层的优雅停机，引出我们的主角--Netty 优雅停机的设计与实现：

![](images/v2-099881a4a7845944649ad10318dbe25c_1440w.jpg)

Reactor优雅关闭总流程.png

下面我们来正式开始本文的内容~~

![](images/v2-664a4ab1352971b1bc0ddd6a449fae22_1440w.jpg)
...
![](images/v2-5335ff29e8f0b22ce206306f2709a0bb_1440w.jpg)
...
![](images/v2-d96e872f2a6c0c92de0e26d193793ac7_1440w.png)
...
这个技术方案在众多 RPC 框架的实现中我们都可以看到，服务调用方会从注册中心拿到所有服务提供方的地址，然后从这些地址中通过特定的 [负载均衡算法](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E8%B4%9F%E8%BD%BD%E5%9D%87%E8%A1%A1%E7%AE%97%E6%B3%95&zhida_source=entity) 从中选取一个服务提供方的发送请求。
...
而 [延迟暴露](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=2&q=%E5%BB%B6%E8%BF%9F%E6%9A%B4%E9%9C%B2&zhida_source=entity) 则是从服务提供方的角度，延迟暴露服务时间，利用延迟的这段时间，服务提供方可以预先加载依赖的一些资源，比如： [缓存数据](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E7%BC%93%E5%AD%98%E6%95%B0%E6%8D%AE&zhida_source=entity) ，spring 容器中的 bean 。等到这些资源全部加载完毕就位之后，我们在将服务提供方实例暴露出去。这样可以有效降低启动前期请求处理出错的概率。
...
![](images/v2-c5e53d42a8af45f09816fe44a855d3f5_1440w.jpg)

image.png

第一步肯定是要将程序承担的现有流量全部切走，告诉服务调用方，我要进行关闭了，请不要在给我发送请求。那么如果进行 [切流](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E5%88%87%E6%B5%81&zhida_source=entity) 呢？？
...
信号在内核中并没有用特别复杂的 [数据结构](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) 来表示，只是用一个代号一样的数字来标识不同的信号。Linux 提供了几十种信号，分别代表不同的意义。信号之间依靠它们的值来区分

信号可以在任何时候发送给进程，进程需要为这个信号配置 [信号处理函数](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E4%BF%A1%E5%8F%B7%E5%A4%84%E7%90%86%E5%87%BD%E6%95%B0&zhida_source=entity) 。当某个信号发生的时候，就默认执行对应的信号处理函数就可以了。这就相当于一个操作系统的应急手册，事先定义好遇到什么情况，做什么事情，提前准备好，出了事情照着做就可以了。

内核发出的信号就代表当前系统遇到了某种情况，我们需要应对的步骤就封装在对应信号的 [回调函数](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E5%9B%9E%E8%B0%83%E5%87%BD%E6%95%B0&zhida_source=entity) 中。

信号机制引入的目的就在于：

- 让应用进程知道当前已经发生了某个特定的事件（比如进程的 [关闭事件](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=2&q=%E5%85%B3%E9%97%AD%E4%BA%8B%E4%BB%B6&zhida_source=entity) ）。
...
- `SIGKILL：` 信号代号为 9 。通过 kill -9 pid 命令结束进程是非常非常危险的动作， **我们应该坚决制止这种关闭进程的行为** ，因为 SIGKILL 信号是不能被进程捕获和忽略的，只能执行内核定义的默认操作直接关闭进程。 **而我们的优雅关闭操作是需要通过捕获操作系统信号，从而可以在对应的信号处理函数中执行优雅关闭的动作** 。由于 SIGKILL 信号不能被捕获，所以优雅关闭也就无法实现。现在大家就赶快检查下自己公司生产环境的运维脚本是否是通过 kill -9 pid 命令来结束进程的，一定要避免用这种方式，因为这种方式是极其无情并且略带残忍的关闭进程行为。
  ![](images/v2-ecfa7d3c25b7ea43f4a78e1c3602071b_1440w.jpg)
...
- `捕获信号：` 应用程序可以利用内核提供的 [系统调用](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) 来捕获信号，并将优雅关闭的步骤封装在对应信号的处理函数中。当向进程发送关闭信号 SIGTERM 的时候，在进程内我们可以通过捕获 SIGTERM 信号，随即就会执行我们自定义的信号处理函数。我们从而可以在信号处理函数中执行进程优雅关闭的逻辑。
...
- `__sighandler_t sa_handler：` 其实本质上是一个 [函数指针](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E5%87%BD%E6%95%B0%E6%8C%87%E9%92%88&zhida_source=entity) ，用来保存我们为信号注册的信号处理函数， **优雅关闭的逻辑就封装在这里** 。
...
Shutdown#sequence 函数中的逻辑就是我们在《3.2 使用ShutdownHook的注意事项》小节中介绍的 JVM 关闭时的运行逻辑：在这里会触发所有 ShutdownHook 的 **[并发运行](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E5%B9%B6%E5%8F%91%E8%BF%90%E8%A1%8C&zhida_source=entity)** 。注意这里并不会保证运行顺序。
...
![](images/v2-5fe56bf782348370c10d3d705923bb37_1440w.jpg)
...
![](images/v2-229840c95f7361a8910260a9b76ab4a5_1440w.jpg)
...
![](images/v2-72b1cbd932d560e0937703e9e47e19c9_1440w.png)
...
![](images/v2-17049e0456668a49b701c08d91a8db2e_1440w.jpg)
...
![](images/v2-a0bbf19e36c8249da742e5f06fe1a40e_1440w.jpg)
...
![](images/v2-498d9691901d3539d14100e42aaa11bb_1440w.jpg)
...
![](images/v2-ef63ff3091c6d3d161efa7427a05cc15_1440w.jpg)
...
- Spring 的 ShutdownHook 线程没有获得 destroyLock，退出 destroy() 方法。
  ![](images/v2-5a178af66ae08bbc430618dd93d566d6_1440w.jpg)
...
>
> 详情可查看Issue： [github.com/apache/dubbo](https://link.zhihu.com/?target=https%3A//github.com/apache/dubbo/issues/7093)
...
在 Netty 进行优雅关闭的整个过程中，这里涉及到了两个非常重要的 [控制参数](https://zhida.zhihu.com/search?content_id=208018934&content_type=Article&match_order=1&q=%E6%8E%A7%E5%88%B6%E5%8F%82%E6%95%B0&zhida_source=entity) ：
...
> 这一点笔者也在**[?《Reactor在Netty中的实现（创建篇）》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%26token%3D1670680185%26lang%3Dzh_CN%23rd)** 一文中的第四小节《4. 向Reactor线程组中所有的Reactor注册terminated回调函数》强调过。
...
![](images/v2-40a570647f5b03ad24928b257fd4d999_1440w.jpg)

Reactor的优雅谢幕流程.png

Reactor 的状态特别重要，从**[?《一文聊透Netty核心引擎Reactor的运转架构》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484087%26idx%3D1%26sn%3D0c065780e0f05c23c8e6465ede86cba0%26chksm%3Dce77c4f0f9004de63be369a664105708bc5975b52993f4a6df223caed34cc1ef6185a16acd75%23rd)** 一文中我们知道 Reactor 是在一个 for (;;) {....} 死循环中 996 不停地工作。比如轮询 Channel 上的 IO 就绪事件，处理 IO 就绪事件，执行异步任务就是在这个死循环中完成的。
...
> 相关细节可在回顾下**[?《详细图解Netty Reactor启动全流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484005%26idx%3D1%26sn%3D52f51269902a58f40d33208421109bc3%26chksm%3Dce77c422f9004d340e5b385ef6ba24dfba1f802076ace80ad6390e934173a10401e64e13eaeb%26token%3D1670680185%26lang%3Dzh_CN%23rd)** 一文。
...
>
> 关于 addTaskWakesUp 字段的详细含义和作用，大家可以回顾下**[?《一文聊透 Netty 核心引擎 Reactor 的运转架构》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484087%26idx%3D1%26sn%3D0c065780e0f05c23c8e6465ede86cba0%26chksm%3Dce77c4f0f9004de63be369a664105708bc5975b52993f4a6df223caed34cc1ef6185a16acd75%26token%3D1670680185%26lang%3Dzh_CN%23rd)** 一文中的《1.2.2 Reactor 开始轮询 IO 就绪事件》小节。
...
![](images/v2-e3a759329d343f120929db1563e3ccfd_1440w.jpg)

Reactor线程优雅关闭流程.png

通过**[?《一文聊透Netty核心引擎Reactor的运转架构》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484087%26idx%3D1%26sn%3D0c065780e0f05c23c8e6465ede86cba0%26chksm%3Dce77c4f0f9004de63be369a664105708bc5975b52993f4a6df223caed34cc1ef6185a16acd75%23rd)** 一文的介绍，我们知道 Reacto r是在一个 for 循环中 996 不停地处理 IO 事件以及执行异步任务。如下面笔者提取的 Reactor 运行框架所示：
...
> 关于无效 Key 的清除，详细细节大家可以回看下**[?《一文聊透Netty核心引擎Reactor的运转架构》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247484087%26idx%3D1%26sn%3D0c065780e0f05c23c8e6465ede86cba0%26chksm%3Dce77c4f0f9004de63be369a664105708bc5975b52993f4a6df223caed34cc1ef6185a16acd75%26token%3D1670680185%26lang%3Dzh_CN%23rd)** 一文中的《3.1.3 从Selector中移除失效的SelectionKey》小节。
...
> Channel 的关闭流程可以回看下笔者的这篇文章 **[?《且看 Netty 如何应对 TCP 连接的正常关闭，异常关闭，半关闭场景》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247485060%26idx%3D1%26sn%3D736360af6eb3a4db496de2d6665ebd3c%26chksm%3Dce77c0c3f90049d5e44692c2cf837e8d85bb28758b243d505c43c48ce703da1edadfc19360b1%26token%3D1978355368%26lang%3Dzh_CN%23rd)**
...
![](images/v2-670f1b3cc1ec15875b1949f35671cc85_1440w.jpg)
...
这里还记得我们在**[?《Reactor 在 Netty 中的实现(创建篇)》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483907%26idx%3D1%26sn%3D084c470a8fe6234c2c9461b5f713ff30%26chksm%3Dce77c444f9004d52e7c6244bee83479070effb0bc59236df071f4d62e91e25f01715fca53696%26token%3D1470157323%26lang%3Dzh_CN%23rd)** 一文中介绍的，在 ReactorGroup 中的所有 Reactor 被挨个全部创建成功之后，会向所有 Reactor 的 terminationFuture 注册一个 terminationListener 。
...
![](images/v2-23a517ecebee5de90d83f17c14262d90_1440w.png)
...
发布于 2022-07-09 10:11[Java](https://www.zhihu.com/topic/19561132)[Netty](https://www.zhihu.com/topic/19732975)[dubbo](https://www.zhihu.com/topic/20032385)[小学兴趣班少儿编程横测：西瓜创客、核桃编程、编程猫、有道小图灵四大机构！各自的优势是什么？娃该怎么选？](https://zhuanlan.zhihu.com/p/1962161704680034451)

[

孩子刚开学半个月，我手机上的同学家长信息就爆个不停，因为暑期我让孩子参加了一个编程比赛获奖，这项比赛正好属于白名单奖项。 开学了，老师要统计孩子们暑期的获奖情况，全班就我家...
