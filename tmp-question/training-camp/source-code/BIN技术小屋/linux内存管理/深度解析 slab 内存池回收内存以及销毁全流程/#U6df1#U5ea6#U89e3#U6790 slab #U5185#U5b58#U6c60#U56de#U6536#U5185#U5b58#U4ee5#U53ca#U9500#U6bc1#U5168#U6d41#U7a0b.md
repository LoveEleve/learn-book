在上篇文章 **[《深入理解 slab cache 内存分配全链路实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488152%26idx%3D1%26sn%3D7c65f8ee28e9cc14a86e9df92b6d2b93%26chksm%3Dce77d4dff9005dc9003a7df42b6560df13b581e45927bf3ef3e13cc24acc4eb78c2332bd1351%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 中，笔者详细地为大家介绍了 slab cache 进行内存分配的整个链路实现，本文我们就来到了 slab cache 最后的一部分内容了，当申请的内存使用完毕之后，下面就该释放内存了。

![](images/v2-5477af55fd1996a3c9159be215edb657_1440w.jpg)

在接下来的内容中，笔者为大家介绍一下内核是如何将内存块释放回 slab cache 的。我们还是先从 slab cache 释放内存的内核 API 开始聊起~~~

![](images/v2-4e50df96b6fbef6f76a046a6a664af02_1440w.jpg)

image.png

内核提供了 [kmem_cache_free](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=kmem_cache_free&zhida_source=entity) 函数，用于将对象释放回其所属的 slab cache 中，参数 x 表示我们要释放的内存块（对象）的虚拟 [内存地址](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%9C%B0%E5%9D%80&zhida_source=entity) ，参数 s 指向内存块所属的 slab cache。
...
slab cache 针对内存的回收流程其实和我们在上篇文章 **[《深入理解 slab cache 内存分配全链路实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488152%26idx%3D1%26sn%3D7c65f8ee28e9cc14a86e9df92b6d2b93%26chksm%3Dce77d4dff9005dc9003a7df42b6560df13b581e45927bf3ef3e13cc24acc4eb78c2332bd1351%26token%3D1861909455%26lang%3Dzh_CN%23rd)** 中介绍的 slab cache 内存分配流程是相似的。

内存回收总体也是分为快速路径 fastpath 和慢速路径 slow path，在 do_slab_free 函数中内核会首先尝试 fastpath 的回收流程。

如果释放对象所在的 slab 刚好是 slab cache 在本地 cpu 缓存 [kmem_cache_cpu](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=kmem_cache_cpu&zhida_source=entity) ->page 缓存的 slab，那么内核就会直接将对象释放回缓存 slab 中。
...
> 在开始阅读本小节的内容之前，建议大家先回顾下 **[《细节拉满，80 张图带你一步一步推演 slab 内存池的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247487996%26idx%3D1%26sn%3D12f62cfcfcc89ffcbbd1b7e4fa286940%26chksm%3Dce77d7bbf9005ead3d4a953e470dba6b4708573a785e21f8f184f1c1fb6b3fed67a742b020d4%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文中的 ”8. slab 内存释放原理“ 小节。
...
1. 如果 slab 不在 kmem_cache_cpu->partial 链表中，并且该 slab 由于对象的释放刚好由一个 partial slab 变为了一个 empty slab，说明该 slab 并不是很活跃，内核会将该 slab 放入对应 [NUMA 节点](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=NUMA+%E8%8A%82%E7%82%B9&zhida_source=entity) 缓存 [kmem_cache_node](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=kmem_cache_node&zhida_source=entity) ->partial 链表中，刀枪入库，马放南山。
...
这一部分的逻辑比较简单，在 \_\_slab_free 内存释放流程的开始，内核不管三七二十一，首先会将对象直接释放回其所在的 slab 中。
...
但是我们查看 \_\_slab_free 的代码发现，内核并不是这样操作的，内核只是将原有 slab 的 counter 属性赋值给 new page，而原有 slab 中的 frozen，inuse 属性并没有赋值过去。
...
![](images/v2-ced5d0fb7013f3543ad0ccc3d81bb0ff_1440w.jpg)
...
page 结构中的 counters 是和 inuse，frozen 共用同一块内存的，内核在 \_\_slab_free 中将原有 slab 的 counters 属性赋值给 new.counters 的一瞬间，counters 所在的内存块也就赋值到 new page 的 union 结构中了。
...
我们都知道 slab 在内核中的 [数据结构](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) 用 struct page 中的相关结构体表示，slab 在 slab cache 架构中一般是由 kmem_cache_cpu->partial 链表和 kmem_cache_node->partial 链表来组织管理。
...
slab cache 的销毁过程刚刚好和 slab cache 的创建过程相反，笔者在 **[《从内核源码看 slab 内存池的创建初始化流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488095%26idx%3D1%26sn%3Dffc73f33cb03f0be7cf2ab1172fdc448%26chksm%3Dce77d418f9005d0e9d95f223f679198fd2e40377abdcaf2c636962994c3508bcd814f9929937%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 的内容中，通过一步一步的源码演示，最终勾勒出 slab cache 的完整架构：
...
3. 在 sys [文件系统](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F&zhida_source=entity) 中移除 `/sys/kernel/slab/<cacchename>` 节点相关信息。
...
![](images/v2-12606cb31f3876ec86ca5dc3b692e840_1440w.jpg)
...
在开始正式销毁 slab cache 之前，首先需要将 slab cache 的 [引用计数](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=%E5%BC%95%E7%94%A8%E8%AE%A1%E6%95%B0&zhida_source=entity) refcount 减 1。并需要判断 slab cache 是否还存在其他地方的引用。

slab cache 这里在其他地方存在引用的可能性，相关细节笔者在 **[《从内核源码看 slab 内存池的创建初始化流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488095%26idx%3D1%26sn%3Dffc73f33cb03f0be7cf2ab1172fdc448%26chksm%3Dce77d418f9005d0e9d95f223f679198fd2e40377abdcaf2c636962994c3508bcd814f9929937%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文中的 ”1. \_\_kmem_cache_alias“ 小节的内容中已经详细介绍过了。
...
随后会在 sys 文件系统中为复用 slab cache 起一个别名 alias 并创建一个 `/sys/kernel/slab/aliasname ` 目录，但是该目录下的文件需要 [软链接](https://zhida.zhihu.com/search?content_id=228647984&content_type=Article&match_order=1&q=%E8%BD%AF%E9%93%BE%E6%8E%A5&zhida_source=entity) 到原有 slab cache 在 sys 文件系统对应目录下的文件。这里的 aliasname 就是我们通过 kmem_cache_create 指定的 slab cache 名称。
...
![](images/v2-075745e558f58bbee62d67b26158599e_1440w.jpg)

image.png

在 slab 内存池创建出来之后，随后笔者又深入介绍了 slab 内存池如何分配内存块的相关源码实现，其中详细介绍了在多种不同场景下，内核如何处理内存块的分配。

![](images/v2-7b4d7bc6a2b476c970b668d963f88e55_1440w.jpg)
...
发布于 2023-05-26 10:13・广东[内存管理](https://www.zhihu.com/topic/19579205)[Linux 内核](https://www.zhihu.com/topic/19614193)[Linux](https://www.zhihu.com/topic/19554300)[Abaqus结构仿真双证班，保姆式小班教学，从0到1快速学会做仿真！](https://zhuanlan.zhihu.com/p/689280534)

[

2024结构仿真双证研修班招生简章为推动中国智造强国战略，仿真秀以“为行业选才育才”为宗旨，2022年正式成立仿真高研院开展仿真工程师职业培训。 截止目前 已经有超1500学员报名学习，...
