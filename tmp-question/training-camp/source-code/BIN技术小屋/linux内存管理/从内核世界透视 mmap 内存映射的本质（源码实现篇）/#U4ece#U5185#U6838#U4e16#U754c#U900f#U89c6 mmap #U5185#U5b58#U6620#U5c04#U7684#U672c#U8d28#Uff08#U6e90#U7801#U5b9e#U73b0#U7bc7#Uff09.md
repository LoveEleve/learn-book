...
通过上篇文章 **[《从内核世界透视 mmap 内存映射的本质（原理篇）》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488750%26idx%3D1%26sn%3D247a4603299e203793fac8b6c5e61071%26chksm%3Dce77d2a9f9005bbf3b024bc9f9192f2de63a70fd33db1113d9f9c0d8a1ced2099fbeb727a3d7%26token%3D1862605518%26lang%3Dzh_CN%23rd)** 的介绍，我们现在已经非常清楚了 mmap 背后的映射原理以及它的使用方法，其核心就是在进程虚拟内存空间中分配一段虚拟内存出来，然后将这段虚拟内存与磁盘文件映射起来，整个 mmap [系统调用](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) 就结束了。

而在 mmap 内存映射的整个过程中，最为核心且复杂烧脑的环节其实不是内存映射的逻辑，而是虚拟内存分配的整个流程。笔者曾在之前的文章 **[《深入理解 Linux 物理内存分配全链路实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247487111%26idx%3D1%26sn%3De57371f9c3e6910f4f4721aa0787e537%26chksm%3Dce77c8c0f90041d67b2d344d413a2573f3662a1a64a802b41d4618982fcbff1617d9a5da9f7b%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 中详细地为大家介绍了物理内存的分配过程，那么虚拟内存的分配过程又是什么样的呢？

本文我们将进入到内核源码实现中，来看一下虚拟内存分配的过程，在这个过程中，我们还可以亲眼看到前面介绍的 mmap 内存映射原理在内核中具体是如何实现的，下面我们就从 mmap 系统调用的入口处来开始本文的内容：

![](images/v2-7a19866093be9204b43b3942808c243d_1440w.jpg)
...
- mmap 在对文件进行大页映射的时候，映射文件必须是 hugetlbfs 中的文件， **flags 参数无需设置 MAP_HUGETLB** ， mmap 不能对普通文件进行大页映射，这种映射方式必须提前手动挂载 hugetlbfs [文件系统](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F&zhida_source=entity) 到指定路径下。映射长度需要与大页尺寸进行对齐。
...
而物理内存的分配工作需要延后到这段虚拟内存被 CPU 访问的时候，通过 [缺页中断](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E7%BC%BA%E9%A1%B5%E4%B8%AD%E6%96%AD&zhida_source=entity) 来进入内核，分配物理内存，并在页表中建立好映射关系。
...
紧接着就会调用 mm_populate，内核会在 mmap 刚刚映射出来的这段虚拟内存区域上，依次扫描这段 vma 中的每一个虚拟页，并对每一个虚拟页触发 [缺页异常](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E7%BC%BA%E9%A1%B5%E5%BC%82%E5%B8%B8&zhida_source=entity) ，从而为其立即分配物理内存。
...
mm_populate 函数的作用主要是在进程虚拟内存空间中，找出 `[ret , ret + populate]` 这段 [虚拟地址](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%9C%B0%E5%9D%80&zhida_source=entity) 范围内的所有 vma，并为每一个 vma 填充物理内存。
...
![](images/v2-a020d7adaeeac117242e8fbc379ed2a9_1440w.jpg)
...
除了这两个核心内容之外，do_mmap 函数还承担了对一些内存映射约束条件的检查，比如：内核规定一个进程虚拟内存空间内所能映射的虚拟内存区域 vma 是有数量限制的， [sysctl_max_map_count](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=sysctl_max_map_count&zhida_source=entity) 规定了进程虚拟内存空间所能包含 VMA 的最大个数，我们可以通过 ` /proc/sys/vm/max_map_count` 内核参数来调整 sysctl_max_map_count。

![](images/v2-f584e156ffccccfd4009866d705c6a57_1440w.jpg)
...
![](images/v2-27cc24fc571f2cd23c6467a990a07f40_1440w.jpg)
...
当我们使用 mmap 系统调用进行虚拟内存申请的时候，会受到内核 overcommit 策略的影响，内核会综合物理内存的 [总体容量](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E6%80%BB%E4%BD%93%E5%AE%B9%E9%87%8F&zhida_source=entity) 以及 swap 交换区的总体大小来决定是否允许本次虚拟内存用量的申请。mmap 申请过大的虚拟内存，内核会拒绝。
...
文件映射与匿名映射区的布局在 linux 内核中分为两种：一种是经典布局，另一种是新式布局，不同的 [体系结构](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E4%BD%93%E7%B3%BB%E7%BB%93%E6%9E%84&zhida_source=entity) 可以通过内核参数 `/proc/sys/vm/legacy_va_layout` 来指定具体采用哪种布局。 1 表示采用经典布局， 0 表示采用新式布局。

![](images/v2-6f9685da2d0c2cb53b1891c66fb222f5_1440w.jpg)

image.png

在经典布局下，文件映射与匿名映射区的地址增长方向是从低地址到高地址，也就是说映射区是从下往上增长，这也就导致了 mmap 在分配虚拟内存的时候需要从下往上搜索空闲 vma。

![](images/v2-a96dce9538eb43651d6d22afef2034ed_1440w.jpg)
...
这表明了文件映射与匿名映射区起始于进程虚拟内存空间开始的 1G 位置处，而映射区恰好位于整个进程虚拟内存空间的中间，其下方就是堆了，由于代码段， [数据段](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E6%AE%B5&zhida_source=entity) 的存在，可供堆进行扩展的空间是小于 1G 的，否则就会与映射区冲突了。
...
![](images/v2-f75c31d5e8d5844aff7abb4d279393fb_1440w.jpg)
...
由于栈变为有界的了，所以文件映射与匿名映射区可以在栈的下方立即开始，为确保栈与映射区不会冲突，它们中间还设置了 1M 大小的 [安全间隙](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E5%AE%89%E5%85%A8%E9%97%B4%E9%9A%99&zhida_source=entity) stack_guard_gap。

这样一来堆在进程地址空间中较低的地址处开始向上增长，而映射区位于进程空间较高的地址处向下增长，因此堆区和映射区在新式布局下都可以较好的扩展，直到耗尽剩余的虚拟内存区域。

### 4.2 内核具体如何对文件映射与匿名映射区进行布局

进程虚拟内存空间的创建以及初始化是由 load_elf_binary 函数负责的，当进程通过 fork() 系统调用创建出 [子进程](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E5%AD%90%E8%BF%9B%E7%A8%8B&zhida_source=entity) 之后，子进程可以通过前面介绍的 execve 系统调用加载并执行一个指定的 [二进制](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E4%BA%8C%E8%BF%9B%E5%88%B6&zhida_source=entity) 执行文件。

execve 函数会调用到 load_elf_binary，由 load_elf_binary 负责解析指定的 ELF 格式的二进制可执行文件，并将 [二进制文件](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E4%BA%8C%E8%BF%9B%E5%88%B6%E6%96%87%E4%BB%B6&zhida_source=entity) 中的.text,.data 映射到新进程的虚拟内存空间中的代码段，数据段，BSS 段中。
...
无论是经典布局下的 arch_get_unmapped_area，还是新布局下的 arch_get_unmapped_area_topdown 都会设置到 mm_struct->get_unmapped_area 这个 [函数指针](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E5%87%BD%E6%95%B0%E6%8C%87%E9%92%88&zhida_source=entity) 中，后续 mmap 会利用这个 get_unmapped_area 来在文件映射与匿名映射区中划分虚拟内存区域 vma。
...
如果我们开启了进程虚拟内存空间的随机化， [全局变量](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E5%85%A8%E5%B1%80%E5%8F%98%E9%87%8F&zhida_source=entity) randomize_va_space 就会为 1，进程的 flags 标志将会设置为 PF_RANDOMIZE，表示对进程地址空间进行随机化布局。
...
![](images/v2-c033c1c6530dfe3e84695a1c16794b7a_1440w.jpg)
...
![](images/v2-222e85e6cdae3693cc9f4c421e432ec5_1440w.jpg)
...
如果 mmap 进行的是共享匿名映射，由于共享匿名映射的本质其实是基于 tmpfs 的 [虚拟文件系统](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F&zhida_source=entity) 中的匿名文件进行的共享文件映射，所以这种情况下 get_unmapped_area 函数是需要基于 tmpfs 的虚拟文件系统的，在共享匿名映射的情况下 get_unmapped_area 指向 shmem_get_unmapped_area 函数。
...
![](images/v2-fa7c4a23c5e09e346a462bf009aaed39_1440w.jpg)
...
![](images/v2-8dd81edb8e3e731c7aaa9585cd4dda9a_1440w.jpg)
...
![](images/v2-9239b9816e81a267f6a4826a339cc08f_1440w.jpg)
...
2. 第二种情况是我们指定的 addr 恰好位于一个 vma 和另一个 vma 中间的地址间隙中，并且这个地址间隙刚好大于或者等于我们指定的映射长度 len。内核就可以将这个地址间隙映射起来。
   ![](images/v2-3bed4dd61d14cdce206ceaa6e39145ed_1440w.jpg)
...
![](images/v2-ebf665873c07d5d30e11fcbf4a6f4d25_1440w.jpg)
...
![](images/v2-e8663d0a507d51c6afbf6da3cb9b5c93_1440w.jpg)
...
根据指定地址 addr 在进程地址空间中查找第一个符合 `addr < vma->vm_end ` 条件 vma 的操作在 find_vma 函数中进行，内核为了高效地在进程地址空间中查找特定条件的 vma，会按照地址的增长方向将所有的 vma 组织在一颗 [红黑树](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E7%BA%A2%E9%BB%91%E6%A0%91&zhida_source=entity) mm_rb 中。
...
![](images/v2-1d6a1254e63b50c61235c65392e36e9b_1440w.jpg)
...
![](images/v2-dbc92045fbfa9bb804121de826e56652_1440w.jpg)
...
![](images/v2-867a73847597747fa01a4047637ce3df_1440w.jpg)
...
3. gap_start <= high_limit - length。
   ![](images/v2-e0876980a4c66b171990eee9bd24a038_1440w.jpg)
...
![](images/v2-f8a5080cd68d523a0192e33d28f9b5b4_1440w.jpg)
...
内核为了精细化的控制内存的开销，避免创建没有必要的 vma 结构，内核会本着能省则省的原则，在创建新的 vma 之前，按照最大程度合并的原则，内核会尝试看能不能将当前寻找出来的空闲虚拟内存区域 \[addr, addr + len\] 与其前一个 vma 以及后一个 vma 进行合并，然后重新调整合并后的 vma 相关属性，比如：vm_start, vm_end, vm_pgoff，以及涉及到相关 [数据结构](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) 的改变。这样一来，内核就不需要为这段空闲虚拟内存创建新的 vma 了。
...
![](images/v2-587bf81cfa12101c6811e14089d5420b_1440w.jpg)
...
![](images/v2-98e0757694f4c7b252d5e11b3c0866fe_1440w.jpg)
...
![](images/v2-fe261cdaa0c02d8a0d3c24feefab2ac7_1440w.jpg)
...
![](images/v2-a0c605d8b379ade7b4e86710cf13ce11_1440w.jpg)
...
除此之外，内核会在 CommitLimit 的基础上为进程预留一部分内存，用于在紧急情况下做一些恢复的操作，这部分预留的内存包括两种，一种是 sysctl_admin_reserve_kbytes，另一种是 sysctl_user_reserve_kbytes。它们的大小均可以在 `/proc/sys/vm` 目录下相应的 [配置文件](https://zhida.zhihu.com/search?content_id=234892380&content_type=Article&match_order=1&q=%E9%85%8D%E7%BD%AE%E6%96%87%E4%BB%B6&zhida_source=entity) 中进行调整，单位为 KB。

![](images/v2-ac9ac914de816054e445a066d70bb8a7_1440w.jpg)
...
![](images/v2-e851a9d78c66ff9f2451706dc28c8ddb_1440w.jpg)
...
7. 合并区域的 numa policy 必须是相同的。关于 numa policy 的介绍，感兴趣的同学可以查看笔者之前的文章 **[《一步一图带你深入理解 Linux 物理内存管理》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486879%26idx%3D1%26sn%3D0bcc59a306d59e5199a11d1ca5313743%26chksm%3Dce77cbd8f90042ce06f5086b1c976d1d2daa57bc5b768bac15f10ee3dc85874bbeddcd649d88%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 第 “3.2.1 NUMA 的内存分配策略” 小节的内容。
...
![](images/v2-9be9c4c82f59151373633f92739b4b2d_1440w.jpg)

image.png

如果我们在 mmap 系统调用参数 flags 中设置了 MAP_FIXED 标志，表示需要内核进行强制映射，在这种情况下，area 区域有可能会与 prev 区域和 next 区域有部分重合。

![](images/v2-dbcf813aa55a5f44e2c3024ea856e4e1_1440w.jpg)
...
![](images/v2-44a63ca61ef232f98bb3bdfa18658408_1440w.jpg)
...
![](images/v2-b9b29a2db0e989d800899663c3e0b092_1440w.jpg)
...
到现在为止，笔者通过两篇文章，一篇 **[原理](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488750%26idx%3D1%26sn%3D247a4603299e203793fac8b6c5e61071%26chksm%3Dce77d2a9f9005bbf3b024bc9f9192f2de63a70fd33db1113d9f9c0d8a1ced2099fbeb727a3d7%26token%3D1862605518%26lang%3Dzh_CN%23rd)** ，一篇源码，深入到内核世界中，将 mmap 内存映射的本质给大家呈现了出来，知识点比较密集且比较烧脑，因此笔者又画了一副 mmap 内存映射的整体思维导图方便大家回顾。

![](images/v2-a3ae9e719bc3f0c4c1d63ed7fca0ebf0_1440w.jpg)
...
发布于 2023-10-10 10:20・广东[操作系统](https://www.zhihu.com/topic/19552686)[Linux](https://www.zhihu.com/topic/19554300)[Linux 内核](https://www.zhihu.com/topic/19614193)[RAG学习全流程，存一下吧很难找全了](https://zhuanlan.zhihu.com/p/1985748049927111600)
