...
在经过上篇文章 **[《从内核源码看 slab 内存池的创建初始化流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488095%26idx%3D1%26sn%3Dffc73f33cb03f0be7cf2ab1172fdc448%26chksm%3Dce77d418f9005d0e9d95f223f679198fd2e40377abdcaf2c636962994c3508bcd814f9929937%26token%3D877598148%26lang%3Dzh_CN%23rd)** 的介绍之后，我们最终得到下面这幅 [slab cache](https://zhida.zhihu.com/search?content_id=227438839&content_type=Article&match_order=1&q=slab+cache&zhida_source=entity) 的完整架构图：
...
![](images/v2-9755c59e5e9f74a0acca9acb38bbfd43_1440w.jpg)

## 1\. slab cache 如何分配内存

当我们使用 fork() [系统调用](https://zhida.zhihu.com/search?content_id=227438839&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) 创建进程的时候，内核需要为进程创建 task_struct 结构，struct task_struct 是内核中的核心数据结构，当然也会有专属的 slab cache 来进行管理，task_struct 专属的 slab cache 为 [task_struct_cachep](https://zhida.zhihu.com/search?content_id=227438839&content_type=Article&match_order=1&q=task_struct_cachep&zhida_source=entity) 。
...
内核中通过 kmem_cache_alloc_node 函数要求 slab cache 从指定的 [NUMA 节点](https://zhida.zhihu.com/search?content_id=227438839&content_type=Article&match_order=1&q=NUMA+%E8%8A%82%E7%82%B9&zhida_source=entity) 中分配对象。
...
![](images/v2-683d71fdb9ca1af84ca8a74d32343746_1440w.jpg)
...
如果当前 cpu 缓存 slab 是空的（没有空闲对象可供分配）或者该 slab 所在的 NUMA 节点并不是我们指定的。那么就会通过 \_\_slab_alloc 进入到慢速分配路径 slowpath 中。
...
- 另一个是 kmem_cache_cpu->freelist，特指 slab 被 slab cache 的本地 cpu 缓存之后，slab 中的空闲对象链表。这里可以理解为 slab 中被 cpu 缓存的空闲对象。当 slab 被提升为 cpu 缓存之后，page->freeelist 直接赋值给 kmem_cache_cpu->freelist，然后 page->freeelist 置空。slab->frozen 设置为 1，表示 slab 被冻结在当前 cpu 的本地缓存中。
  ![](images/v2-8e1c9ff13930d5fef7c5324ce8036dd6_1440w.jpg)
...
![](images/v2-673f6595b4f7974c4069fd38a4e5c254_1440w.jpg)
...
明白了这些，让我们再次回到 \_\_\_slab_alloc 函数的开始处，首先内核会在 slab cache 的本地 cpu 缓存 kmem_cache_cpu->freelist 中查找是否有空闲对象，如果这里没有，内核会继续到 page->freelist 中查看是否有其他 cpu 释放的空闲对象
...
下面内核正式进入到 slowpath 开始分配对象，首先内核会到本地 cpu 缓存的 partial 列表中去查看是否有一个 slab 可以分配对象。这里内核会从 partial 列表中的 [头结点](https://zhida.zhihu.com/search?content_id=227438839&content_type=Article&match_order=1&q=%E5%A4%B4%E7%BB%93%E7%82%B9&zhida_source=entity) 开始遍历直到找到一个可以满足分配的 slab 出来。
...
通过 oo 的高 16 位获取 slab 需要的物理内存页数，然后调用 alloc_pages 或者 \_\_alloc_pages_node 向伙伴系统申请。
...
![](images/v2-d7d20a6ebc3f7a5e9f3653635f33305b_1440w.jpg)
...
> slab 对象详细的内存布局介绍，可以回看下笔者之前的文章 **[?《细节拉满，80 张图带你一步一步推演 slab 内存池的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247487996%26idx%3D1%26sn%3D12f62cfcfcc89ffcbbd1b7e4fa286940%26chksm%3Dce77d7bbf9005ead3d4a953e470dba6b4708573a785e21f8f184f1c1fb6b3fed67a742b020d4%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 中的 “ 5. 从一个简单的内存页开始聊 slab ” 小节。
...
发布于 2023-05-05 10:14・广东[内存管理](https://www.zhihu.com/topic/19579205)[Linux 内核](https://www.zhihu.com/topic/19614193)[Linux](https://www.zhihu.com/topic/19554300)[在没有任何基础的情况下，能入行FPGA吗？](https://www.zhihu.com/question/613238896/answer/3248410617)

[

不能，需要先学习，学会就已经入行FPGA了，学习FPGA，首先得学会Verilog HDL，先学习Verilog HDL再学VHDL，先入门（Verilog HDL）再锦上添花（VHDL）。接下...

](https://www.zhihu.com/question/613238896/answer/3248410617)
