笔者之前在自己的专栏 **[《聊聊 Linux 内核》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/mp/appmsgalbum%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26action%3Dgetalbum%26album_id%3D2559805446807928833%26scene%3D173%26from_msgid%3D2247486879%26from_itemidx%3D1%26count%3D3%26nolastread%3D1%23wechat_redirect)** 里通过大量的篇幅写了一个系列关于内存管理相关的文章，在这个系列文章中，笔者分别通过虚拟内存管理和物理内存管理两个角度算是把 Linux 内存 [管理子系统](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%AE%A1%E7%90%86%E5%AD%90%E7%B3%BB%E7%BB%9F&zhida_source=entity) 的全貌给大家呈现了出来。
...
经过 **[《深入理解 Linux 虚拟内存管理》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486732%26idx%3D1%26sn%3D435d5e834e9751036c96384f6965b328%26chksm%3Dce77cb4bf900425d33d2adfa632a4684cf7a63beece166c1ffedc4fdacb807c9413e8c73f298%26token%3D1931867638%26lang%3Dzh_CN%23rd)** 一文的介绍我们知道，虚拟内存其实是 CPU 和 [操作系统](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F&zhida_source=entity) 使用的一个 [障眼法](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E9%9A%9C%E7%9C%BC%E6%B3%95&zhida_source=entity) ，联手给进程编织了一个假象，让进程误以为自己独占了全部的内存空间：

- 在 32 位系统中，进程以为自己独占了 3G 的内存空间。
  ![](images/v2-d5b2370aad10c68e3df8f1c41a2b32bf_1440w.jpg)

32位系统中虚拟内存空间整体布局.png

- 在 64 位系统中，进程以为自己独占了 128T 的内存空间。
  ![](images/v2-598ecd334bdd10b7ff8e6c87420bc06f_1440w.jpg)

64位系统中虚拟内存空间整体布局.png

这么做的好处是，操作系统为每个进程营造出一片独立的 [虚拟地址空间](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%9C%B0%E5%9D%80%E7%A9%BA%E9%97%B4&zhida_source=entity) ，使得进程与进程之间相互隔离，互不干扰的，解决了多进程同时运行时产生的 [内存地址](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%9C%B0%E5%9D%80&zhida_source=entity) 冲突问题。

![](images/v2-2966519d0c10562616445863d2f1a741_1440w.jpg)

image.png

同时虚拟内存还提供了系统安全方面的保障，会对进程访问内存的行为进行相关的安全权限检查，保障了系统的稳定性和安全性。比如：

- 有些物理内存页只允许内核来访问，进程在 [用户态](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%94%A8%E6%88%B7%E6%80%81&zhida_source=entity) 的时候是无法访问的。
- 虚拟内存中保存了访问其 [映射](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E6%98%A0%E5%B0%84&zhida_source=entity) 的物理内存相关的权限，进程只能执行规定权限范围内的访存操作。比如，上面虚拟内存空间里代码段的权限是可读，可执行，但是不可写。 [数据段](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E6%AE%B5&zhida_source=entity) 具有可读可写的权限但是不可执行。堆则具有可读可写，可执行的权限（Java 中的 [字节码](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%AD%97%E8%8A%82%E7%A0%81&zhida_source=entity) 存储在堆中，所以需要可执行权限），栈一般是可读可写的权限，一般很少有 [可执行权限](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=2&q=%E5%8F%AF%E6%89%A7%E8%A1%8C%E6%9D%83%E9%99%90&zhida_source=entity) 。而 [文件映射](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E6%96%87%E4%BB%B6%E6%98%A0%E5%B0%84&zhida_source=entity) 与匿名映射区存放了 [共享链接库](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%85%B1%E4%BA%AB%E9%93%BE%E6%8E%A5%E5%BA%93&zhida_source=entity) ，所以也需要可执行的权限。
  ![](images/v2-7e5ce667be185bdf3ccc86a312eea6db_1440w.jpg)

image.png

但是当程序运行起来之后，程序中所需要的数据本质上还是保存在物理内存中的，无论操作系统对虚拟内存设计的多么精彩，最终虚拟内存空间中每一个 [虚拟内存地址](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98%E5%9C%B0%E5%9D%80&zhida_source=entity) 都是要映射到 [物理内存空间](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%86%85%E5%AD%98%E7%A9%BA%E9%97%B4&zhida_source=entity) 的中某一个特定 [物理内存地址](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%86%85%E5%AD%98%E5%9C%B0%E5%9D%80&zhida_source=entity) 上的。

> [进程虚拟内存空间](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E8%BF%9B%E7%A8%8B%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98%E7%A9%BA%E9%97%B4&zhida_source=entity) 中的每一个字节都有与其对应的虚拟内存地址，同样物理内存空间中每一个字节都有与其对应的物理内存地址。

下面我们就来把舞台上的桌布拿走，一起到内核中探秘一下 CPU 和操作系统联手编织的这个障眼戏法是如何玩转起来的~~~~

![](images/v2-51f72dbf1f61f1b4ac5452f42337f86d_1440w.jpg)

本文概览.png

## 1\. 虚拟内存如何与物理内存映射起来

在 **[《深入理解 Linux 物理内存管理》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486879%26idx%3D1%26sn%3D0bcc59a306d59e5199a11d1ca5313743%26chksm%3Dce77cbd8f90042ce06f5086b1c976d1d2daa57bc5b768bac15f10ee3dc85874bbeddcd649d88%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文中，笔者在介绍物理内存管理的时候曾提到，内核会将整个物理内存空间划分为一页一页大小相同的的 [内存块](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%9D%97&zhida_source=entity) ，每个内存 [块大小](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%9D%97%E5%A4%A7%E5%B0%8F&zhida_source=entity) 为 4K，称为一个 [物理内存页](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=2&q=%E7%89%A9%E7%90%86%E5%86%85%E5%AD%98%E9%A1%B5&zhida_source=entity) 。

一页大小的内存块在内核中用 struct page 结构体来进行管理，struct page 中封装了每页内存块的状态信息，比如： [组织结构](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%BB%84%E7%BB%87%E7%BB%93%E6%9E%84&zhida_source=entity) ，使用信息，统计信息，以及与其他内核结构的关联映射信息等。
...
![](images/v2-487583572f113f7d02304ab8992dced9_1440w.jpg)
...
![](images/v2-e6162900419f36180125682eb2eaa105_1440w.jpg)

image.png

如上图所示，在内存映射的场景中， [虚拟内存页](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98%E9%A1%B5&zhida_source=entity) 的类型总共分为以下三种：
...
2. 第二种就是图中紫色方框里标注的 **已分配未映射页面** ，我们在进程中可以通过 [动态链接库](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%8A%A8%E6%80%81%E9%93%BE%E6%8E%A5%E5%BA%93&zhida_source=entity) glic 中的 [malloc](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=malloc&zhida_source=entity) 接口或者直接通过 [系统调用](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) mmap 向内核申请虚拟内存，申请到的虚拟内存页此时就变为了已分配的页面。但此时的虚拟内存页只是虚拟内存，其背后并没有与物理内存映射起来，所以称为已分配未映射页面。
3. 第三种是图中绿色方框里标注的 **正常页面** ，当进程开始读写这些已分配未映射的虚拟内存页时，在 CPU 中用于地址翻译的硬件 [MMU](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=MMU&zhida_source=entity) 会产生一个 [缺页中断](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%BC%BA%E9%A1%B5%E4%B8%AD%E6%96%AD&zhida_source=entity) ，随后内核会为其分配相应的物理内存页面，并将虚拟内存页与物理内存页映射起来。此时这些已分配未映射的虚拟内存页就变为了 **正常页面** 。从此以后，进程就可以正常读写这些虚拟内存页了。
...
谈到映射，我们自然会想到 Map 这个 [数据结构](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) ，那么虚拟内存与物理内存之间的映射关系如果用 Map 来表达的话，就是如下形式：
...
![](images/v2-b227f79eaf6124b0ca9715e18954fc8f_1440w.jpg)

image.png

`Map<虚拟内存，物理内存>` 的映射关系在内核中是被一个叫做 [页表](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E9%A1%B5%E8%A1%A8&zhida_source=entity) 的东西来管理的，页表除了管理虚拟内存与物理内存之间的映射关系之外，还会有一些 [访问权限](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E8%AE%BF%E9%97%AE%E6%9D%83%E9%99%90&zhida_source=entity) 的管理，来控制进程对物理内存的访问权限。
...
而内核会在页表中划分出来一个个大小相等的小内存块，这些小内存块我们称之为页表项 [PTE](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=PTE&zhida_source=entity) （Page Table Entry），正是这个 PTE 保存了进程虚拟内存空间中的虚拟页与物理内存页的映射关系，以及控制物理内存访问的相关权限位。
...
![](images/v2-5c98116c76895a01be38f048dcad40af_1440w.jpg)

image.png

而进程虚拟内存空间中的每一个字节都有一个虚拟内存地址来表示，格式为： `页表内偏移 + 物理内存页内偏移`

![](images/v2-41dc4e66911ba6282b0c07615374e81d_1440w.jpg)
...
进程的虚拟内存空间在内核中是用 [struct mm_struct](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=struct+mm_struct&zhida_source=entity) 结构来描述的，每个进程都有自己独立的虚拟内存空间，而进程的虚拟内存到物理内存的映射也是独立的，为了保证每个进程里内存映射的独立进行，所以每个进程都会有独立的页表，而页表的起始地址就存放在 struct mm_struct 结构中的 [pgd](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=pgd&zhida_source=entity) 属性中。
...
很显然这个设置的时机是在进程被创建出来的时候，当我们使用 fork 系统调用创建进程的时候，内核在 \_do_fork 函数中会通过 copy_process 将 [父进程](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%88%B6%E8%BF%9B%E7%A8%8B&zhida_source=entity) 的所有资源拷贝到 [子进程](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%AD%90%E8%BF%9B%E7%A8%8B&zhida_source=entity) 中，这其中也包括父进程的虚拟内存空间。
...
1. 进程虚拟内存空间的切换。
2. [寄存器](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%AF%84%E5%AD%98%E5%99%A8&zhida_source=entity) 以及进程栈的切换。
...
和本小节主题相关的是 switch_mm_irqs_off 函数，它主要负责对进程虚拟内存空间进行切换，其中就包括了调用 load_new_mm_cr3 函数将进程顶级页表起始地址 mm_struct-> pgd 中的虚拟内存地址通过 `__sme_pa 宏` 转换为物理内存地址，并将 pgd 的物理内存地址加载到 [cr3 寄存器](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=cr3+%E5%AF%84%E5%AD%98%E5%99%A8&zhida_source=entity) 中。
...
![](images/v2-33a6742b658b094801b21b965d586b9a_1440w.jpg)
...
通过前边文章 **[《深入理解 Linux 虚拟内存管理》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486732%26idx%3D1%26sn%3D435d5e834e9751036c96384f6965b328%26chksm%3Dce77cb4bf900425d33d2adfa632a4684cf7a63beece166c1ffedc4fdacb807c9413e8c73f298%26token%3D1931867638%26lang%3Dzh_CN%23rd)** 的介绍，我们知道，进程的整个虚拟内存空间分为两个部分，一个是用户态虚拟内存空间，一个是内核态虚拟内存空间。
...
![](images/v2-f709627a735ef3785b72d04e24d32fee_1440w.jpg)
...
![](images/v2-73b9233c38c87d097f035c1f609eb846_1440w.jpg)
...
但程序的 [局部性原理](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%B1%80%E9%83%A8%E6%80%A7%E5%8E%9F%E7%90%86&zhida_source=entity) 告诉我们，进程在运行之后，对于内存的访问不会一下子就要访问全部的内存，相反进程对于内存的访问会表现出明显的倾向性，更加倾向于访问最近访问过的数据以及热点数据附近的数据。

> 程序局部性原理表现为：时间局部性和空间局部性。时间局部性是指如果程序中的某条指令一旦执行，则不久之后该指令可能再次被执行；如果某块数据被访问，则不久之后该数据可能再次被访问。空间局部性是指一旦程序访问了某个 [存储单元](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%AD%98%E5%82%A8%E5%8D%95%E5%85%83&zhida_source=entity) ，则不久之后，其附近的存储单元也将被访问。
...
2. 在 32 位系统中，一个 PTE 占用 4 个字节，可以映射 4K 的物理内存，一张页表本身占用 4K 的物理内存，可以映射 4M 的物理内存。
   ![](images/v2-a9e3dd94ead42b8338e182d3f86c26d1_1440w.jpg)
...
![](images/v2-348572644ff7360a68b5ff904f36f334_1440w.jpg)
...
![](images/v2-356c13345bc187ccee23efbecf3ecb54_1440w.jpg)
...
![](images/v2-792c7b250980fffe322be221503ade0e_1440w.jpg)
...
![](images/v2-d01b3b5847dae46e5435dd0db2e4d306_1440w.jpg)
...
![](images/v2-15f1036b9c0bed7ca55d72067725a03f_1440w.jpg)

image.png

当进程需要访问下一个 4M 的物理内存时，这时候第一个一级页表已经映射满了，那就需要再创建第二张页表用来映射下一个 4M 的物理内存，当然了，第二张页表依然需要索引在页目录表的 PDE 中。

![](images/v2-5488cc2ebc2ba55f8e1739ba5bdfd6b5_1440w.jpg)
...
除此之外二级页表体系还有一个优势，就是当内存紧张的时候，那些不经常使用的一级页表可以被 swap out 到磁盘中，当进程再次访问到该页表映射的物理内存时，内核在将页表从磁盘中 swap in 到内存中。当然了，顶级页表（这里指页目录表）必须是 [常驻内存](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%B8%B8%E9%A9%BB%E5%86%85%E5%AD%98&zhida_source=entity) 的，不允许 swap 。

![](images/v2-841c737e00966a2c4f0371f05dc89e0b_1440w.jpg)
...
![](images/v2-4143fcf9f6188fbaf4fb1896c411f9a4_1440w.jpg)

image.png

1. cr3 寄存器保存了顶级页表的起始物理内存地址，顶级页表随着进程的创建而创建，保存在进程 mm_struct->pgd，当进程被 CPU 调度的时候，会伴随着进程 [上下文切换](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E4%B8%8A%E4%B8%8B%E6%96%87%E5%88%87%E6%8D%A2&zhida_source=entity) ，其中就会将 mm_struct->pgd 转换为物理内存地址并加载到 cr3 寄存器中。
...
1. 无论在几级页表体系下，进程虚拟内存空间中的虚拟内存地址格式在设计上总共分为两大部分，一部分是用来定位虚拟内存页在页表中对应的 PTE 偏移，另一部分是用来定位物理内存页中要访问的具体字节。
   ![](images/v2-4ceb4565542ee5924dc2b54e1993e258_1440w.jpg)
...
![](images/v2-1f0d4ceb2048844bc7dd011d88b93bd2_1440w.jpg)
...
![](images/v2-cfe355aff5f11e4a4f091eb5fa3a5882_1440w.jpg)
...
![](images/v2-fa09b1cec97919b2d9b6002040c9acf3_1440w.jpg)
...
![](images/v2-9ed968bca31deabb2e1b529f90e3ecec_1440w.jpg)
...
![](images/v2-ccb84e3e81756c71dda4b9fc79fd6086_1440w.jpg)
...
比如，父进程通过 fork 系统调用创建子进程之后，父子进程的虚拟内存空间完全是一模一样的，包括父子进程的页表内容都是一样的，父子进程页表中的 PTE 均指向同一物理内存页面，此时内核会将父子进程页表中的 PTE 均改为只读的，并将父子进程共同映射的这个物理页面 [引用计数](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%BC%95%E7%94%A8%E8%AE%A1%E6%95%B0&zhida_source=entity) + 1。
...
`A(5)` 表示 PTE 指向的这个物理内存页最近是否被访问过，1 表示最近被访问过（读或者写访问都会设置为 1），0 表示没有。该 bit 位被硬件 MMU 设置，由操作系统重置。内核会经常检查该 [比特位](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E6%AF%94%E7%89%B9%E4%BD%8D&zhida_source=entity) ，以确定该物理内存页的活跃程度，不经常使用的内存页，很可能就会被内核 swap out 出去。

`D(6)` 主要针对文件页使用，当 PTE 指向的物理内存页是一个文件页时，进程对这个文件页写入了新的数据，这时文件页就变成了脏页，对应的 PTE 中 D 比特位会被设置为 1，表示文件页中的内容与其背后对应磁盘中的文件内容不同步了。关于脏页的详细描述，可以回看下笔者之前的这篇文章 **[?《从 Linux 内核角度探秘 JDK NIO 文件读写本质》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486623%26idx%3D1%26sn%3D0cafed9e89b60d678d8c88dc7689abda%26chksm%3Dce77cad8f90043ceaaca732aaaa7cb692c1d23eeb6c07de84f0ad690ab92d758945807239cee%26token%3D1931867638%26lang%3Dzh_CN%23rd)** 。
...
![](images/v2-52735be79c761b917bc5e312625dd153_1440w.jpg)
...
![](images/v2-4abaf661bbe88b29845f193a5873e095_1440w.jpg)
...
![](images/v2-5781a3543e133f2654ecb63c2ab710e7_1440w.jpg)
...
我们可以通过如下 [位运算](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E4%BD%8D%E8%BF%90%E7%AE%97&zhida_source=entity) ，从 PDE 中提取 PS 比特位来确定该 PDE 指向的是一级页表还是 4M 大页。
...
![](images/v2-fa82def9b524ae239d9c6b1e6bae9d35_1440w.jpg)
...
![](images/v2-e512c318d76abce0675551b8b0f02461_1440w.jpg)
...
![](images/v2-733cd5a76b335fc6633382c00709c6fe_1440w.jpg)
...
![](images/v2-9c36318aea2dac9d5c29d1fdea6b21d3_1440w.jpg)
...
> `PTRS_PER_PUD - 1` 转换为 [二进制](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E4%BA%8C%E8%BF%9B%E5%88%B6&zhida_source=entity) 是 9 个 1，用来截取最低的 9 个比特位。
...
![](images/v2-a6c233cba12ff27aceb123ba822b62ee_1440w.jpg)
...
4. 后面的寻址流程就和二级页表一样了，pmd_t 指向具体页表的起始内存地址，通过 pte_index 截取虚拟内存地址在 `一级页表中的 PTE 偏移` ，最终定位到一个具体的 PTE 中，PTE 指向的正是虚拟内存 [地址映射](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E5%9C%B0%E5%9D%80%E6%98%A0%E5%B0%84&zhida_source=entity) 的物理内存页面，然后通过虚拟内存地址中的低 12 位（物理内存页内偏移），最终确定到一个具体的物理字节上。
...
![](images/v2-eecbe5b4b9d2b9b0ff6f305a47ab8632_1440w.jpg)

image.png

当 64 位 PDE 的 PS(7) 比特位为 1 时，这些页目录项 PDE 就被当做了一个特殊的 ”PTE“ 对待了，因此 PDE 中的比特位布局又就变成了 64 位 PTE 中的样子了。

![](images/v2-72fa2d8371c08b853d53dea4a9319acb_1440w.jpg)
...
![](images/v2-8113b35522053988f8bbdc287a972980_1440w.jpg)
...
同样的道理，在 MMU 经过层层的页 [目录遍历](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%9B%AE%E5%BD%95%E9%81%8D%E5%8E%86&zhida_source=entity) 之后，终于定位到了一级页表中的 PTE，MMU 也是先会从 CPU 高速缓存中去获取 PTE，如果 PTE 不在高速缓存中，MMU 才会去内存中去获取。获取到 PTE 之后，MMU 就得到了虚拟内存地址背后映射的物理内存地址了。

![](images/v2-62fdb17cffcc57d5d7ee72cd1b1d35de_1440w.jpg)
...
![](images/v2-05d2b4816ad98eab55bf1293c7d95d66_1440w.jpg)
...
![](images/v2-ab9967a80cc3cec16b4ba9e74b28e305_1440w.jpg)

image.png

- 当 MMU 获取到最终的物理内存地址，首先会根据物理内存地址到 CPU 高速缓存中去查找数据，如果 cache hit，整个访存操作快速结束。
- 如果 cache miss，那么 MMU 会将物理内存地址放到 [系统总线](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E6%80%BB%E7%BA%BF&zhida_source=entity) 上传输，随后 IO bridge 会将系统总线上传输的地址信号传递到存储总线上。
- 内存中的存储控制器感受到存储总线上的地址信号之后，会将物理内存地址从存储总线上读取出来。并根据物理内存地址定位到具体的存储器模块，随后解析物理内存地址从 DRAM 芯片中取出对应物理内存地址里的数据。
  ![](images/v2-e25a748679377a3bedc35441b31889bd_1440w.jpg)

存储器模块.png

> 关于 DRAM 芯片的具体访问细节，感兴趣的读者朋友可以回看下笔者之前文章 **[?《一步一图带你深入理解 Linux 虚拟内存管理》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486732%26idx%3D1%26sn%3D435d5e834e9751036c96384f6965b328%26chksm%3Dce77cb4bf900425d33d2adfa632a4684cf7a63beece166c1ffedc4fdacb807c9413e8c73f298%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 的 ”8. 到底什么是物理内存地址“ 小节。

- 存储控制器将读取到的数据放到存储总线传输上，随后 IO bridge 将存储总线上的 [数据信号](https://zhida.zhihu.com/search?content_id=231478167&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E4%BF%A1%E5%8F%B7&zhida_source=entity) 转换为系统总线上的数据信号，然后继续沿着系统总线传递。
...
发布于 2023-07-21 16:24・广东[Linux](https://www.zhihu.com/topic/19554300)[Linux 内核](https://www.zhihu.com/topic/19614193)[内存管理](https://www.zhihu.com/topic/19579205)[大厂！RAG知识库问答系统架构搭建（0-1）](https://zhuanlan.zhihu.com/p/1985752467212895306)
