![](images/v2-e7c8a8ebe1a2ff6c7b62e604d650423e_1440w.jpg)

本文是笔者 slab 系列的最后一篇文章，为了方便大家快速检索，先将相关的文章列举出来：

- **[《细节拉满，80 张图带你一步一步推演 slab 内存池的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488319%26idx%3D1%26sn%3D062b7b4e9521bdc6e7050f78229a6cd4%26chksm%3Dce77d578f9005c6e2beb3d60a147c2927542c7e5ebd281e6af2f63d70ffde79322ca634142cb%26token%3D1246371462%26lang%3Dzh_CN%23rd)**
- **[《从内核源码看 slab 内存池的创建初始化流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488095%26idx%3D1%26sn%3Dffc73f33cb03f0be7cf2ab1172fdc448%26chksm%3Dce77d418f9005d0e9d95f223f679198fd2e40377abdcaf2c636962994c3508bcd814f9929937%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)**
- **[《深入理解 slab cache 内存分配全链路实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488152%26idx%3D1%26sn%3D7c65f8ee28e9cc14a86e9df92b6d2b93%26chksm%3Dce77d4dff9005dc9003a7df42b6560df13b581e45927bf3ef3e13cc24acc4eb78c2332bd1351%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)**
- **[《深度解析 slab 内存池回收内存以及销毁全流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488210%26idx%3D1%26sn%3D61fb6bb2dc19ee3ecf889de0702642d7%26chksm%3Dce77d495f9005d837013fa698914479e5d805040224c2110ba1b8291fb8508b5a6b477f95e09%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)**

在之前的这四篇文章中，笔者详细的为大家介绍了 slab 内存池的整体架构演化过程，随后基于这个演化过程，介绍了整个 slab alloactor 体系的创建，内存分配，内存释放以及销毁等相关复杂流程在内核中的实现。

我们知道 slab 内存池是专门为了应对内核中关于小内存分配需求而应运而生的，内核会为每一个核心 [数据结构](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) 创建一个专属的 slab 内存池，专门用于内核核心对象频繁分配和释放的场景。比如，内核中的 task_struct 结构，mm_struct 结构，struct page 结构，struct file 结构，socket 结构等等，在内核中都有一个属于自己的专属 slab 内存池。

而之前介绍的这些都属于专有的 slab 内存池，slab 在向伙伴系统申请若干 [物理内存页](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%86%85%E5%AD%98%E9%A1%B5&zhida_source=entity) page 之后，内核会按照需要被池化的专有数据结构在内存中的布局 size，从这些物理内存页中划分出多个大小相同的 [内存块](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%9D%97&zhida_source=entity) 出来，然后将这些划分出来的内存块统一交给其所属的 slab 内存池管理。每个内存块用来专门存储特定结构的内核对象，不能用作其他用途。

![](images/v2-793225196c0d8f0d7856c49f01041c0d_1440w.jpg)

image.png

内核中除了上述这些专有内存的分配需求之外，其实更多的是通用小内存的分配需求，比如说，内核会申请一些 8 字节，16 字节，32 字节等特定尺寸的通用内存块，内核并不会限制这些通用内存块的用途，可以拿它们来存储任何信息。

内核为了应对这些通用小内存的频繁分配释放需求，于是本文的主题 —— [kmalloc 内存池](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=kmalloc+%E5%86%85%E5%AD%98%E6%B1%A0&zhida_source=entity) 体系就应用而生了，在内核启动初始化的时候，通过 [kmem_cache_create](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=kmem_cache_create&zhida_source=entity) 接口函数预先创建多个特定尺寸的 slab cache 出来，用以应对不同尺寸的通用内存块的申请。
...
我们可以通过 kmem_cache_create 函数中的 size 参数来指定要创建的通用内存块尺寸，相关的创建流程细节，感兴趣的同学可以回看下这篇文章 **[《从内核源码看 slab 内存池的创建初始化流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488095%26idx%3D1%26sn%3Dffc73f33cb03f0be7cf2ab1172fdc448%26chksm%3Dce77d418f9005d0e9d95f223f679198fd2e40377abdcaf2c636962994c3508bcd814f9929937%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 。

kmalloc 内存池体系的底层基石是基于 slab alloactor 体系构建的，其本质其实就是各种不同尺寸的通用 slab cache。

![](images/v2-083cb5717817de7d4aaebb3187cfc113_1440w.jpg)

image.png

我们可以通过 `cat /proc/slabinfo` 命令来查看系统中不同尺寸的通用 slab cache：

![](images/v2-49f03c0c9628f015ba52518f50126445_1440w.jpg)
...
从 kmalloc_info\[\] 数组中我们可以看出，kmalloc 内存池 [体系理论](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E4%BD%93%E7%B3%BB%E7%90%86%E8%AE%BA&zhida_source=entity) 上最大可以支持 64M 尺寸大小的通用内存池。
...
其中 `size_index[24]` 数组中每个元素后面跟的注释部分为内核要申请的 [字节数](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E5%AD%97%E8%8A%82%E6%95%B0&zhida_source=entity) ， `size_index[24]` 数组中每个元素表示最佳合适尺寸的通用 slab cache 在 kmalloc_info\[\] 数组中的索引。
...
![](images/v2-e3387b5bac130712fae94a13c1577ad4_1440w.jpg)
...
我们看到 kmalloc_info\[\] 数组中定义的内存块尺寸非常的多，但实际上 kmalloc 体系所支持的内存块尺寸与 slab allocator 体系的实现有关，在 Linux 内核中 slab allocator 体系的实现分为三种：slab 实现， [slub 实现](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=slub+%E5%AE%9E%E7%8E%B0&zhida_source=entity) ，slob 实现。

而在被大规模运用的服务器 Linux [操作系统](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F&zhida_source=entity) 中，slab allocator 体系采用的是 slub 实现，所以本文我们还是以 slub 实现来讨论。
...
![](images/v2-ec41dd90ddfd5762042d67005dc3b990_1440w.jpg)

image.png

好了，现在 kmalloc 体系中的内存块尺寸我们已经划分好了，那么 kmalloc 体系中的这些不同尺寸的内存块究竟来自于哪些物理内存区域呢 ？

笔者在 **[《一步一图带你深入理解 Linux 物理内存管理》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486879%26idx%3D1%26sn%3D0bcc59a306d59e5199a11d1ca5313743%26chksm%3Dce77cbd8f90042ce06f5086b1c976d1d2daa57bc5b768bac15f10ee3dc85874bbeddcd649d88%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文中的 “4.3 NUMA 节点物理内存区域的划分” 小节中曾介绍到，内核会根据各个物理内存区域的功能不同，将 [NUMA 节点](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=NUMA+%E8%8A%82%E7%82%B9&zhida_source=entity) 内的物理内存划分为以下几个物理内存区域：
...
**而 kmalloc 内存池中的内存来自于上面的 ZONE_DMA 和 ZONE_NORMAL 物理内存区域** ，也就是内核 [虚拟内存](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98&zhida_source=entity) 空间中的直接映射区域。

![](images/v2-a89cd2b237f5f43b15609682c467eb83_1440w.jpg)
...
![](images/v2-9bc7038aded3003edc8eaeda3786312c_1440w.jpg)

image.png

上图中所展示的 kmalloc 内存池整体架构体系，内核将其定义在一个 kmalloc_caches [二维数组](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E4%BA%8C%E7%BB%B4%E6%95%B0%E7%BB%84&zhida_source=entity) 中，位于文件： `/include/linux/slab.h` 中。
...
- 第 [一维数组](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E4%B8%80%E7%BB%B4%E6%95%B0%E7%BB%84&zhida_source=entity) 用于表示 kmalloc 内存池中的内存来源于哪些物理内存区域中，也就是前边介绍的 `enum kmalloc_cache_type` 。
...
由于 kmalloc 体系底层依赖的是 slab allocator 体系，所以 kmalloc 体系的创建是在 slab allocator 体系创建之后进行的，关于 slab allocator 体系创建的详细内容笔者已经在 **[《从内核源码看 slab 内存池的创建初始化流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488095%26idx%3D1%26sn%3Dffc73f33cb03f0be7cf2ab1172fdc448%26chksm%3Dce77d418f9005d0e9d95f223f679198fd2e40377abdcaf2c636962994c3508bcd814f9929937%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文的 “12. 内核第一个 slab cache 是如何被创建出来的” 小节介绍过了，在内核初始化内存 [管理子系统](https://zhida.zhihu.com/search?content_id=230051811&content_type=Article&match_order=1&q=%E7%AE%A1%E7%90%86%E5%AD%90%E7%B3%BB%E7%BB%9F&zhida_source=entity) 的时候，会在 kmem_cache_init 函数中完成 slab alloactor 体系的创建初始化工作，之后紧接着就会创建初始化 kmalloc 体系。
...
> 关于 slab allocator 体系状态变迁的详细内容，感兴趣的同学可以回看下 **[《从内核源码看 slab 内存池的创建初始化流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488095%26idx%3D1%26sn%3Dffc73f33cb03f0be7cf2ab1172fdc448%26chksm%3Dce77d418f9005d0e9d95f223f679198fd2e40377abdcaf2c636962994c3508bcd814f9929937%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 》一文中的 “4. slab allocator 整个体系的状态变迁” 小节。
...
![](images/v2-81b071c41eccb8d3f53677587c44be15_1440w.jpg)
...
![](images/v2-48e5d0f5d0db278fa569dbf3ff00463f_1440w.jpg)

cat /proc/slabinfo 命令查看.png

最后调用 create_kmalloc_cache 根据 kmalloc_info\[idx\].size 和 kmalloc_info\[idx\].name 指定的尺寸和名称创建 slab cache。关于 slab cache 的详细创建过程，感兴趣的同学可以回看下 **[《从内核源码看 slab 内存池的创建初始化流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488095%26idx%3D1%26sn%3Dffc73f33cb03f0be7cf2ab1172fdc448%26chksm%3Dce77d418f9005d0e9d95f223f679198fd2e40377abdcaf2c636962994c3508bcd814f9929937%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 。
...
![](images/v2-dc382e65e6d1cf8266493b112816e9a0_1440w.jpg)
...
![](images/v2-621c07f302c7f65b6e55d25ddfb04c95_1440w.jpg)

image.png

当申请内存块的尺寸在 192 字节以上的时候，内核直接通过 `fls(size - 1)` 来定位 kmalloc_caches 数组中的索引 index。

![](images/v2-598b55a6f15cb23f00ad29a307c21158_1440w.jpg)
...
2. 如果 gfp_t flags 明确指定了 \_\_GFP_DMA，则内核向 kmalloc 内存池申请的内存均来自于 ZONE_DMA 物理内存区域。
3. 如果 gfp_t flags 明确指定了 \_\_GFP_RECLAIMABLE，则内核向 kmalloc 内存池申请的内存均是可以被回收的。
...
> 关于内存页 page 中 flag 属性的详细内容介绍，感兴趣的读者可以回看下**[?《深入理解 Linux 物理内存管理》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486879%26idx%3D1%26sn%3D0bcc59a306d59e5199a11d1ca5313743%26chksm%3Dce77cbd8f90042ce06f5086b1c976d1d2daa57bc5b768bac15f10ee3dc85874bbeddcd649d88%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文中的 “6.3 物理内存页属性和状态的标志位 flag” 小节。

如果 page->flag 没有设置 PG_slab 标识，说明该物理内存页没有被 slab cache 管理，说明当初调用 kmalloc 分配的时候直接走的是伙伴系统，并没有从 kmalloc 内存池中分配。

那么在这种情况下，可以直接调用 \_\_free_pages 将物理内存页释放回伙伴系统中。关于伙伴系统回收内存的详细内容，感兴趣的读者可以回看下 **[《深度剖析 Linux 伙伴系统的设计与实现》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247487228%26idx%3D1%26sn%3D85e44fa5b090b29ab23ca6abf98da221%26chksm%3Dce77c8bbf90041ad4958a3871a880a3f6d812e282530ea047d9eaf76d9f03aafb4b987a64ae9%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文中的 “7. 内存释放源码实现” 小节。
...
关于 slab cache 回收内存块的详细内容，感兴趣的读者可以回看下 **[《深度解析 slab 内存池回收内存以及销毁全流程》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247488210%26idx%3D1%26sn%3D61fb6bb2dc19ee3ecf889de0702642d7%26chksm%3Dce77d495f9005d837013fa698914479e5d805040224c2110ba1b8291fb8508b5a6b477f95e09%26scene%3D178%26cur_album_id%3D2559805446807928833%23rd)** 一文中的内容。
...
发布于 2023-06-21 11:10・广东[Linux](https://www.zhihu.com/topic/19554300)[Linux 内核](https://www.zhihu.com/topic/19614193)[内存管理](https://www.zhihu.com/topic/19579205)[IC设计求职者梦寐以求的流片项目，终于找到了！](https://zhuanlan.zhihu.com/p/702152453)
