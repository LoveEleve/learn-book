...
内存管理子系统可谓是 Linux 内核众多子系统中最为复杂最为庞大的一个，其中包含了众多繁杂的概念和原理，通过内存管理这条主线我们把可以把操作系统的众多核心系统给拎出来，比如：进程管理子系统，网络子系统， [文件子系统](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%96%87%E4%BB%B6%E5%AD%90%E7%B3%BB%E7%BB%9F&zhida_source=entity) 等。
...
当程序运行起来之后就变成了进程，而这些业务数据结构的引用在进程的视角里全都都是 [虚拟内存地址](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98%E5%9C%B0%E5%9D%80&zhida_source=entity) ，因为进程无论是在用户态还是在内核态能够看到的都是虚拟内存空间，物理内存空间被操作系统所屏蔽进程是看不到的。

进程通过虚拟内存地址访问这些数据结构的时候，虚拟内存地址会在内存管理子系统中被转换成 [物理内存地址](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%86%85%E5%AD%98%E5%9C%B0%E5%9D%80&zhida_source=entity) ，通过物理内存地址就可以访问到真正存储这些数据结构的物理内存了。随后就可以对这块物理内存进行各种业务操作，从而完成业务逻辑。
...
![](images/v2-72275ea489e68213639a11f82cccaeea_1440w.jpg)

本文概要.png

## 1\. 到底什么是虚拟内存地址

首先人们提出地址这个概念的目的就是用来方便定位现实世界中某一个具体事物的真实地理位置，它是一种用于定位的 [概念模型](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%A6%82%E5%BF%B5%E6%A8%A1%E5%9E%8B&zhida_source=entity) 。

举一个生活中的例子，比如大家在日常生活中给亲朋好友邮寄一些本地特产时，都会填写收件人地址以及寄件人地址。以及在日常网上购物时，都会在相应电商 APP 中填写自己的收获地址。

![](images/v2-199c9bbc4a6fa2cea83b95c9056e72b7_1440w.jpg)
...
综上所述，收货地址是一个 [虚拟地址](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E8%99%9A%E6%8B%9F%E5%9C%B0%E5%9D%80&zhida_source=entity) ，它是人为定义的，而我们的城市，小区，街道是真实存在的，他们的地理位置就是 [物理地址](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%89%A9%E7%90%86%E5%9C%B0%E5%9D%80&zhida_source=entity) 。

![](images/v2-900fc9c7424028197574ee15626ccca2_1440w.jpg)
...
我们还是以日常生活中的收货地址为例做出类比，我们都很熟悉收货地址的格式：xx省xx市xx区xx街道xx小区xx室，它是按照地区层次递进的。同样，在 [计算机世界](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E8%AE%A1%E7%AE%97%E6%9C%BA%E4%B8%96%E7%95%8C&zhida_source=entity) 中的虚拟内存地址也有这样的递进关系。
...
那么我们该如何解决这个问题呢？程序的 [局部性原理](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%B1%80%E9%83%A8%E6%80%A7%E5%8E%9F%E7%90%86&zhida_source=entity) 再一次救了我们~~

> [程序局部性原理](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%A8%8B%E5%BA%8F%E5%B1%80%E9%83%A8%E6%80%A7%E5%8E%9F%E7%90%86&zhida_source=entity) 表现为：时间局部性和空间局部性。时间局部性是指如果程序中的某条指令一旦执行，则不久之后该指令可能再次被执行；如果某块数据被访问，则不久之后该数据可能再次被访问。空间局部性是指一旦程序访问了某个存储单元，则不久之后，其附近的存储单元也将被访问。
...
![](images/v2-81359b37ae8d9c563628c13ad368d547_1440w.jpg)
...
这样进程就以为自己独占了整个内存空间资源，给进程产生了所有内存资源都属于它自己的幻觉，这其实是 CPU 和操作系统使用的一个障眼法罢了，任何一个虚拟内存里所存储的数据，本质上还是保存在真实的物理内存里的。只不过内核帮我们做了虚拟内存到物理内存的这一层映射，将不同进程的虚拟地址和不同内存的物理 [地址映射](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%9C%B0%E5%9D%80%E6%98%A0%E5%B0%84&zhida_source=entity) 起来。
...
首先我们会想到的是一个进程运行起来是为了执行我们交代给进程的工作，执行这些工作的步骤我们通过程序代码事先编写好，然后编译成 [二进制文件](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E4%BA%8C%E8%BF%9B%E5%88%B6%E6%96%87%E4%BB%B6&zhida_source=entity) 存放在磁盘中，CPU 会执行二进制文件中的机器码来驱动进程的运行。所以在进程运行之前，这些存放在二进制文件中的机器码需要被加载进内存中，而用于存放这些机器码的虚拟内存空间叫做 [代码段](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E4%BB%A3%E7%A0%81%E6%AE%B5&zhida_source=entity) 。

![](images/v2-6a59ffc2afa10336d9537bc188aee2ff_1440w.jpg)

image.png

在程序运行起来之后，总要操作变量吧，在程序代码中我们通常会定义大量的 [全局变量](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%85%A8%E5%B1%80%E5%8F%98%E9%87%8F&zhida_source=entity) 和静态变量，这些全局变量在程序编译之后也会存储在二进制文件中，在程序运行之前，这些全局变量也需要被加载进内存中供程序访问。所以在虚拟内存空间中也需要一段区域来存储这些全局变量。

- 那些在代码中被我们指定了初始值的全局变量和静态变量在虚拟内存空间中的存储区域我们叫做数据段。
- 那些没有指定初始值的全局变量和静态变量在虚拟内存空间中的存储区域我们叫做 [BSS 段](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=BSS+%E6%AE%B5&zhida_source=entity) 。这些未初始化的全局变量被加载进内存之后会被初始化为 0 值。
  ![](images/v2-2997bf19bf3cf78b9d427ba37af4ed5a_1440w.jpg)

image.png

上面介绍的这些全局变量和静态变量都是在编译期间就确定的，但是我们程序在运行期间往往需要动态的申请内存，所以在虚拟内存空间中也需要一块区域来存放这些动态申请的内存，这块区域就叫做 [堆](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%A0%86&zhida_source=entity) 。注意这里的堆指的是 OS 堆并不是 JVM 中的堆。

![](images/v2-c5b1cd86f65459abdef7cc9e237d540c_1440w.jpg)

image.png

除此之外，我们的程序在运行过程中还需要依赖 [动态链接库](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%8A%A8%E6%80%81%E9%93%BE%E6%8E%A5%E5%BA%93&zhida_source=entity) ，这些动态链接库以.so 文件的形式存放在磁盘中，比如 C 程序中的 glibc，里边对 [系统调用](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&zhida_source=entity) 进行了封装。glibc 库里提供的用于动态申请 [堆内存](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%A0%86%E5%86%85%E5%AD%98&zhida_source=entity) 的 malloc 函数就是对系统调用 sbrk 和 mmap 的封装。这些动态链接库也有自己的对应的代码段，数据段，BSS 段，也需要一起被加载进内存中。
...
![](images/v2-fd4e3c38a3abf5c9c29387d82fee9e91_1440w.jpg)

image.png

最后我们在程序运行的时候总该要调用各种函数吧，那么调用函数过程中使用到的局部变量和函数参数也需要一块内存区域来保存。这一块区域在虚拟内存空间中叫做 [栈](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%A0%88&zhida_source=entity) 。

![](images/v2-179627b3c76f834c905f0b1085da4c1b_1440w.jpg)
...
- 用于在程序运行过程中动态申请内存的堆。
- 用于存放动态链接库以及 [内存映射](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E6%98%A0%E5%B0%84&zhida_source=entity) 区域的文件映射与匿名映射区。
...
接下来用户态虚拟内存空间的最后一块区域就是栈空间了，在这里会保存函数运行过程所需要的局部变量以及函数参数等函数调用信息。 **栈空间中的地址增长方向是从高地址向低地址增长** 。每次进程申请新的 [栈地址](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%A0%88%E5%9C%B0%E5%9D%80&zhida_source=entity) 时，其地址值是在减少的。
...
![](images/v2-6c48dce505b02cbe9d49259bce74c08c_1440w.jpg)
...
这样一来就在用户态虚拟内存空间与内核态虚拟内存空间之间形成了一段 0x0000 7FFF FFFF F000 - 0xFFFF 8000 0000 0000 的地址空洞，我们把这个空洞叫做 [canonical address 空洞](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=canonical+address+%E7%A9%BA%E6%B4%9E&zhida_source=entity) 。

![](images/v2-4dc50f7f3e5542287230baf1adf91129_1440w.jpg)
...
如下图所示：\_\_PAGE_OFFSET 的值在 32 位系统下为 0xC000 000。

![](images/v2-ea96331b44f562abfa33dfdc3d1872b3_1440w.jpg)
...
![](images/v2-2865347469b8b70955b6f219acd3e3c6_1440w.jpg)
...
arg_start 和 arg_end 是参数列表的位置， env_start 和 env_end 是 [环境变量](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%8E%AF%E5%A2%83%E5%8F%98%E9%87%8F&zhida_source=entity) 的位置。它们都位于栈中的最高地址处。

![](images/v2-31dbb9021b193db4b451f04ef9fe4fc4_1440w.jpg)
...
![](images/v2-647d093b59b247c701c678d84e9bec19_1440w.jpg)
...
![](images/v2-c7dcb8e62b3029f29bfbfaeff87fd30c_1440w.jpg)

image.png

VM_SHARD 用于指定这块虚拟内存区域映射的物理内存是否可以在多进程之间共享，以便完成 [进程间通讯](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E8%BF%9B%E7%A8%8B%E9%97%B4%E9%80%9A%E8%AE%AF&zhida_source=entity) 。

> 设置这个值即为 mmap 的共享映射，不设置的话则为私有映射。这个等后面我们讲到 mmap 的相关实现时还会再次提起。

VM_IO 的设置表示这块虚拟内存区域可以映射至设备 IO 空间中。通常在设备驱动程序执行 mmap 进行 IO [空间映射](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%A9%BA%E9%97%B4%E6%98%A0%E5%B0%84&zhida_source=entity) 时才会被设置。
...
我们可以通过 posix_fadvise，madvise 系统调用来暗示内核是否对相关内存区域进行顺序读取或者随机读取。相关的详细内容，大家可以看下笔者上篇文章 **[《从 Linux 内核角度探秘 JDK NIO 文件读写本质》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486623%26idx%3D1%26sn%3D0cafed9e89b60d678d8c88dc7689abda%26chksm%3Dce77cad8f90043ceaaca732aaaa7cb692c1d23eeb6c07de84f0ad690ab92d758945807239cee%26token%3D1276722624%26lang%3Dzh_CN%23rd)** 中的第 9 小节文件页预读部分。
...
struct vm_area_struct 结构中还有一个 vm_ops 用来指向针对虚拟内存区域 VMA 的相关操作的 [函数指针](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%87%BD%E6%95%B0%E6%8C%87%E9%92%88&zhida_source=entity) 。
...
内核中这种类似的用法其实有很多，在内核中每个特定领域的描述符都会定义相关的操作。比如在前边的文章 **[《从 Linux 内核角度探秘 JDK NIO 文件读写本质》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486623%26idx%3D1%26sn%3D0cafed9e89b60d678d8c88dc7689abda%26chksm%3Dce77cad8f90043ceaaca732aaaa7cb692c1d23eeb6c07de84f0ad690ab92d758945807239cee%26token%3D1276722624%26lang%3Dzh_CN%23rd)** 中我们介绍到内核中的文件描述符 struct file 中定义的 struct file_operations \*f_op。里面定义了内核针对文件操作的函数指针，具体的实现根据不同的文件类型有所不同。
...
在 ext4 文件系统中管理的文件对应的 file_operations 指向 ext4_file_operations，专门用于操作 ext4 文件系统中的文件。还有针对 page cache 页高速缓存相关 [操作定义](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%93%8D%E4%BD%9C%E5%AE%9A%E4%B9%89&zhida_source=entity) 的 address_space_operations 。

![](images/v2-778f135005bcc22dabe066c14a756de9_1440w.jpg)

image.png

还有我们在 **[《从 Linux 内核角度看 IO 模型的演变》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247483737%26idx%3D1%26sn%3D7ef3afbb54289c6e839eed724bb8a9d6%26chksm%3Dce77c71ef9004e08e3d164561e3a2708fc210c05408fa41f7fe338d8e85f39c1ad57519b614e%26scene%3D21%23wechat_redirect)** 一文中介绍到，socket 相关的操作接口定义在 inet_stream_ops 函数集合中，负责对上给用户提供接口。而 socket 与内核 [协议栈](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%8D%8F%E8%AE%AE%E6%A0%88&zhida_source=entity) 之间的操作接口定义在 struct sock 中的 sk_prot 指针上，这里指向 tcp_prot 协议操作函数集合。
...
在内核中其实是通过一个 struct vm_area_struct 结构的 [双向链表](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%8F%8C%E5%90%91%E9%93%BE%E8%A1%A8&zhida_source=entity) 将虚拟内存空间中的这些虚拟内存区域 VMA 串联起来的。
...
![](images/v2-1be29d6871fdf9f75e8c2a81ad910451_1440w.jpg)
...
尤其在进程虚拟内存空间中包含的内存区域 VMA 比较多的情况下，使用 [红黑树](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%BA%A2%E9%BB%91%E6%A0%91&zhida_source=entity) 查找特定虚拟内存区域的 [时间复杂度](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%97%B6%E9%97%B4%E5%A4%8D%E6%9D%82%E5%BA%A6&zhida_source=entity) 是 O( logN ) ，可以显著减少查找所需的时间。
...
> \_\_PAGE_OFFSET 的值在 32 位系统下为 0xC000 000

![](images/v2-51273ca7d58610674dc108fa4b82acac_1440w.jpg)

image.png

在 32 位 [体系结构](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E4%BD%93%E7%B3%BB%E7%BB%93%E6%9E%84&zhida_source=entity) 下进程用户态虚拟内存空间为 3 GB，虚拟内存地址范围为：0x0000 0000 - 0xC000 000 。内核态虚拟内存空间为 1 GB，虚拟内存地址范围为：0xC000 000 - 0xFFFF FFFF。

本小节我们主要关注 0xC000 000 - 0xFFFF FFFF 这段虚拟内存地址区域也就是内核虚拟内存空间的布局情况。

### 7.1.1 直接映射区

在总共大小 1G 的内核虚拟内存空间中，位于最前边有一块 896M 大小的区域，我们称之为直接映射区或者 [线性映射](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%BA%BF%E6%80%A7%E6%98%A0%E5%B0%84&zhida_source=entity) 区，地址范围为 3G -- 3G + 896m 。
...
![](images/v2-f53cf113501c25850e09bb4641f7a97d_1440w.jpg)
...
![](images/v2-9877a509988d25cae7df955f4c3ccb86_1440w.jpg)
...
![](images/v2-af91aec5c3edc7c13ae1ba2afcc0a8fa_1440w.jpg)
...
![](images/v2-0055f28b2d7c5d2d7b5c22e1d18f8838_1440w.jpg)

image.png

和用户态进程使用 malloc 申请内存一样，在这块动态映射区内核是使用 vmalloc 进行 [内存分配](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%86%85%E5%AD%98%E5%88%86%E9%85%8D&zhida_source=entity) 。由于之前介绍的动态映射的原因，vmalloc 分配的内存在虚拟内存上是连续的，但是物理内存是不连续的。通过页表来建立物理内存与虚拟内存之间的映射关系，从而可以将不连续的物理内存映射到连续的虚拟内存上。
...
![](images/v2-a0cf1f02cc61f843496e8d0ddb2ba0a4_1440w.jpg)
...
![](images/v2-de7b6479bc40da70b71211c00f64a0fb_1440w.jpg)
...
在固定映射区中的虚拟内存地址可以自由映射到物理内存的高端地址上，但是与动态映射区以及永久映射区不同的是，在固定映射区中虚拟地址是固定的，而被映射的物理地址是可以改变的。也就是说，有些虚拟地址在编译的时候就固定下来了，是在内核启动过程中被确定的，而这些虚拟地址对应的物理地址不是固定的。采用固定虚拟地址的好处是它相当于一个 [指针常量](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%8C%87%E9%92%88%E5%B8%B8%E9%87%8F&zhida_source=entity) （常量的值在编译时确定），指向物理地址，如果虚拟地址不固定，则相当于一个指针变量。
...
笔者在之前文章 **[《从 Linux 内核角度探秘 JDK NIO 文件读写本质》](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s%3F__biz%3DMzg2MzU3Mjc3Ng%3D%3D%26mid%3D2247486623%26idx%3D1%26sn%3D0cafed9e89b60d678d8c88dc7689abda%26chksm%3Dce77cad8f90043ceaaca732aaaa7cb692c1d23eeb6c07de84f0ad690ab92d758945807239cee%26token%3D1276722624%26lang%3Dzh_CN%23rd)** 的 “ 12.3 iov_iter_copy_from_user_atomic ” 小节中介绍在 Buffered IO 模式下进行文件写入的时候，在下图中的第四步，内核会调用 iov_iter_copy_from_user_atomic 函数将用户空间缓冲区 DirectByteBuffer 中的待写入数据拷贝到 page cache 中。
...
![](images/v2-4ceccb12795b0689d1041b0b2b2e8f8a_1440w.jpg)
...
从 \_\_START_KERNEL_map 开始是大小为 512M 的区域用于存放内核代码段、全局变量、BSS 等。这里对应到物理内存开始的位置，减去 \_\_START_KERNEL_map 就能得到物理内存的地址。这里和直接映射区有点像，但是不矛盾，因为直接映射区之前有 8T 的空洞区域，早就过了内核代码在物理内存中加载的位置。

\_\_START_KERNEL_map 变量定义在 `/arch/x86/include/asm/page_64_types.h` 文件中：
...
聊完了虚拟内存，我们接着聊一下物理内存，我们平时所称的内存也叫随机访问存储器（ [random-access](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=random-access&zhida_source=entity) memory ）也叫 RAM 。而 RAM 分为两类：

- 一类是静态 RAM（ `SRAM` ），这类 SRAM 用于 CPU 高速缓存 L1Cache，L2Cache，L3Cache。其特点是访问速度快，访问速度为 1 - 30 个 [时钟周期](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%97%B6%E9%92%9F%E5%91%A8%E6%9C%9F&zhida_source=entity) ，但是容量小，造价高。
...
![](images/v2-1d4a98004a56a4f39ea1cec3bd6497bf_1440w.jpg)

image.png

如图所示内存条上黑色的元器件就是存储器模块（memory module）。多个存储器模块连接到存储控制器上，就聚合成了主存。

![](images/v2-8250e1e83351be0ac63efdd4980d67ae_1440w.jpg)

内存结构.png

而 DRAM 芯片就包装在存储器模块中，每个存储器模块中包含 8 个 DRAM 芯片，依次编号为 0 - 7 。

![](images/v2-a4e7810066c60d2a8b419078b758e993_1440w.jpg)

存储器模块.png

而每一个 DRAM 芯片的 [存储结构](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E5%AD%98%E5%82%A8%E7%BB%93%E6%9E%84&zhida_source=entity) 是一个二维矩阵，二维矩阵中存储的元素我们称为超单元（supercell），每个 supercell 大小为一个字节（8 bit）。每个 supercell 都由一个坐标地址（i，j）。
...
![](images/v2-1a81a7e85a25d4b6a606ecab03acde4d_1440w.jpg)
...
![](images/v2-47cc0cd3f1be629898746cee6e11458b_1440w.jpg)
...
总线上传输的信号包括：地址信号，数据信号，控制信号。其中 [控制总线](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%8E%A7%E5%88%B6%E6%80%BB%E7%BA%BF&zhida_source=entity) 上传输的控制信号可以同步事务，并能够标识出当前正在被执行的事务信息：
...
如上图所示，其中 [系统总线](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E7%B3%BB%E7%BB%9F%E6%80%BB%E7%BA%BF&zhida_source=entity) 是连接 CPU 与 IO bridge 的，存储总线是来连接 IO bridge 和主存的。
...
首先 CPU 芯片中的 [总线接口](https://zhida.zhihu.com/search?content_id=216363268&content_type=Article&match_order=1&q=%E6%80%BB%E7%BA%BF%E6%8E%A5%E5%8F%A3&zhida_source=entity) 会在总线上发起读事务（read transaction）。 该读事务分为以下步骤进行：
...
![](images/v2-1ce467976f78f72f11604fd800714021_1440w.jpg)
...
发布于 2022-10-25 12:30[内存管理](https://www.zhihu.com/topic/19579205)[Linux](https://www.zhihu.com/topic/19554300)[虚拟内存](https://www.zhihu.com/topic/19831239)
