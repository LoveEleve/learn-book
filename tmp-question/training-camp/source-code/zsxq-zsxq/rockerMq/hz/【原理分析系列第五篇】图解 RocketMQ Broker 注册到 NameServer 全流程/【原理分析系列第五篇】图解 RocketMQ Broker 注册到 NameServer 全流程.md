## 【原理分析系列第五篇】图解 RocketMQ Broker 注册到 NameServer 全流程

大家好，我是**华仔**, 又跟大家见面了。

从今天开始，我们开始对 RocketMQ 进行相关实现原理进行剖析，今天是第五篇，我们来聊聊 RocketMQ 「**Broker**」注册到「**NameServer**」全流程，深度剖析下其内部底层原理设计思想，**下面进入正题**。

![](images/FlKjjTV49Fct4l9eFxmzxBwP0voY.png)

##   
**01 总体概述**

在 [原理分析系列第三篇】图解 RocketMQ NameServer 架构设计](https://articles.zsxq.com/id_plt5q5i603r3.html) 这篇中，我们深度剖析了关于「**NameServer**」组件服务，了解了「**NameServer**」是⼀个非常简单的 「**Topic 路由注册中心**」，其角色类似 Kafka、Dubbo 中的 Zookeeper ，支持 Broker 的「**动态注册与发现**」。
...
「**Broker**」启动的时候会初始化一些「**配置组件**」，初始化完成后就会进行「**正式启动**」，此时它会先启动各个组件，如下图所示：

![](images/FrEBahZ81ZAuzs5rs5MW5k7ADFIg.png)

![](images/FjGS_v4SR6J-857TChrFWGJOaCgw.png)
...
![](images/lhaMpfXkbaxrG5hke5jlXgSlJ6Pl.png)
...
![](images/llp2tRwQvskD9QQQWZDfB5F1CgEx.png)
...
![](images/loWS57ADsygKkKabW_sG5mYsFxrI.png)
...
下篇我们来深度剖析「**Topic 数据分片机制**」，大家期待，我们下期见。
