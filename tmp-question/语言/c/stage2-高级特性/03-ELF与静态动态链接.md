...
**面试官**：动态链接的程序，`printf` 的第一次调用和后续调用有什么区别？用 GOT 和 PLT 解释。

**回答**：第一次调用走 PLT 桩代码——`jmp *GOT[n]` 指向 PLT 的下一行（push reloc_index + jmp PLT[0]），触发 `_dl_runtime_resolve` 解析 `printf` 的实际地址并写回 GOT[n]。后续调用直接通过 `jmp *GOT[n]` 跳到 printf——不再经过动态链接器，只有一次间接跳转的开销。这叫"懒惰绑定"——只解析实际使用的符号，而不是程序启动时所有动态符号。

**追问 1（面试官）**：`.rela.plt` 和 `.rela.dyn` 的区别是什么？为什么函数重定位可以延迟但数据重定位不行？

**追问 1 回答**：`.rela.plt` 存函数调用的重定位（`R_X86_64_JUMP_SLOT`），`.rela.dyn` 存数据引用的重定位（`R_X86_64_GLOB_DAT`）。函数可以延迟——第一次调用时再解析地址并写回 GOT，启动时不需要知道所有函数地址。数据不能延迟——`extern int global_var;` 的值必须知道物理地址才能存/取，程序启动前就必须全部解析完成，否则访问全局变量会操作野地址。这就是为什么 Full RELRO（`-z now`）强制所有重定位在启动时完成——它把 `.rela.plt` 也提前解析，GOT 设为只读，防止攻击者覆写。

**追问 2（面试官）**：那 PIE 和 PIC 的关系是什么？为什么共享库需要 `-fPIC` 但旧的非 PIE 可执行文件不需要？

**追问 2 回答**：PIE 是"可执行文件本身位置无关"（ASLR 保护），PIC 是"代码本身位置无关"（`-fPIC` 编译选项的效果）。共享库必须用 `-fPIC`——因为不同进程把同一个 `.so` 加载到不同地址，代码不能假设固定地址。而非 PIE 可执行文件有固定的加载地址（`readelf -h` 显示 Type: EXEC），编译时不需要位置无关。但安全考虑——非 PIE 固定地址让 ROP（Return-Oriented Programming）攻击更容易，所以现代 Linux 分发版默认用 PIE（Type: DYN）。

---

### 面试题 2：动态库搜索与 LD_PRELOAD

**面试官**：程序运行时，动态链接器按什么顺序找 `.so`？`LD_LIBRARY_PATH` 和 `rpath` 的区别是什么？

**回答**：搜索顺序——(1) LD_LIBRARY_PATH - 环境变量，调试用，不用于生产；(2) DT_RPATH - 编译时硬编码在 ELF 中的路径（`-Wl,-rpath,/path`）；(3) DT_RUNPATH - 类似 rpath 但优先级更低，可被 LD_LIBRARY_PATH 覆盖；(4) ld.so.cache - `ldconfig` 构建的缓存；(5) /lib、/usr/lib - 系统默认路径。rpath 嵌入在二进制中，不需要环境变量，适合生产部署——程序到哪都能找到自己的共享库。

**追问 1（面试官）**：LD_PRELOAD 怎么工作的？有什么防御手段？

**追问 1 回答**：LD_PRELOAD 指定的 `.so` 在符号查找中优先级最高——如果 `libevil.so` 定义了 `open()`，所有 `open()` 调用先进入 `libevil.so`，它可以选择转发、拦截或篡改参数。防御三层——(1) Full RELRO（`-z relro -z now`）将 GOT 设为只读，攻击者在运行时无法改写 GOT；(2) 检查 `/proc/self/maps` 确认没有未知 `.so` 被注入进程地址空间；(3) 静态链接消除 LD_PRELOAD 攻击面，但失去库升级能力。

**追问 2（面试官）**：SONAME 和符号版本控制是什么？为什么 glibc 升级不打破老程序？

**追问 2 回答**：SONAME 是 `.so` 的运行时标识（如 `libcurl.so.4`），嵌入在 ELF 的 `DT_SONAME` 字段中。符号版本控制（symbol versioning）让同一个 `.so` 同时提供多个不兼容版本的接口——glibc 中 `pthread_create@@GLIBC_2.34` 是新版默认，`pthread_create@GLIBC_2.2.5` 是旧版兼容符号。老程序链接到 `@GLIBC_2.2.5`，新程序链接到 `@@GLIBC_2.34`——同一份 `libc.so.6` 同时满足新旧需求，实现了"升级库不打破现有程序"。
