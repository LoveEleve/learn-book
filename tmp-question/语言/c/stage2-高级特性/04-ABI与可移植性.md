...
**面试官**：`sizeof(long)` 在 Linux x86_64 和 Windows x64 上分别是多少？为什么不一样？

**回答**：Linux 8 字节（LP64 模型），Windows 4 字节（LLP64 模型）。原因 ——Linux 选择 LP64 因为 `long` 应该能存指针和文件大小（64 位平台就应该是 64 位），Windows 选择 LLP64 因为要向后兼容——Win32 API 大量使用 `LONG`/`DWORD`，把 `long` 从 32 改 64 会让大量旧代码 ABI 不兼容。

**追问 1（面试官）**：那你在生产代码里怎么写跨平台的整数？什么时候用 `int`、`long`、`int64_t`？

**追问 1 回答**：三条规则——(1) 网络协议、文件格式、跨平台数据交换用精确宽度类型（`uint32_t`、`int64_t`）；(2) 大小和索引用 `size_t`（永远无符号，跨平台一致）；(3) 局部循环计数器、临时变量用 `int`（自然寄存器宽度，最快）。**绝对不用 `long`** 做协议/格式/数据交换——它在不同平台大小不同。格式化输出用 `<inttypes.h>` 的 `PRIu64`/`PRId64` 等宏而不是硬编码 `%lu`/`%lld`。

**追问 2（面试官）**：`int` 在所有现代平台都是 32 位——这个假设安全吗？

**追问 2 回答**：在桌面/服务器/移动平台安全——x86_64、ARM64、MIPS64 上 `int` 都是 32 位。但在某些 DSP（数字信号处理器）和嵌入式 MCU 上，`int` 可以是 16 位——C 标准只要求 `int` ≥ 16 位。所以如果代码要支持 DSP/嵌入式（如 AUTOSAR、FreeRTOS），不能用"int = 32"的假设，应该用 `int32_t`。大多数后端/系统/中间件 C 项目面向桌面/服务器，`int = 32` 成立——但这是一个"看场景"的假设。

---

### 面试题 2：字节序与序列化

**面试官**：把你写的 struct 直接 `send(fd, &my_struct, sizeof(my_struct), 0)` 发给另一台机器——有什么问题？

**回答**：三个问题——(1) 字节序不一致（x86 小端 vs 网络大端），接收端读到的 multibyte 值字节顺序错误；(2) struct padding 不一致（不同编译器/平台对齐规则不同），接收端的 `offsetof(field, struct)` 和发送端不一样；(3) 类型大小不一致（Linux LP64 `long`=8 字节，Windows LLP64 `long`=4 字节），接收端期望的字段大小和发送端不一致。正确的做法是逐字段序列化为网络字节序——`htonl(my_struct.id)` 然后 `send(&id_net, 4)`。

**追问 1（面试官）**：`htonl` 和 `ntohl` 在 x86（小端）上做什么？在 SPARC（大端）上做什么？

**追问 1 回答**：在小端机上颠倒字节顺序——`htonl(0x01020304)` → 发送的是 `04 03 02 01`（低地址到高地址），接收端 `ntohl` 再颠倒回来。在大端机上 `htonl` 是 no-op（恒等函数）——大端和网络序一致。`htonl` 的含义是"把主机字节序转换为网络（大端）字节序"——所以不同平台上它做的事不同，但结果相同（都以网络大端序发送）。

**追问 2（面试官）**：那跨语言的序列化呢？C 和 Python 之间传 struct 数据？

**追问 2 回答**：不能用结构体直接传——用协议：Protobuf、FlatBuffers、Cap'n Proto 等跨语言序列化框架，它们处理类型大小、对齐、字节序的差异。如果自己实现，用 JSON 或 MessagePack 的二进制格式 + 显式字节序规范。如果必须用裸 struct——定义统一的数据格式规范（所有字段用精确宽度类型 + 大端序 + packed），两端各写符合规范的序列化代码。这也是为什么网络协议 RFC 定义的字段都是 `uint32_t`/`uint16_t`（bits on the wire），而不是 C 的 `long`/`int`。

---

### 面试题 3：编译器差异与条件编译

**面试官**：你的 C 项目要在 GCC、Clang、MSVC 上编译——`__attribute__((packed))` 在 MSVC 上不能用，怎么处理？

**回答**：条件编译——GCC/Clang 用 `__attribute__((packed))`，MSVC 用 `#pragma pack(push, 1)` / `#pragma pack(pop)` 包围 struct 定义。用 `#if defined(__GNUC__) || defined(__clang__)` 和 `#elif defined(_MSC_VER)` 区分，最后 `#else #error "Unsupported compiler"` 兜底。

**追问 1（面试官）**：MSVC 为什么连 C11 的 `_Generic` 和 `<stdatomic.h>` 都不支持？Windows 上怎么弥补？

**追问 1 回答**：微软的策略是 C++ 第一，C 只是兼容旧代码的遗留物。MSVC 的 C 编译器从 C89 之后几乎没有更新标准支持。弥补方案——(1) 用 MinGW（GCC 的 Windows 移植，完全支持 C11/C17）；(2) 用 Clang（Clang 的 Windows 版本支持完整 C11/C17）；(3) 如果用 MSVC 必须——条件编译降级：`#if __STDC_VERSION__ >= 201112L && !defined(__STDC_NO_ATOMICS__)` 用 `<stdatomic.h>`，`#else` 用 pthread mutex 回退。

**追问 2（面试官）**：`__attribute__((cleanup))`（GCC 扩展，RAII 风格自动释放）在 Clang 和 MSVC 上能不能用？有标准替代吗？

**追问 2 回答**：`__attribute__((cleanup))` 是 GCC 特有扩展——Clang 支持（兼容 GCC），MSVC 不支持。C 语言层面没有标准替代——C 没有 RAII 或析构函数。替代方案——(1) 用 `goto cleanup` 模式（Linux 内核风格，显式但可靠）；(2) 用 opaque pointer + `create/destroy` 命名约定强制调用者释放；(3) 实在需要跨平台自动释放→不用 C，用 C++ 的 RAII。这体现了 Ch01 §5.1 讲的"C 没有构造/析构函数"——这是 C 的设计取舍，无法通过条件编译零开销补救。
