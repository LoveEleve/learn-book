...
### 面试题 1：stdio 缓冲与 fork 陷阱

**面试官**：`printf("Hello "); fork();` 为什么可能导致输出 "Hello " 两次？

**回答**：`printf("Hello ")` 没有 `\n`→数据在 stdio 缓冲��（行缓冲模式下），还没写到内核。`fork()` 完整复制父进程内存→子进程得到同一份 stdio 缓冲区（里面有 "Hello "）。父子进程退出时各自 `fclose(stdout)`→各自 flush 缓冲区→"Hello " 输出两次。解决：`fflush(stdout)` 在 fork 前强制刷新，或 fork 后子进程立即 `_exit(0)`（跳过 stdio flush）。

**追问 1（面试官）**：page cache 和 stdio 缓冲分别在哪一层？为什么 `write` 返回后数据可能还没到磁盘？

**追问 1 回答**：stdio 缓冲在用户态（libc 层）——减少系统调用频率。page cache 在内核态——数据进入 page cache 后 `write` 系统调用返回，但内核仅标记页面为"脏"（dirty），由后台 kworker 线程异步刷磁盘。一旦机器崩溃，page cache 数据丢失。`fsync/fdatasync` 强制内核立即刷脏页到磁盘——数据库事务提交必须调用。

**追问 2（面试官）**：`O_DIRECT` 绕过 page cache，什么时候该用？什么时候不该用？

**追问 2 回答**：该用——数据库（MySQL InnoDB/PostgreSQL）自建了 buffer pool，如果 page cache 也缓存同一份数据→双重缓存浪费内存。`O_DIRECT` 让数据库直接管理缓存命中率。不该用——(1) 小文件频繁读写——page cache 的合并写入和预读极大提升性能；(2) 日志文件需要 page cache 异步刷盘；(3) 没有自建缓存机制的应用——`O_DIRECT` 的每次 I/O 是真实磁盘操作，没有 page cache 的合并写入优势。

---

### 面试题 2：短写与 EINTR

**面试官**：`write(fd, buf, 4096)` 返回了 1024——什么场景下会发生？正确的处理方式是什么？

**回答**：短写（partial write）发生在管道缓冲满、socket 发送缓冲满、或非阻塞 fd 上只能写一部分。正确处理——循环直到写完：`while (total < n) { w = write(...); if (w < 0) { if (errno == EINTR) continue; return -1; } total += w; }`。判断 write 返回 0 也是一个需处理的情况（防止死循环）。

**追问 1（面试官）**：`read` 被信号中断返回 `EINTR`——什么场景下会发生？怎么正确处理？

**追问 1 回答**：进程注册了信号 handler 且设置了 `SA_RESTART` 标志时，`read` 在等待数据期间信号到达→内核自动重试，不返回 `EINTR`。没设置 `SA_RESTART`→`read` 返回 -1 + `errno = EINTR`。处理——`do { ret = read(fd, buf, size); } while (ret == -1 && errno == EINTR);` 或用 `TEMP_FAILURE_RETRY` 宏。这体现了 Unix 信号模型的"异步中断"本质——任何阻塞系统调用都可能被信号打断。

**追问 2（面试官）**：`FD_CLOEXEC` 是什么？不设有什么后果？

**追问 2 回答**：`FD_CLOEXEC` 是 fd 属性——标记为"在 exec 时自动关闭"。未设置的 fd 在 fork+exec 后被新程序继承→新程序不知道这些 fd 的存在→泄漏（监听 socket、数据库连接、日志文件 fd）。经典 bug——daemon 进程 fork+exec 子进程后，子进程继承了父进程的监听 socket，阻止了父进程重启时重新绑定端口。`open()` 时用 `O_CLOEXEC` 或在 `fcntl` 中设置这是生产环境的标准配置。
