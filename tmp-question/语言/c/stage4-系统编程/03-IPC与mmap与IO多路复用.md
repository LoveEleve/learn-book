...
**面试官**：select 是 O(n) 扫描，epoll 为什么是 O(1)？红黑树和就绪链表分别做什么？

**回答**：epoll 用两个数据结构——红黑树存储所有被监控的 fd（epoll_ctl 增删 O(log n)），就绪链表只存有数据的 fd。当网卡数据到达→内核协议栈处理→触发回调→将 fd 插入就绪链表。epoll_wait 直接返回就绪链表内容——不需要遍历所有 fd。select/poll 需要每次拷贝所有 fd 到内核并 O(n) 扫描哪些有数据。epoll 用"回调推"替代"每次轮询"。

**追问 1（面试官）**：ET 和 LT 的区别？为什么 Nginx 用 ET，Redis 用 LT？

**追问 1 回答**：LT（水平触发）——只要 fd 有数据就持续返回，直到读完。Redis 单线程用 LT 避免一个慢客户端饿死其他客户端——每个客户端每次 epoll_wait 都能被处理一些。ET（边沿触发）——只在 fd 状态变化（不可读→可读）时返回一次，必须循环读到 EAGAIN。Nginx 用 ET 减少 epoll_wait 调用次数——一次通知读空所有数据。ET 必须配合非阻塞 fd——否则循环 read 最后阻塞等数据。

**追问 2（面试官）**：epoll 惊群是什么？怎么解？

**追问 2 回答**：多个进程/线程各自拥有 epoll 实例监听同一 fd——一个客户端连接唤醒所有等待者，但只有一个 accept 成功，其余白醒→惊群。Linux 4.5+ 的 `EPOLLEXCLUSIVE` 将唤醒限制为一个。或 `SO_REUSEPORT`（Linux 3.9+）让内核分担连接——多个进程 bind 同一端口，内核按四元组哈希将 SYN 分给不同进程的 listen 队列——直接不产生惊群。

---

### 面试题 2：mmap 的正确使用与陷阱

**面试官**：`mmap` 和 `read` 的区别是什么？什么时候用 mmap 什么时候用 read？

**回答**：mmap 零拷贝——内核 page cache 直接映射到用户态地址空间，读写内存即读写文件。适合随机访问（直接 `p[offset]`）、大文件（按需缺页加载）。read 一次拷贝——page cache→用户态缓冲区，适合小文件/顺序读（mmap 的页表设置开销大于收益）、网络 I/O（socket 无 page cache 映射）。mmap 风险——MAP_SHARED 下文件截断后访问→SIGBUS。

**追问 1（面试官）**：MAP_PRIVATE 和 MAP_SHARED 的区别？COW 怎么体现的？

**追问 1 回答**：MAP_PRIVATE——修改不对文件可见，也不对其他进程可见（fork 子进程用 COW）。MAP_SHARED——修改直接写回文件，对其他映射同一文件的进程可见。MAP_PRIVATE 的写入只改当前进程的私有页——内核用 COW 只复制实际被写的页。匿名映射（MAP_ANONYMOUS）通常用 MAP_PRIVATE——等价于简化版 malloc。

**追问 2（面试官）**：PIPE_BUF 是什么？多进程同时写 pipe，超过 4096 字节会怎样？

**追问 2 回答**：PIPE_BUF 是原子写入上限——Linux 下 4096 字节（POSIX 最小 512）。≤4096 字节的 write 保证原子（不被其他进程的写入交错），>4096 可能被打碎。多进程日志系统如果往同一个 pipe 写，必须保证每条日志 ≤4096 字节——或每条日志后加界限符。这就是为什么 syslog 有消息长度限制 8192（但超过 PIPE_BUF 就不保证原子性）。
