...
本目录下预先建好了 5 个 stage 子目录，作为分阶段落地目录。`C-C++技术专家学习路线.md`（本文档）是**总览地图**（按 11 个 Layer 的知识体系分层），5 个 `stageN/README.md` 是**分阶段落地指南**（按学习时间分阶段，每个 stage 学几个 Layer，放该 stage 的具体章节解读文档）。

### 17.1 两套维度的关系（互补，不冲突）

- **11 个 Layer**（本文档 §1）= 知识地图（按主题分，回答"学什么"）
- **5 个 stage**（子目录）= 学习阶段（按时间分，回答"什么时候学、按什么顺序学"）

### 17.2 5 个 stage ↔ 11 个 Layer 映射表

| stage 子目录 | 对应 Layer | 学习内容 | 预计时间 | stage README 位置 |
|---|---|---|---|---|
| `stage1-核心语法/` | Layer 0-2 | C 本质 / 标准演进 / 编译模型 / 内存模型 / 类型系统 / 安全编程 | 4 周 | `stage1-核心语法/README.md` |
| `stage2-高级特性/` | Layer 3 | 编译/链接/ELF/ABI/可移植性 | 1-2 周 | `stage2-高级特性/README.md` |
| `stage3-并发编程/` | Layer 5 | pthread / 同步原语 / 原子操作 / 内存序 / 无锁数据结构 | 2 周 | `stage3-并发编程/README.md` |
| `stage4-系统编程/` | Layer 4, 6, 7 | OS 接口 / 网络编程 / 数据结构自己写 | 4-5 周 | `stage4-系统编程/README.md` |
| `stage5-工程实践/` | Layer 8-11 | OOP/FFI / 调试 / 源码阅读 / 生产化 | 3 周 + 长期 | `stage5-工程实践/README.md` |

**注**：5 个 stage 比 11 个 Layer 更紧凑（把 11 个 Layer 合并成 5 个阶段），是把 Layer 落地到目录结构的方式。每个 stage 子目录下除了 `README.md`，未来还会放该 stage 的章节级技术解读文档（参考分布式主题每本书的章节解读模式，如 `stage1-核心语法/01-C语言本质.md` / `02-内存模型.md` 等）。

### 17.3 每个 stage 的入口文档

每个 stage 子目录下的 `README.md` 应包含：

1. **学习目标**：本 stage 完成后能掌握什么（可量化、可验证）
2. **对应 Layer**：与本文档的映射（Layer 编号 + 主题）
3. **学习内容**：章节清单（每章主题）
4. **实战项目**：本 stage 的动手项目（3-5 个，从简单到复杂）
5. **参考书章节**：6 本书的对应章节
6. **完成标准**：如何验证学会了（能讲清楚什么 / 能写出什么）
7. **下一步**：完成本 stage 后进入哪个 stage

### 17.4 推荐学习顺序

```
stage1（4 周）→ stage2（1-2 周）→ stage3（2 周）→ stage4（4-5 周）→ stage5（3 周 + 长期）
   核心语法      高级特性          并发编程       系统编程            工程实践
   ↓             ↓                ↓              ↓                   ↓
 Layer 0-2     Layer 3          Layer 5       Layer 4,6,7         Layer 8-11
```

**stage 之间的依赖关系**：
- stage1（核心语法）必须先学，是所有后续 stage 的基础
- stage2（高级特性）依赖 stage1（编译链接需要理解 C 基础）
- stage3（并发编程）依赖 stage1（内存模型）+ stage2（编译/链接理解原子操作的实现）
- stage4（系统编程）依赖 stage1（内存）+ stage2（ABI/可移植性）+ stage3（并发原语用于 I/O 多路复用）
- stage5（工程实践）依赖前 4 个 stage（综合应用）

### 17.5 stage 子目录的扩展方式

每个 stage 子目录下的章节级技术解读文档命名规则：

```
stage1-核心语法/
├── README.md                          (本 stage 入口文档)
├── 01-C语言本质与编译模型.md           (对应 Layer 0)
├── 02-内存模型.md                      (对应 Layer 1)
├── 03-类型系统与表达式.md              (对应 Layer 2)
└── 04-C安全编程.md                     (对应 Layer 1 的 §1.2.9)
```

每个章节文档遵循 `feedback_book_learning_writing.md` 规则：
- 书的章节只是主题大纲参考，不转录
- 站在技术专家视角重新组织
- 字数无上限，讲清楚为止
- 每章写完后跑 3-agent review（准确性 / 完整性 / 生产落地）
