...
### 面试题 1：mutex 的 futex 实现

**面试官**：`pthread_mutex_lock` 在无竞争时大概多少纳秒？有竞争时呢？为什么差距这么大？

**回答**：无竞争时 ~20ns（一次 `atomic_cmpxchg`——单条 CPU 指令），有竞争时 ~1-10μs（陷入 `futex` 系统调用 + 上下文切换）。原因——mutex 基于 futex（Fast Userspace muTEX）：无竞争时完全在用户态用原子 CAS 完成，只在真正有竞争时才通过 `futex(FUTEX_WAIT)` 陷入内核将线程放入等待队列。用户态快速路径 + 内核态等待队列——这是 Linux 同步原语的核心设计。

**追问 1（面试官）**：那 `pthread_spinlock_lock` 和 mutex 的区别呢？什么场景下 spinlock 比 mutex 更快？

**追问 1 回答**：spinlock 不睡眠——用 while 循环反复 `atomic_exchange`（忙等），始终在用户态，不进入内核。所以临界区极短（< 100ns）时 spinlock 更快——因为 mutex 的 futex 系统调用开销 ~1μs 已大于临界区执行时间。但临界区 > 1μs 时 mutex 更快——因为等待时 mutex 线程让出 CPU，spinlock 线程浪费 CPU 核心瞎忙。生产建议：临界区内不睡眠、不做 I/O、不加另一个锁——这些操作远超 spinlock 的适用范围。

**追问 2（面试官）**：futex 是怎么实现"唤醒等待者"的？condvar 的 `pthread_cond_signal` 有什么特殊优化？

**追问 2 回答**：futex 等待队列是内核管理的 per-futex 哈希桶——`FUTEX_WAKE` 从桶中取出等待者，放入调度器的运行队列。condvar 的特殊优化是 **futex requeue**——`pthread_cond_signal` 不是"唤醒等待者→等其者自己抢锁→可能再次阻塞"，而是内核直接把等待者从 condvar 的等待队列移到 mutex 的等待队列——减少一次不必要的上下文切换。这就是为什么 condvar signal 比"自己写 wake+while+lock"性能好。

---

### 面试题 2：condvar 的 while vs if 和信号丢失

**面试官**：为什么 `pthread_cond_wait` 返回后必须用 `while` 重新检查条件而不是 `if`？

**回答**：两个原因——(1) 假唤醒（spurious wakeup）：POSIX 允许 `pthread_cond_wait` 在没有 signal 的情况下返回，醒来后条件可能仍不满足；(2) 争用：另一个消费者可能先拿到锁并消费了数据——条件在你释放锁和重新获取锁之间被改回了"不满足"。用 `while` 在每次醒来后重新检查——`if` 则只检查一次，在假唤醒或争用场景下读到错误（可能已不存在的）数据。

**追问 1（面试官）**：再深一层——为什么 `pthread_cond_wait` 需要传入一个 mutex？信号丢失是什么，怎么发生的？

**追问 1 回答**：如果 `pthread_cond_wait` 不传 mutex，消费者在"检查条件（buffer==-1）"和"调用 wait 进入睡眠"之间有一个窗口——生产者在这个窗口写入数据并发 signal，signal 抵达时消费者还没进入睡眠状态→signal 丢失→消费者永远等不到。mutex 让"检查条件+wait"变成原子的——生产者必须获取同一把 mutex 才能写入数据，消除了这个窗口。这是条件变量的核心设计。

**追问 2（面试官）**：`pthread_cond_signal` 和 `pthread_cond_broadcast` 怎么选？

**追问 2 回答**：signal 唤醒至少一个等待者（可能只唤醒一个），broadcast 唤醒所有。用 broadcast 的情况——(1) 多个消费者等待不同条件但共享同一个 condvar（如资源池有多个不同大小的资源可用，消费者各需不同大小——必须 broadcast 让所有人自查）；(2) 状态变化所有等待者都需要重新评估（如 shutdown 信号）。不确定时用 broadcast——安全性优先，且 futex requeue 优化让 broadcast 的性能代价可控。

---

### 面试题 3：线程池与线程生命周期

**面试官**：每秒处理 10000 个短任务——每个请求 `pthread_create` + `pthread_join` 有什么问题？怎么优化？

**回答**：每次 create+join 消耗 15-30μs，10000 次 = 150-300ms 的开销——还没算任务本身。线程池解决——预创建 N 个 worker 线程（通常 CPU 核数 × 1-2），用一个任务队列（配合 mutex + condvar）协调。任务到来时放入队列，空闲 worker 用 condvar_wait 等待新任务，取出并执行。创建/销毁开销从"每请求一次"降到"进程启动一次"。

**追问 1（面试官）**：线程池大小怎么定？4 核机器上设 4 个线程还是 40 个？

**追问 1 回答**：CPU 密集型→核数（4 核 = 4 线程）——多余线程不获调度，只会增加上下文切换。IO 密集型→核数 × 2-4（4 核 = 8-16 线程）——IO 等待时其他线程可以跑。线程过多的问题——(1) 默认 8MB 栈/线程，1000 线程 = 8GB 虚拟地址空间；(2) 上下文切换增多，O(n) 的 CFS 调度开销上升；(3) 缓存竞争——多个线程争夺同一 L2/L3 缓存行。`pthread_attr_setstacksize` 可以减栈大小但每次线程调用递归函数可能溢出——这是 trade-off。

**追问 2（面试官）**：detached 线程有什么坑？

**追问 2 回答**：(1) 无法 join→无法获取返回值；(2) detached 线程访问创建者栈变量→创建函数返回后栈变量失效→UAF（这是最隐蔽的 bug，ASan 可能检测不到因为栈内存还没被覆盖）；(3) pthread_key destructor 的清理顺序不确定——如果 destructor 依赖其他全局资源，后一个线程退出时这些资源可能已被另一个线程先清理了。
